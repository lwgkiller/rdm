...
<div>
    ...
    <#if (searchSize>0) >
    <rx-search slot="top" v-model="search.${searchName}" v-on:input="handSearch" <#if (searchSize>1) > :senior="true" v-on:showsenior="showSenior()" </#if> ></rx-search>
    </#if>
    <!--参数-->
    <#if (searchSize>1) >
    	...
    	<div class="search-params">
    		...
    		<div class="search-content">
    			<#if (item.fc=="mini-textbox") >
    				<rx-input  v-model="search.${getSearchParams(item)}"  placeholder="请输入${item.fieldLabel}"/>
    			...
    			<#elseif (item.fc=="mini-dialog" && item.dialogalias??) >
    ...
    <!-- 列表字段 -->
    <yd-pullrefresh :callback="refreshList" ref="pullrefreshList_">
