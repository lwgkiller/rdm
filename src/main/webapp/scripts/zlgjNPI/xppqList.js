...
//新增流程（后台根据配置的表单进行跳转）
function addZlgj() {
    var url = jsUseCtxPath + "/bpm/core/bpmInst/XPPQYZ/start.do";
...
function zlgjDetail(wtId, status) {
    var action = "detail";
    var url = jsUseCtxPath + "/zlgjNPI/core/xppq/editPage.do?action=" + action + "&wtId=" + wtId + "&status=" + status;
    var winObj = window.open(url);
    var loop = setInterval(function () {
        if (winObj.closed) {
            clearInterval(loop);
...
function removeZlgj(record) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            if (rowIds.length > 0){
                _SubmitJson({
                    url: jsUseCtxPath + "/zlgjNPI/core/xppq/deleteZlgj.do",
                    method: 'POST',
                    showMsg:false,
