...
function _OpenWindow(config){
	...
	var win=mini.open({
        ...
        showCloseButton: config.showCloseButton, //显示关闭按钮
    ...
    _CommonDialog({
        ...
        ondestroy: function (action) {
            ...
            if (rows != null && rows.length > 0) {
                ...
                //@lwgkiller:此处应该加上主动激发调用组件的值变事件，因为miniui的buttonedit控件不自动激发
		...
		if($('.mini-panel-body .mini-grid-rows-view').length){
			try{
				...
				grid.on('load',function(e){
					if(e.data.length == 0 && !$('.listEmpty').length){
						$('#'+grid.id+' .mini-panel-body .mini-grid-rows-view').append("<div class='listEmpty'><img src='"+__rootPath+"/styles/images/index/empty2.png'/><span>暂无数据</span></div>");
...
/**
 * 通过流程实例Id找到当前待办任务，钉钉提醒处理人
 * @param instId
 */
function taskRemind(instId) {
    if(!instId) {
        instId='';
    }
    $.ajax({
        url:__rootPath+"/rdm/core/taskRemind.do?instId="+instId,
 ...
 function renderTextAreaWrapAutoHeight(e) {
     var record = e.record;
     var html = "<div style='line-height: 20px;overflow: auto' >";
     var contentStr = record[e.field];
     if (contentStr == null) {
         contentStr = "";
     }
     html += '<span style="white-space:pre-wrap" >' + contentStr + '</span>';
     html += '</div>'
     return html;
 }

function renderTextAreaWrapFixHeight(e) {
    var record = e.record;
    var html = "<div style='line-height: 20px;height:140px;overflow: auto' >";
    var contentStr = record[e.field];
    if (contentStr == null) {
        contentStr = "";
    }
