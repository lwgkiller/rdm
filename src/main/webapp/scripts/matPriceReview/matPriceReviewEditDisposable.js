...
function saveOrCommitMatPriceReview(scene) {
    ...
    postData.requireArrivalTime=mini.get("requireArrivalTime").getText();
    //必填项检查
...
function applyBaseInfoValid(postData) {
    ...
    if(!postData.zlFeature) {
        result.success=false;
        result.message="请选择质量特性";
        return result;
    }
    ...
    if(!postData.moneyCategory) {
        result.success=false;
        result.message="请选择币种";
        return result;
    }
    if(!postData.zxRate) {
        result.success=false;
...
function matDetailInfoValid(postData) {
    ...
    if(!postData.jghs) {
        result.success=false;
        result.message="请填写价格(含税)";
        return result;
    }
    if(!postData.jgbhs) {
        result.success=false;
        result.message="请填写价格(不含税)";
        return result;
    }
    if(!postData.yfcdf) {
        result.success=false;
