...
*/
function checkAndHandTask(uid,fromMgr,fromPortal,colId,belongSubSysKey){
	if(fromPortal){
		$.ajax({
			...
			success:function (result) {
				if(!result.success){
					...
					handTask(uid,false,fromPortal,belongSubSysKey);
				}
			}
		...
		$.ajax({
			...
			success:function (result) {
				if(!result.success){
					...
					handTask(uid,fromMgr,false,belongSubSysKey);
				}
			}
 ...
 * @returns
 */
function handTask(uid,fromMgr,fromPortal,belongSubSysKey){
	var row = {id:uid};
	if(!fromPortal){
		row = grid.getRowByUID(uid);
	}
	var pageUrl='/bpm/core/bpmTask/toStart.do?taskId='+row.id + '&fromMgr=' + fromMgr;
    var url = __rootPath+"/pageJumpRedirect.do?targetSubSysKey="+belongSubSysKey+"&targetUrl="+encodeURIComponent(pageUrl);
