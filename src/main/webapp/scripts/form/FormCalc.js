...
function FormCalc(formId){
	...
	*/
	...
	this.calcDateSubTableStart=function(e){
		...
		for(var key in dateStart){
			...
			if(setting.type=="today"){
				...
				var date=row[name];
				if(!val) return;
				var val=mini.clone(date);
				
				if(setting.compare=="GTE"){
					ctl.setMinDate(val.Min());
				}
				else if(setting.compare=="GT"){
					ctl.setMinDate(val.add(1,"day"));
				}
				else if(setting.compare=="LTE"){
					ctl.setMaxDate(val.Max());
				}
				else if(setting.compare=="LT"){
					ctl.setMaxDate(val.sub(1,"day"));
				}
			}
		}
	},
		...
		for(var i=0;i<controls.length;i++){
			...
			control.on("focus",function(e){
				...
				if(setting.type=="today"){
					...
					var name=setting.control;
					var date=mini.getByName(name).getValue();
					if(!date) return;
					var val=mini.clone(date);
	 ...
	 * 
	 */
	this.handGridData=function(targetField){//debugger;
