 ...
 * 添加弹窗
 */
function openAtsEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "人才奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/awardTalents/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchAtsList() {
    ...
    data.pageIndex = atsListGrid.getPageIndex();
    data.pageSize = atsListGrid.getPageSize();
    data.sortField = atsListGrid.getSortField();
    data.sortOrder = atsListGrid.getSortOrder();
    //查询
    atsListGrid.load(data);
}
 ...
 * @param record
 */
function removeAts(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = atsListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/awardTalents/deleteAts.do?",
                method: 'POST',
                showMsg: false,
...
//导入
function importAtsMaterialName() {
    importWindow.show();
}

//上传批量导入
function importAtsMaterial() {
    var file = null;
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardTalents/importExcel.do', false);
...
//导入模板下载
function downAtsImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardTalents/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}

function returnAtsPreviewSpan(fileName, fileId, formId, coverContent) {
    var fileType = getFileType(fileName);
    var s = '';
    if (fileType == 'other') {
        s = '<span  title="预览" style="color: silver" >预览</span>';
    } else if (fileType == 'pdf') {
        var url = '/zhgl/core/awardTalents/atsPdfPreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'office') {
        var url = '/zhgl/core/awardTalents/atsOfficePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'pic') {
        var url = '/zhgl/core/awardTalents/atsImagePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPic(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    }
    return s;
}

function downLoadAtsFile(fileName, fileId, jsjlId) {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardTalents/fileDownload.do?action=download");
