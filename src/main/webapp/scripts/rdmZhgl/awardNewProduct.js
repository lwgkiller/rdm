 ...
 * 添加弹窗
 */
function openAnpEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "新产品奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/awardNewProduct/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchAnpList() {
    ...
    if (certificateNumber) {
        queryParam.push({name: "certificateNumber", value: certificateNumber});
    }
    ...
    data.pageIndex = anpListGrid.getPageIndex();
    data.pageSize = anpListGrid.getPageSize();
    data.sortField = anpListGrid.getSortField();
    data.sortOrder = anpListGrid.getSortOrder();
    //查询
    anpListGrid.load(data);
}
 ...
 * @param record
 */
function removeAnp(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = anpListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/awardNewProduct/deleteAnp.do?",
                method: 'POST',
                showMsg: false,
...
//导入
function importAnpMaterialName() {
    importWindow.show();
}

//上传批量导入
function importAnpMaterial() {
    var file = null;
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardNewProduct/importExcel.do', false);
...
//导入模板下载
function downAmpImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardNewProduct/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}

function returnAnpPreviewSpan(fileName, fileId, formId, coverContent) {
    var fileType = getFileType(fileName);
    var s = '';
    if (fileType == 'other') {
        s = '<span  title="预览" style="color: silver" >预览</span>';
    } else if (fileType == 'pdf') {
        var url = '/zhgl/core/awardNewProduct/anpPdfPreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'office') {
        var url = '/zhgl/core/awardNewProduct/anpOfficePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'pic') {
        var url = '/zhgl/core/awardNewProduct/anpImagePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPic(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    }
    return s;
}

function downLoadAnpFile(fileName, fileId, jsjlId) {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardNewProduct/fileDownload.do?action=download");
