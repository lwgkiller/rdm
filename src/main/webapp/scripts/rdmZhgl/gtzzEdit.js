...
function saveData() {
    ...
    var config = {
        url: jsUseCtxPath+"/rdmZhgl/core/gtzz/save.do",
        method: 'POST',
        data: formData,
...
function judgePlanData() {
    ...
    if (data.length > 0) {
        for (var i = 0; i < data.length; i++) {
            ...
            if (!data[i].indexSort || !data[i].important || !data[i].projectName|| !data[i].projectTarget
...
function saveRow() {
    ...
    if (data.length > 0) {
        ...
        if (needReload) {
            ...
            $.ajax({
                url: jsUseCtxPath + "/rdmZhgl/core/gtzz/dealData.do",
                data: json,
                type: "post",
