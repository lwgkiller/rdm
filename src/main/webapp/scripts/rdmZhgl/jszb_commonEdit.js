...
function taskActionProcess() {
    ...
    if(stageName == 'bianzhi') {
        ...
        formJszb.setEnabled(false);
        mini.get("xmNum").setEnabled(true);
    } else {
        formJszb.setEnabled(false);
    }
}
...
function commitProcessValidCheck() {
    ...
    if (!zbbmId) {
        return {"result": false, "message": "请选择招标部门"};
    }
    ...
    if(!zbName) {
        return {"result": false, "message": "请填写招标项目名称"};
    }

    var zbjg=$.trim(mini.get("zbjg").getValue());
    if(!zbjg) {
        return {"result": false, "message": "请填写招标总价或项目预算"};
    }
    var zbzzxs=$.trim(mini.get("zbzzxs").getValue());
    if(!zbzzxs) {
        return {"result": false, "message": "请选择招标组织形式"};
    }
    var zbxs=$.trim(mini.get("zbxs").getValue());
    if(!zbxs) {
        return {"result": false, "message": "请选择招标形式"};
    }
    var lxTime=$.trim(mini.get("lxTime").getValue());
    if(!lxTime) {
        return {"result": false, "message": "请选择立项时间"};
    }
    var publishType=$.trim(mini.get("publishType").getValue());
    if(!publishType) {
        return {"result": false, "message": "请选择信息发布方式"};
    }
