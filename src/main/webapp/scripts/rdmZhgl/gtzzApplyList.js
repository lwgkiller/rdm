...
//新增计划审批流程
function doApply() {
    var url = jsUseCtxPath + "/bpm/core/bpmInst/GTDZSP/start.do";
...
function removeApply(record) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            if(ids.length>0) {
                _SubmitJson({
                    url: jsUseCtxPath+"/rdmZhgl/core/gtzzApply/delete.do",
                    method: 'POST',
...
function detailApply(id,status,applyType) {
    var action = "detail";
    var url=jsUseCtxPath+"/rdmZhgl/core/gtzzApply/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
