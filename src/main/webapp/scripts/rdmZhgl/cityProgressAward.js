 ...
 * 添加弹窗
 */
function openCpaEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "市级及以上科技进步奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/cityProgressAward/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchCpaList() {
    ...
    data.pageIndex = cpaListGrid.getPageIndex();
    data.pageSize = cpaListGrid.getPageSize();
    data.sortField = cpaListGrid.getSortField();
    data.sortOrder = cpaListGrid.getSortOrder();
    //查询
    cpaListGrid.load(data);
}
 ...
 * @param record
 */
function removeCpa(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = cpaListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/cityProgressAward/deleteCpa.do?",
                method: 'POST',
                showMsg: false,
...
function importMaterial() {
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/cityProgressAward/importExcel.do', false);
...
//导入模板下载
function downCpaImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/cityProgressAward/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}

function returnCpaPreviewSpan(fileName, fileId, formId, coverContent) {
    var fileType = getFileType(fileName);
    var s = '';
    if (fileType == 'other') {
        s = '<span  title="预览" style="color: silver" >预览</span>';
    } else if (fileType == 'pdf') {
        var url = '/zhgl/core/cityProgressAward/cpaPdfPreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'office') {
        var url = '/zhgl/core/cityProgressAward/cpaOfficePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'pic') {
        var url = '/zhgl/core/cityProgressAward/cpaImagePreview.do';
...
function downLoadJsjlFile(fileName, fileId, jsjlId) {
    ...
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/cityProgressAward/fileDownload.do?action=download");
