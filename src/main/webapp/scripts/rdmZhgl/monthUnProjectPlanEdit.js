$(function () {
    ...
    if (action == 'finish') {
        ...
        mini.get('processRate').setEnabled(true);
        mini.get('processStatus').setEnabled(true);
        mini.get('stageId').setEnabled(true);
...
function saveData() {
    ...
    var stageId = mini.get('stageId').getValue();
    if (!processRate) {
        mini.alert("此项目进度百分比必填！");
        return;
    }
    if (!processStatus) {
        mini.alert("此项目进度状态必填！");
        return;
    }
    if (!stageId) {
        mini.alert("当前阶段必填！");
    ...
    var config = {
        url: jsUseCtxPath + "/rdmZhgl/core/monthUnProjectPlan/save.do",
        method: 'POST',
        data: formData,
...
function removeRow() {
    ...
    mini.confirm("确定取消选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            if (ids.length > 0) {
                _SubmitJson({
                    url: jsUseCtxPath + "/rdmZhgl/core/monthUnProjectPlan/removeItem.do",
                    method: 'POST',
                    data: {ids: ids.join(',')},
...
function saveRow() {
    ...
    if (data.length > 0) {
        ...
        if (needReload) {
            ...
            $.ajax({
                url: jsUseCtxPath + "/rdmZhgl/core/monthUnProjectPlan/dealData.do",
                data: json,
                type: "post",
