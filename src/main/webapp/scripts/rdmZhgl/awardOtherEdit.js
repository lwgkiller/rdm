$(function () {
    // console.log("测试数据",aotObj)
    formStandard.setData(aotObj);
    if (action == 'detail') {
 ...
 * 新增或者更新的保存
 */
function saveAot(awardId) {

    var formData = formStandard.getData();

    var formValid = validAotMessage();
    if (!formValid.result) {
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardOther/saveAot.do?awardId='+awardId, false);
