 ...
 * 添加弹窗
 */
function openAgspEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "高品奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/awardGrouplScienceProgress/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchAgpList() {
    ...
    data.pageIndex = agspeListGrid.getPageIndex();
    data.pageSize = agspeListGrid.getPageSize();
    data.sortField = agspeListGrid.getSortField();
    data.sortOrder = agspeListGrid.getSortOrder();
    //查询
    agspeListGrid.load(data);
}
 ...
 * @param record
 */
function removeAgspe(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = agspeListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/awardGrouplScienceProgress/deleteAgp.do?",
                method: 'POST',
                showMsg: false,
...
//导入
function importAgspeMaterialName() {
    importWindow.show();
}

//上传批量导入
function importAgspeMaterial() {
    var file = null;
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardGrouplScienceProgress/importExcel.do', false);
...
//导入模板下载
function downAgspeImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardGrouplScienceProgress/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}

function returnAgpPreviewSpan(fileName, fileId, formId, coverContent) {
    var fileType = getFileType(fileName);
    var s = '';
    if (fileType == 'other') {
        s = '<span  title="预览" style="color: silver" >预览</span>';
    } else if (fileType == 'pdf') {
        var url = '/zhgl/core/awardGrouplScienceProgress/cpaPdfPreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'office') {
        var url = '/zhgl/core/awardGrouplScienceProgress/cpaOfficePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'pic') {
        var url = '/zhgl/core/awardGrouplScienceProgress/cpaImagePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPic(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    }
    return s;
}

function downLoadAgpFile(fileName, fileId, jsjlId) {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardGrouplScienceProgress/fileDownload.do?action=download");
