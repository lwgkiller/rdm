...
function removeApply(record) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            if(ids.length>0) {
                _SubmitJson({
                    url: jsUseCtxPath+"/rdmZhgl/core/delayApply/delete.do",
                    method: 'POST',
...
function detailApply(id,status,applyType) {
    var action = "detail";
    var url=jsUseCtxPath+"/rdmZhgl/core/delayApply/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
...
function exportDelayApply(){
    var rows = applyListGrid.getSelecteds();
    if (rows.length < 1) {
        mini.alert("最少选择一条申请单进行导出！");
        return;
    }
    var itemIds = rows.map(function (item) {
        return item.id;
    })
    var ids = itemIds.join(',');
    var params = [];
    params.push({name: 'mainIds',value: ids});
    $("#filter").val(mini.encode(params));
    var excelForm = $("#excelForm");
    excelForm.attr("action", jsUseCtxPath + "/rdmZhgl/core/applyDetail/exportDelayApply.do");
    excelForm.submit();
}
