...
//打开标准的编辑页面（新增、编辑、明细）
function openStandardEditWindow(standardId,systemId, action,njjdId) {
    ...
    mini.open({
        title: "用戶编辑",
        url: jsUseCtxPath + "/zhgl/core/njjd/editUser.do?standardId=" + standardId + '&action=' + action+'&njjdId='+ njjdId,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
//预览标准全文
function previewStandard(standardId, standardName, standardNumber, categoryName, status,coverConent) {
    if(status=='disable') {
        mini.confirm("该标准已废止，确定预览吗？","提示",function (action) {
            if(action=="ok") {
                previewStandardDo(standardId, standardName, standardNumber, categoryName, status,coverConent);
            }
        });
    } else {
        previewStandardDo(standardId, standardName, standardNumber, categoryName, status,coverConent);
...
function previewStandardDo(standardId, standardName, standardNumber, categoryName, status, coverConent) {
