...
function removeApply(record) {
    ...
    mini.confirm(saleFileApplyList_name5, saleFileApplyList_name6, function (action) {
        if (action != 'ok') {
            ...
            if(ids.length>0) {
                _SubmitJson({
                    url: jsUseCtxPath+"/rdmZhgl/core/saleFile/delete.do",
                    method: 'POST',
...
function detailApply(id,status,applyType) {
    var action = "detail";
    var url=jsUseCtxPath+"/rdmZhgl/core/saleFile/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
...
//新增提报审批流程
function doApply() {
    var url = jsUseCtxPath + "/bpm/core/bpmInst/SQWJSP/start.do?applyType="+applyType;
    var winObj = window.open(url);
    var loop = setInterval(function () {
        if (winObj.closed) {
            clearInterval(loop);
