...
function removeApply(record) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            if(ids.length>0) {
                _SubmitJson({
                    url: jsUseCtxPath+"/rdmZhgl/core/gzxm/projectApply/delete.do",
                    method: 'POST',
...
function detailApply(id,status,applyType) {
    var action = "detail";
    var url=jsUseCtxPath+"/rdmZhgl/core/gzxm/projectApply/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
