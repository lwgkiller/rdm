 ...
 * 添加弹窗
 */
function openAgpEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "集团科技进步奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/awardGroupProgress/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchAgpList() {
    ...
    data.pageIndex = agpListGrid.getPageIndex();
    data.pageSize = agpListGrid.getPageSize();
    data.sortField = agpListGrid.getSortField();
    data.sortOrder = agpListGrid.getSortOrder();
    //查询
 ...
 * @param record
 */
function removeAgp(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = agpListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/awardGroupProgress/deleteAgp.do?",
                method: 'POST',
                showMsg: false,
...
function importAgpMaterial() {
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardGroupProgress/importExcel.do', false);
...
//导入模板下载
function downAgpImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardGroupProgress/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}
...
function returnAgpPreviewSpan(fileName, fileId, formId, coverContent) {
    ...
    if (fileType == 'other') {
        ...
        var url = '/zhgl/core/awardGroupProgress/cpaPdfPreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'office') {
        var url = '/zhgl/core/awardGroupProgress/cpaOfficePreview.do';
        s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
    } else if (fileType == 'pic') {
        var url = '/zhgl/core/awardGroupProgress/cpaImagePreview.do';
...
function downLoadAgpFile(fileName, fileId, jsjlId) {
    ...
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardGroupProgress/fileDownload.do?action=download");
