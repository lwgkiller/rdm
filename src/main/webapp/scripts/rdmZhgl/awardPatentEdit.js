 ...
 * 添加弹窗
 */
function openApeEditWindow( action, awardId) {
    var title = "";
    if (action == "add") {
        title = "专利奖添加"
    }else {
    ...
    mini.open({
        title: title,
        url: jsUseCtxPath + "/zhgl/core/awardPatent/edit.do?awardId=" + awardId + '&action='+ action ,
        width: 800,
        height: 600,
        showModal: true,
        allowResize: true,
...
function searchAgpList() {
    ...
    data.pageIndex = apeListGrid.getPageIndex();
    data.pageSize = apeListGrid.getPageSize();
    data.sortField = apeListGrid.getSortField();
    data.sortOrder = apeListGrid.getSortOrder();
    //查询
 ...
 * @param record
 */
function removeApe(record) {

    var rows = [];
    if (record) {
        rows.push(record);
    } else {
        rows = apeListGrid.getSelecteds();
    }
    if (rows.length <= 0) {
    ...
    mini.confirm("确定删除选中记录？", "提示", function (action) {
        if (action != 'ok') {
            ...
            _SubmitJson({
                url: jsUseCtxPath + "/zhgl/core/awardPatent/deleteAgp.do?",
                method: 'POST',
                showMsg: false,
...
//导入
function importApeMaterialName() {
    importWindow.show();
}

//上传批量导入
function importApeMaterial() {
    var file = null;
    ...
    if (xhr.upload) {
        ...
        // 开始上传
        xhr.open('POST', jsUseCtxPath + '/zhgl/core/awardPatent/importExcel.do', false);
...
//导入模板下载
function downApeImportTemplate() {
    var form = $("<form>");
    form.attr("style", "display:none");
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardPatent/importTemplateDownload.do");
    $("body").append(form);
    form.submit();
    form.remove();
}
...
function downLoadAgpFile(fileName, fileId, jsjlId) {
    ...
    form.attr("target", "");
    form.attr("method", "post");
    form.attr("action", jsUseCtxPath + "/zhgl/core/awardPatent/fileDownload.do?action=download");
