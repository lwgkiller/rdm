...
function editApplyRow(applyId,instId) {
    var action = "edit";
    var url=jsUseCtxPath+"/bpm/core/bpmInst/start.do?instId="+instId+"&action="+action;
...
function removeApply(record) {
    ...
    if (rows.length <= 0) {
        mini.alert(projectFileApplyList_name5);
        return;
    }

    mini.confirm(projectFileApplyList_name6, projectFileApplyList_name7, function (action) {
        if (action != 'ok') {
            return;
        } else {
            //判断该行是否已经提交流程
            ...
            if(existStartInst) {
                alert(projectFileApplyList_name8);
            }
            if(ids.length>0) {
                _SubmitJson({
...
//明细 的点击查看方法
function detailFileApply(id,status) {
    var action = "detail";
    var url=jsUseCtxPath+"/xcmgProjectManager/core/xcmgProjectFile/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
...
function changeApplyRow(applyId) {
    var action = "change";
    var url=jsUseCtxPath+"/xcmgProjectManager/core/xcmgProjectFile/edit.do?action="+action+"&id=" + applyId;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
