...
function removeApply(record) {
    ...
    if (rows.length <= 0) {
        mini.alert(projectAbolishApplyList_name4);
        return;
    }

    mini.confirm(projectAbolishApplyList_name5, projectAbolishApplyList_name6, function (action) {
        if (action != 'ok') {
            return;
        } else {
            //判断该行是否已经提交流程
            ...
            if(existStartInst) {
                alert(projectAbolishApplyList_name7);
            }
            if(ids.length>0) {
                _SubmitJson({
                    url: jsUseCtxPath+"/xcmgProjectManager/core/xcmgProjectAbolish/delete.do",
                    method: 'POST',
...
//明细 的点击查看方法
function detailAbolishApply(id,status) {
    var action = "detail";
    var url=jsUseCtxPath+"/xcmgProjectManager/core/xcmgProjectAbolish/edit.do?action="+action+"&id=" + id+"&status="+status;
    var winObj = window.open(url);
    var loop = setInterval(function() {
        if(winObj.closed) {
            clearInterval(loop);
