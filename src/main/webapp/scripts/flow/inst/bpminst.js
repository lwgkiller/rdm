 ...
 * @returns
 */
function startProcess(e,manualSkipFirst,skipUrl) {
    if (manualSkipFirst) {
        conf.manualSkipFirst = manualSkipFirst;
        conf.skipUrl = skipUrl;
    }
				...
				if(window.afterStart){
                    ...
                    mini.showMessageBox({
                        ...
                        callback: function (action) {
                            if (conf.manualSkipFirst) {
                                var url = __rootPath +conf.skipUrl+ "?instId="+result.data.instId;
                                $.ajax({
                                    url:url,
                                    method:'get',
	...
	OfficeControls.save(function(){
		...
		_SubmitJson({
			...
			success : function(result) {
				...
				if(conf.closeWindow=="notClose"){
					var url=__rootPath+"/bpm/core/bpmInst/start.do?instId="+result.data.instId;
