...
<head>
	...
	<script src="${ctxPath}/scripts/xcmgJsjl/xcmgJsjlEdit.js?version=${static_res_version}" type="text/javascript"></script>
</head>

<body>
<div id="toolBar" class="topToolBar">
	<div>
		<a id="saveJsjlDraft" class="mini-button" style="display: none" onclick="saveJsjlDraft()">保存草稿</a>
		<a id="commitJsjl" class="mini-button" style="display: none" onclick="commitJsjl()">提交</a>
		<a id="feedbackJsjl" class="mini-button" style="display: none" onclick="feedbackJsjl()">保存</a>
...
<div class="mini-fit">
	<div class="form-container" style="margin:0 auto; width: 100%;height:98%;padding: 0">
		<form id="jsjlForm" method="post" style="height: 95%;width: 100%">
			<input class="mini-hidden" id="id" name="id" />
			<table class="table-detail grey"  cellspacing="1" cellpadding="0" style="height: 100%;width: 100%">
				<caption style="font-weight: bold;font-size: 20px !important;">外部会议管理</caption>
				<tr>
					<td style="text-align: center;width: 20%">组织部门：</td>
					...
					<td style="text-align: center;width: 20%">业务接待人：</td>
					<td style="min-width:170px">
						<input id="receiveUserId" name="receiveUserId" textname="receiveUserName" class="mini-user rxc"
							   plugins="mini-user" style="width:98%;height:34px;" allowinput="false" label="业务接待人" length="50" showclose="false"
							   mainfield="no"  single="true" />
					</td>
				</tr>
				<tr>
					<td style="text-align: center;width: 20%">参会人员：</td>
					<td style="min-width:170px">
						<input id="meetingUserIds" name="meetingUserIds" textname="meetingUserNames" class="mini-user rxc"
							   plugins="mini-user" style="width:98%;height:34px;" allowinput="false" label="参会人员" length="50"
							   mainfield="no"  single="false" />
					</td>
					<td style="text-align: center;width: 20%">交流时间：</td>
					<td style="min-width:170px">
						<input id="communicateTime"  name="communicateTime"  class="mini-datepicker" format="yyyy-MM-dd HH:mm:ss"
							   timeFormat="HH:mm:ss" showTime="true" showOkButton="true" showClearButton="false" style="width:98%;"/>
					</td>
				</tr>
				<tr>
					<td style="text-align: center;width: 20%">交流地点：</td>
					<td style="min-width:170px">
						<input id="communicateRoom"  name="communicateRoom"  class="mini-textbox" style="width:98%;"/>
					</td>
					<td style="text-align: center;width: 20%">交流单位：</td>
					<td style="min-width:170px">
						<input style="width:98%;" class="mini-textbox" id="communicateCompany" name="communicateCompany" />
					</td>
				</tr>
				<tr>
					<td style="text-align: center;width: 20%">会议类型：</td>
					<td style="min-width:170px">
						<input id="dimensionId" name="dimensionId" class="mini-combobox" style="width:98%;"
                ...
                <tr>
                    <td style="text-align: center;width: 20%">是否钉钉发送参会通知：</td>
                    <td style="min-width:170px">
				...
				<tr>
					<td style="text-align: center;width: 20%">交流内容、结论（1500字以内）：</td>
					<td colspan="3">
				...
				<tr>
					...
					<td colspan="3">
						<div class="mini-toolbar" id="fileButtons" style="display: none">
							<a id="uploadFile" class="mini-button"  style="margin-bottom: 5px;margin-top: 5px" onclick="uploadJsjlFile">上传文件</a>
						</div>
						<div id="fileListGrid" class="mini-datagrid" style="width: 100%; height: 85%" allowResize="false"
							 idField="id" url="${ctxPath}/jsjl/core/file/dataList.do?jsjlId=${jsjlId}"
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <div class="mini-toolbar" id="planButtons" style="display: none">
                            ...
                            <a id="deletePlan" class="mini-button btn-red" style="margin-bottom: 5px;margin-top: 5px" onclick="deleteMeetingPlan">删除任务</a>
                            <a class="mini-button" style="margin-right: 5px" plain="true" onclick="exportPlanBusiness()">导出</a>
                            <div style="display: inline-block" class="separator"></div>
                            <a id="savePlan" class="mini-button btn-yellow" style="margin-bottom: 5px;margin-top: 5px" onclick="saveMeetingPlan">保存任务</a>
                        ...
                        <div id="planListGrid" class="mini-datagrid" style="width: 100%; height: 85%" allowResize="false" allowCellWrap="true"
                             idField="id" url="${ctxPath}/jsjl/core/getMeetingPlanList.do?meetingId=${jsjlId}"
...
<!--导出Excel相关HTML-->
<form id="excelForm" action="${ctxPath}/jsjl/core/exportPlanList.do" method="post" target="excelIFrame">
...
<script type="text/javascript">
    ...
    var jsjlForm = new mini.Form("#jsjlForm");
    ...
    function operationRenderer(e) {
        var record = e.record;
        var cellHtml = '';
        cellHtml=returnJsjlPreviewSpan(record.fileName,record.id,record.jsjlId);
		cellHtml+='&nbsp;&nbsp;&nbsp;<span title="下载" style="color:#409EFF;cursor: pointer;" ' +
                'onclick="downLoadJsjlFile(\''+record.fileName+'\',\''+record.id+'\',\''+record.jsjlId+'\')">下载</span>';
        //增加删除按钮
        if(record.CREATE_BY_==currentUserId && action!='detail') {
            cellHtml+='&nbsp;&nbsp;&nbsp;<span title="删除" style="color:#409EFF;cursor: pointer;" ' +
                'onclick="deleteJsjlFile('+JSON.stringify(record).replace(/"/g, '&quot;')+')">删除</span>';
        }
        return cellHtml;
    }
