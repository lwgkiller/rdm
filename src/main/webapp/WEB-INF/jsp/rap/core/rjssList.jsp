...
<div class="mini-fit" style="height: 100%;">
    <div id="rjssListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         sizeList="[50,100,200]" pageSize="50"
         url="${ctxPath}/environment/core/Rjss/queryRjss.do"
        ...
        <div property="columns">
            ...
            <div field="fdjModel"  width="40" headerAlign="center" align="center" allowSort="true">发动机型号</div>
...
<script type="text/javascript">
    ...
    var rjssListGrid = mini.get("rjssListGrid");
    ...
    function onMessageActionRenderer(e) {
        var record = e.record;
        var rjssId = record.rjssId;
        var instId = record.instId;
        var status = record.status;
        var s = '';
        if (status != 'DRAFTED'){
            s += '<span  title="明细" onclick="rjssDetail(\'' + rjssId + '\',\'' + status + '\')">明细</span>';
        }
        if (record.status == 'DRAFTED') {
            s += '<span  title="编辑" onclick="rjssEdit(\'' + rjssId + '\',\'' + instId + '\')">编辑</span>';
        }
        if(record.status =='RUNNING'){
            if(record.myTaskId) {
                s+='<span  title="办理" onclick="rjssTask(\'' + record.myTaskId + '\')">办理</span>';
            }
        }
        if (status == 'DRAFTED' ) {
            s += '<span  title="删除" onclick="removeRjss(' + JSON.stringify(record).replace(/"/g, '&quot;') + ')">删除</span>';
        }
    ...
    function addRjss() {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/FDJBGSS/start.do";
    ...
    function rjssEdit(rjssId,instId) {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/start.do?instId=" + instId+ "&rjssId=" + rjssId;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function rjssDetail(rjssId,status) {
        var action = "detail";
        var url = jsUseCtxPath + "/environment/core/Rjss/editPage.do?action=" + action + "&rjssId=" + rjssId+ "&status=" + status;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function removeRjss(record) {
        var rows = [];
        if (record) {
            rows.push(record);
        } else {
            rows = rjssListGrid.getSelecteds();
        }
        if (rows.length <= 0) {
        ...
        mini.confirm("确定删除选中记录？", "提示", function (action) {
            if (action != 'ok') {
                ...
                for (var i = 0, l = rows.length; i < l; i++) {
                    var r = rows[i];
                    rowIds.push(r.rjssId);
                }
                _SubmitJson({
                    url: jsUseCtxPath + "/environment/core/Rjss/deleteRjss.do",
                    method: 'POST',
                    showMsg:false,
...
<redxun:gridScript gridId="rjssListGrid" entityName="" winHeight="" winWidth="" entityTitle="" baseUrl=""/>
</body>
</html>
