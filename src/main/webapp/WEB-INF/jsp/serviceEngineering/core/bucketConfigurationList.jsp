        ...
        <form id="searchForm" class="search-form" style="margin-bottom: 25px">
            <ul>
                <li style="margin-right: 15px">
                    ...
                    <input class="mini-textbox" id="repUser" name="repUser"/>
                </li>
                ...
                <li style="margin-left: 10px">
                    ...
                    <a id="addBusinessFile" class="mini-button" style="margin-right: 5px" onclick="addBusinessFile()" visible="false">参考文件</a>
                    <a id="testBusiness" class="mini-button btn-red" style="margin-right: 5px" onclick="testBusiness()" visible="false">人工触发自动任务</a>
                </li>
            </ul>
        </form>
...
<div class="mini-fit" style="height: 100%;">
    <div id="businessListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false" allowCellWrap="true"
         ...
         url="${ctxPath}/serviceEngineering/core/bucketConfiguration/dataListQuery.do">
        <div property="columns">
            ...
            <div field="repUser" width="120" headerAlign="center" align="center" allowSort="true">责任人</div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        if (isCustomsDeclarationAdmin == 'true' || currentUserNo == 'admin') {
            mini.get("addBusinessFile").setVisible(true);
            mini.get("testBusiness").setVisible(true);
        }
    });
    //..行功能按钮
    function onActionRenderer(e) {
        ...
        if ((record.status == 'DRAFTED' && (currentUserId == record.repUserId || currentUserNo == 'admin'))) {
        ...
        if (currentUserNo != 'admin') {
            if (record.status == 'DRAFTED' && currentUserId == record.repUserId) {
    ...
    function addBusiness() {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/bucketConfiguration/start.do";
    ...
    function businessDetail(businessId) {
        var action = "detail";
        var url = jsUseCtxPath + "/serviceEngineering/core/bucketConfiguration/editPage.do?action=" + action +
        ...
        mini.confirm('确定删除选中记录', '提示', function (action) {
            if (action != 'ok') {
                ...
                for (var i = 0, l = rows.length; i < l; i++) {
                    var r = rows[i];
                    if (r.businessStatus == 'A' && currentUserId == r.repUserId) {
                        rowIds.push(r.id);
                ...
                if (haveSomeRowsWrong) {
                    mini.alert('仅编辑状态数据可由责任人删除,或者admin可以强制删除');
                    return;
                }
                _SubmitJson({
                    url: jsUseCtxPath + "/serviceEngineering/core/bucketConfiguration/deleteBusiness.do",
                    method: 'POST',
                    showMsg: false,
    ...
    function addBusinessFile() {
        mini.open({
            ...
            "businessId=template&businessType=template&action=edit&coverContent=" + coverContent,
            width: 1000,
            height: 600,
            showModal: true,
            allowResize: true,
    ...
    function testBusiness() {
        _SubmitJson({
            url: jsUseCtxPath + "/serviceEngineering/core/bucketConfiguration/testBusiness.do",
            method: 'POST',
            showMsg: false,
            data: {},
            success: function (data) {
