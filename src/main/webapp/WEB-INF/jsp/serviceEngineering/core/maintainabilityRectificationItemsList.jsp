                ...
                <li style="margin-right: 15px"><span class="text" style="width:auto">审核状态: </span>
                    <input id="businessStatus" name="businessStatus" class="mini-combobox" style="width:120px;"
                           ...
                           data="[{'key' : 'A-editing','value' : '编制中'},
							   ...
							   {'key' : 'D-approving','value' : '批准中'},
							   {'key' : 'E-close','value' : '审批通过'}]"
                    />
                </li>
                <li style="margin-left: 10px">
                    <f:a alias="rectificationItems-searchFrm" onclick="searchFrm()" showNoRight="false">查询</f:a>
                    <a class="mini-button btn-red" style="margin-right: 5px" plain="true" onclick="clearForm()">清空查询</a>
                    <f:a alias="rectificationItems-addBusiness" onclick="addBusiness()" showNoRight="false">新增</f:a>
                    <f:a alias="rectificationItems-removeBusiness" onclick="removeBusiness()" showNoRight="false">删除</f:a>
                </li>
            </ul>
        </form>
...
<div class="mini-fit" style="height: 100%;">
    <div id="maintainabilityRectificationItemsListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         allowCellWrap="false"
         ...
         url="${ctxPath}/serviceEngineering/core/maintainability/rectificationitems/dataListQuery.do">
...
<%--            <div field="status" width="80" headerAlign="center" align="center" allowSort="false" renderer="render">流程状态</div>--%>
            ...
            <div field="approverUserName" width="80" headerAlign="center" align="center" allowSort="false" renderer="render">历史批准人</div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    var maintainabilityRectificationItemsListGrid = mini.get("maintainabilityRectificationItemsListGrid");
    ...
    function addBusiness() {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/rectificationItems/start.do?";
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
                if (maintainabilityRectificationItemsListGrid) {
                    maintainabilityRectificationItemsListGrid.reload()
                }
            }
        }, 1000);
    }
    ...
    function businessDetail(businessId, status) {
        var action = "detail";
        var url = jsUseCtxPath + "/serviceEngineering/core/maintainability/rectificationitems/editPage.do?action=" + action +
    ...
    function removeBusiness(record) {
        ...
        if (record) {
            ...
            rows = maintainabilityRectificationItemsListGrid.getSelecteds();
        }
        if (rows.length <= 0) {
        ...
        mini.confirm("确定删除选中记录？", "提示", function (action) {
            if (action != 'ok') {
                ...
                _SubmitJson({
                    url: jsUseCtxPath + "/serviceEngineering/core/maintainability/rectificationitems/deleteBusiness.do",
                    method: 'POST',
                    showMsg: false,
    ...
    function onStatusRenderer(e) {
        ...
        var arr = [{'key': 'A-editing', 'value': '编制中', 'css': 'blue'},
            ...
            {'key': 'C-reviewing', 'value': '审核中', 'css': 'purple'},
            {'key': 'D-approving', 'value': '批准中', 'css': 'red'},
            {'key': 'E-close', 'value': '审批通过', 'css': 'green'}
        ];

        return $.formatItemValue(arr, businessStatus);
    }
</script>
<redxun:gridScript gridId="maintainabilityRectificationItemsListGrid" entityName="" winHeight="" winWidth="" entityTitle="" baseUrl=""/>
</body>
</html>