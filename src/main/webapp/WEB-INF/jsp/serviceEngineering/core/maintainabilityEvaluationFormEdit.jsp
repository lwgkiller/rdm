...
<div class="mini-fit">
    <div class="form-container" style="margin: 0 auto; width: 80%;">
        <form id="formMaintainabilityEvaluationForm" method="post">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        <div id="fileListGrid" class="mini-datagrid" style="width: 100%; height: 85%" allowResize="false"
                             idField="id" url="${ctxPath}/serviceEngineering/core/maintainability/evaluationform/getFileList.do?businessId=${businessId}"
                             autoload="true"
...
<script type="text/javascript">
    ...
    var formMaintainabilityEvaluationForm = new mini.Form("#formMaintainabilityEvaluationForm");
    ...
    //..
    $(function () {
        var url = jsUseCtxPath + "/serviceEngineering/core/maintainability/evaluationform/getDetail.do";
        $.post(
            url,
            {businessId: businessId},
            function (json) {
                formMaintainabilityEvaluationForm.setData(json);
                if (action == 'detail') {
                    formMaintainabilityEvaluationForm.setEnabled(false);
    ...
    //..流程引擎调用此方法进行表单数据的获取
    function getData() {
        var formData = _GetFormJsonMini("formMaintainabilityEvaluationForm");
    ...
    function saveBusinessInProcess() {
        ...
        $.ajax({
            url: jsUseCtxPath + '/serviceEngineering/core/maintainability/evaluationform/saveBusiness.do',
            type: 'post',
            async: false,
    ...
    function addBusinessFile() {
        ...
        mini.open({
            title: "文件上传",
            url: jsUseCtxPath + "/serviceEngineering/core/maintainability/evaluationform/fileUploadWindow.do?businessId=" + businessId,
            width: 750,
            height: 450,
            showModal: false,
            allowResize: true,
    ...
    function operationRenderer(e) {
        ...
        if (action == 'edit' || (action == 'task' && isBianzhi == 'yes')) {
            var deleteUrl = "/serviceEngineering/core/maintainability/evaluationform/delFile.do"
    ...
    function returnPreviewSpan(fileName, fileId, formId, coverContent) {
        ...
        if (fileType == 'other') {
            ...
            var url = '/serviceEngineering/core/maintainability/evaluationform/PdfPreview.do';
            s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewPdf(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
        } else if (fileType == 'office') {
            var url = '/serviceEngineering/core/maintainability/evaluationform/OfficePreview.do';
            s = '<span  title="预览" style="color:#409EFF;cursor: pointer;" onclick="previewDoc(\'' + fileName + '\',\'' + fileId + '\',\'' + formId + '\',\'' + coverContent + '\',\'' + url + '\')">预览</span>';
        } else if (fileType == 'pic') {
            var url = '/serviceEngineering/core/maintainability/evaluationform/ImagePreview.do';
