...
<div class="mini-fit">
    <div class="form-container" style="margin:0 auto; width: 100%;">
        <form id="byInventoryForm" method="post">
        ...
        <div style="margin-top: 5px;margin-bottom: 2px">
            ...
            <a id = "importByInventory" class="mini-button" style="margin-right: 5px;display: none" plain="true" onclick="importByInventory()">导入</a>
            <a id = "exportByInventory" class="mini-button" style="margin-right: 5px;display: none" plain="true" onclick="exportByInventory()">导出</a>
        ...
        <div id="inventoryListGrid" class="mini-datagrid" style="height:500px" autoLoad="true"
              oncellbeginedit="OnCellBeginEditInventory" allowCellEdit="true" allowCellWrap="true"
             idField="id" url="${ctxPath}/serviceEngineer/core/ByInventory/queryDetail.do?belongId=${id}"
            ...
            <div property="columns">
                ...
                <div header="需要检查的项目" headerAlign="center">
                    <div property="columns">
                        <div field="partName" width="150px" headerAlign="center" align="center">部件名称
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="linkMaterialCode" width="250px" headerAlign="center" align="center">
                            关联物料编码（说明：此项为父项内容，关联部件物料号，如：机油滤芯对应的关联总成为发动机，关联物料编码为发动机）
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="detailContent" width="200px" headerAlign="center" align="center">详细内容
                            <input property="editor" class="mini-textbox"/></div>
                    </div>
                </div>
                <div field="topicNum" width="200px" headerAlign="center" align="center">对应保养操作的TOPIC号
                    （说明：详细项目对应保养操作对应的TOPIC，机油）
                    <input property="editor" class="mini-textbox"/></div>
                <div field="whMaterialCode" width="150px" headerAlign="center" align="center">维护物料编码
                    <input property="editor" class="mini-textbox"/></div>
                <div field="capacity" width="100px" headerAlign="center" align="center">数量/容量
                    <input property="editor" class="mini-textbox"/></div>
                <div header="服务周期" headerAlign="center">
                    <div property="columns">
                        <div field="cycle8" width="200px" headerAlign="center" align="center">8/每日
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle50" width="200px" headerAlign="center" align="center">50
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle100" width="200px" headerAlign="center" align="center">100
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle250" width="200px" headerAlign="center" align="center">250
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle500" width="200px" headerAlign="center" align="center">500
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle1000" width="200px" headerAlign="center" align="center">1000
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle2000" width="200px" headerAlign="center" align="center">2000
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle4000" width="200px" headerAlign="center" align="center">4000
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle4500" width="200px" headerAlign="center" align="center">4500
                            <input property="editor" class="mini-textbox"/></div>
                        <div field="cycle5000" width="200px" headerAlign="center" align="center">5000
                            <input property="editor" class="mini-textbox"/></div>
                    </div>
                </div>
...
<form id="excelForm" action="${ctxPath}/serviceEngineer/core/ByInventory/exportByInventoryList.do?belongId=${id}" method="post" target="excelIFrame">
        ...
        <form id="formImport" method="post">
            <table class="table-detail" cellspacing="1" cellpadding="0">
                <tr>
                    ...
                    <td style="width: 70%;">
                        <a href="#" style="color:blue;text-decoration:underline;" onclick="downImportTemplate()">保养件清单导入模板.xlsx</a>
                    </td>
                </tr>
                <tr>
...
<script type="text/javascript">
    mini.parse();
    var byInventoryForm = new mini.Form("#byInventoryForm");
    ...
    $(function () {
        if (id) {
            var url = jsUseCtxPath + "/serviceEngineer/core/ByInventory/getByInventoryDetail.do";
            $.post(
                url,
                {id: id},
                function (json) {
                    byInventoryForm.setData(json);
                });
        }else {
        ...
        if (action == 'task') {
            ...
            byInventoryForm.setEnabled(false);
            ...
            $("#exportByInventory").show();
            $("#detailToolBar").show();
            if (status != 'DRAFTED') {
                $("#processInfo").show();
            }
        } else if (action == "edit") {
            $("#importByInventory").show();
            $("#exportByInventory").show();
    ...
    function taskActionProcess() {
        ...
        mini.get("addInventoryDetail").setEnabled(false);
        mini.get("removeInventoryDetail").setEnabled(false);
        mini.get("importByInventory").setEnabled(false);
        mini.get("exportByInventory").setEnabled(false);
        ...
        if (stageName == 'first') {
            ...
            $("#importByInventory").show();
            $("#exportByInventory").show();
            byInventoryForm.setEnabled(true);
            inventoryListGrid.setAllowCellEdit(true);
            mini.get("addInventoryDetail").setEnabled(true);
            mini.get("removeInventoryDetail").setEnabled(true);
            mini.get("importByInventory").setEnabled(true);
            mini.get("exportByInventory").setEnabled(true);
    ...
    //流程引擎调用此方法进行表单数据的获取
    function getData() {
        var formData = _GetFormJsonMini("byInventoryForm");
    ...
    function OnCellBeginEditInventory(e) {
        ...
        if ((field == "linkMaterialCode" || field == "topicNum") && stageName == "second" ) {
            e.cancel = true;
        }
    }
    ...
    function importMaterial() {
        ...
        if (xhr.upload) {
            ...
            // 开始上传
            xhr.open('POST', jsUseCtxPath + '/serviceEngineer/core/ByInventory/importExcelByInventory.do?id='+id, false);
    ...
    function downImportTemplate() {
        ...
        form.attr("action", jsUseCtxPath + "/serviceEngineer/core/ByInventory/importTemplateDownloadByInventory.do");
        $("body").append(form);
        form.submit();
        form.remove();
    }
