...
<div class="mini-fit" style="height: 100%;">
    <div id="ysInventoryListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         sizeList="[50,100,200]" pageSize="50"
         url="${ctxPath}/serviceEngineer/core/YsInventory/queryYsInventory.do"
...
<form id="excelForm" action="${ctxPath}/serviceEngineer/core/YsInventory/exportYsInventoryList.do" method="post" target="excelIFrame">
...
<script type="text/javascript">
    ...
    var ysInventoryListGrid = mini.get("ysInventoryListGrid");
    ...
    function onMessageActionRenderer(e) {
        ...
        var s = '';
        if (status != 'DRAFTED'){
            s += '<span  title="明细" onclick="ysInventoryDetail(\'' + id + '\',\'' + status + '\')">明细</span>';
        }
        if (record.status == 'DRAFTED') {
            s += '<span  title="编辑" onclick="ysInventoryEdit(\'' + id + '\',\'' + instId + '\')">编辑</span>';
        }
        if(record.status =='RUNNING'){
            if(record.myTaskId) {
                s+='<span  title="办理" onclick="ysInventoryTask(\'' + record.myTaskId + '\')">办理</span>';
            }
        }
        if (status == 'DRAFTED' ) {
            s += '<span  title="删除" onclick="removeYsInventory(' + JSON.stringify(record).replace(/"/g, '&quot;') + ')">删除</span>';
        }
    ...
    function addYsInventory() {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/YSJQD/start.do";
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
                if (ysInventoryListGrid) {
                    ysInventoryListGrid.reload()
                }
            }
        }, 1000);
    }

    function copyYsInventory() {
        row = ysInventoryListGrid.getSelected();
        if (!row) {
            mini.alert("请选中一条记录");
            return;
        }
        var id = row.id;
        var url = jsUseCtxPath + "/bpm/core/bpmInst/YSJQD/start.do?oldId="+id;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function ysInventoryEdit(id,instId) {
    ...
    function ysInventoryDetail(id,status) {
        var action = "detail";
        var url = jsUseCtxPath + "/serviceEngineer/core/YsInventory/editPage.do?action=" + action + "&id=" + id+ "&status=" + status;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function ysInventoryTask(taskId) {
        $.ajax({
    ...
    function removeYsInventory(record) {
        var rows = [];
        if (record) {
            rows.push(record);
        } else {
            rows = ysInventoryListGrid.getSelecteds();
        }
        if (rows.length <= 0) {
        ...
        mini.confirm("确定删除选中记录？", "提示", function (action) {
            if (action != 'ok') {
                ...
                _SubmitJson({
                    url: jsUseCtxPath + "/serviceEngineer/core/YsInventory/deleteYsInventory.do",
                    method: 'POST',
                    showMsg:false,
...
<redxun:gridScript gridId="ysInventoryListGrid" entityName="" winHeight="" winWidth="" entityTitle="" baseUrl=""/>
</body>
</html>
