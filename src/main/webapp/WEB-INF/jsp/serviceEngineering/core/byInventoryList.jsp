...
<div class="mini-fit" style="height: 100%;">
    <div id="byInventoryListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         sizeList="[50,100,200]" pageSize="50"
         url="${ctxPath}/serviceEngineer/core/ByInventory/queryByInventory.do"
        ...
        <div property="columns">
            ...
            <div field="cpzgName" headerAlign='center' width="30" align='center'>产品主管</div>
            <div field="fwEditName"  width="35" headerAlign="center" align="center" allowSort="true">服务工程编制人员</div>
...
<form id="excelForm" action="${ctxPath}/serviceEngineer/core/ByInventory/exportByInventoryList.do" method="post" target="excelIFrame">
...
<script type="text/javascript">
    ...
    var byInventoryListGrid = mini.get("byInventoryListGrid");
    ...
    function onMessageActionRenderer(e) {
        ...
        var s = '';
        if (status != 'DRAFTED'){
            s += '<span  title="明细" onclick="byInventoryDetail(\'' + id + '\',\'' + status + '\')">明细</span>';
        }
        if (record.status == 'DRAFTED') {
            s += '<span  title="编辑" onclick="byInventoryEdit(\'' + id + '\',\'' + instId + '\')">编辑</span>';
        }
        if(record.status =='RUNNING'){
            if(record.myTaskId) {
                s+='<span  title="办理" onclick="byInventoryTask(\'' + record.myTaskId + '\')">办理</span>';
            }
        }
        if (status == 'DRAFTED' ) {
            s += '<span  title="删除" onclick="removeByInventory(' + JSON.stringify(record).replace(/"/g, '&quot;') + ')">删除</span>';
        }
    ...
    function addByInventory() {
        var url = jsUseCtxPath + "/bpm/core/bpmInst/BYJQD/start.do";
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
                if (byInventoryListGrid) {
                    byInventoryListGrid.reload()
                }
            }
        }, 1000);
    }

    function copyByInventory() {
        row = byInventoryListGrid.getSelected();
        if (!row) {
            mini.alert("请选中一条记录");
            return;
        }
        var id = row.id;
        var url = jsUseCtxPath + "/bpm/core/bpmInst/BYJQD/start.do?oldId="+id;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function byInventoryEdit(id,instId) {
    ...
    function byInventoryDetail(id,status) {
        var action = "detail";
        var url = jsUseCtxPath + "/serviceEngineer/core/ByInventory/editPage.do?action=" + action + "&id=" + id+ "&status=" + status;
        var winObj = window.open(url);
        var loop = setInterval(function () {
            if (winObj.closed) {
                clearInterval(loop);
    ...
    function byInventoryTask(taskId) {
        $.ajax({
    ...
    function removeByInventory(record) {
        var rows = [];
        if (record) {
            rows.push(record);
        } else {
            rows = byInventoryListGrid.getSelecteds();
        }
        if (rows.length <= 0) {
        ...
        mini.confirm("确定删除选中记录？", "提示", function (action) {
            if (action != 'ok') {
                ...
                _SubmitJson({
                    url: jsUseCtxPath + "/serviceEngineer/core/ByInventory/deleteByInventory.do",
                    method: 'POST',
                    showMsg:false,
...
<redxun:gridScript gridId="byInventoryListGrid" entityName="" winHeight="" winWidth="" entityTitle="" baseUrl=""/>
</body>
</html>
