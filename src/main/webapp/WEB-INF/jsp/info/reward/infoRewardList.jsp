...
<div class="mini-toolbar">
    <div class="searchBox">
        <form id="searchForm" class="search-form" style="margin-bottom: 25px">
            <ul>
                <li style="margin-right: 15px"><span class="text" style="width:auto">奖项分类: </span><input class="mini-textbox" style="width: 150px" name="type"/></li>
...
<div class="mini-fit" style="height: 100%;">
    <div id="rewardListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         url="${ctxPath}/info/reward/listData.do" idField="id" sortField="publishDate" sortOrder="desc"
        ...
        <div property="columns">
            ...
            <div field="infoChildType" width="60" headerAlign="center" align="center" allowSort="true" renderer="typeRenderer">奖项分类</div>
...
<script type="text/javascript">
    ...
    function viewForm(id,action) {
        if(action=='view'){
            var url= jsUseCtxPath +"/info/reward/detailViewPage.do?id="+id;
            window.open(url);
        }else{
            var url= jsUseCtxPath +"/info/reward/editPage.do?id="+id+"&action="+action;
            var title = "修改";
            mini.open({
                title: title,
                url: url,
    ...
    function spiderData() {
        $.ajax({
            url:jsUseCtxPath+"/info/reward/doSpider.do?infoTypeName=奖项申报",
    ...
    function addRow() {
        let url = jsUseCtxPath + "/info/reward/editPage.do?action=add";
        mini.open({
            title: "新增",
            url: url,
            width: 1000,
    ...
    function removeRow() {
        var rows = [];
        rows = rewardListGrid.getSelecteds();
        if (rows.length <= 0) {
        ...
        mini.confirm("确定取消选中记录？", "提示", function (action) {
            if (action != 'ok') {
                ...
                if (ids.length > 0) {
                    _SubmitJson({
                        url: jsUseCtxPath + "/info/reward/remove.do",
                        method: 'POST',
                        data: {ids: ids.join(',')},
                        success: function (text) {
                            if (rewardListGrid) {
                                rewardListGrid.reload();
                            }
                        }
                    });
                }
