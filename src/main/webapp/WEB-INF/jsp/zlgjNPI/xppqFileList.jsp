...
<script type="text/javascript">
	...
	var url=jsUseCtxPath+"/zlgjNPI/core/xppq/files.do?wtId="+wtId +"&zxId="+ zxId+"&fileType="+fileType;
	$(function () {
        queryFiles();
    ...
    function uploadZlgjPic() {
        mini.open({
            title: "附件上传",
            url: jsUseCtxPath + "/zlgjNPI/core/xppq/openUploadWindow.do",
            width: 650,
            height: 350,
            showModal:false,
            allowResize: true,
    ...
    function returnZlgjPreviewSpan(fileName,fileId,wtId,coverContent) {
        ...
        if(fileType=='other'){
            ...
            var url='/zlgjNPI/core/xppq/preview.do?fileType='+fileType;
	...
	function deleteFile(record) {
		mini.confirm("确定删除？", "确定？",
				function (action) {
					if (action == "ok") {
						var url = jsUseCtxPath + "/zlgjNPI/core/xppq/deleteFiles.do";
						var data = {
                            wtId: record.wtId,
							id: record.id,
	...
	function downFile(record) {
		...
		form.attr("action", jsUseCtxPath + "/zlgjNPI/core/xppq/fileDownload.do?action=download");
