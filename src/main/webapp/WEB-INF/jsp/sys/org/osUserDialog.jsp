...
<body>
	 	...
	 	>
			 <div class="treeToolBar">
				...
				<input
					...
					url="${ctxPath}/sys/org/osDimension/jsonAll.do?containContact=yes&tenantId=<c:out value="${param.tenantId}" />&dimId=1"
					textField="name"
					valueField="dimId"
					...
					<div class="searchBox">
						<form id="searchForm" class="search-form" >						
							<ul>
								...
								<li class="liAuto">
									<input class="mini-textbox"  id="userNo" name="userNo" emptyText="请输入账号" onenter="onSearch"/>
				...
				<div class="mini-fit ">
					<div 
						...
						<c:choose>
						 ...
						 showColumnsMenu="true" sizeList="[20,50,100,200,500]" pageSize="20" allowAlternating="true">
						<div property="columns"  class="form-outer">
							...
							<div field="userNo" width="120"   headerAlign="center" allowSort="false">账号</div>
							<div field="deptName" width="120" sortField="EMAIL_" headerAlign="center" allowSort="true">部门</div>
							<div field="email" width="120" sortField="EMAIL_" headerAlign="center" allowSort="true" visible="false">邮件</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		*/
        ...
        function moveUp() {
            var row = selectedUserGrid.getSelected();
            if (row) {
                var index = selectedUserGrid.indexOf(row);
                selectedUserGrid.moveRow(row, index - 1);
            }
        }
        function moveDown() {
            var row = selectedUserGrid.getSelected();
            if (row) {
                var index = selectedUserGrid.indexOf(row);
                selectedUserGrid.moveRow(row, index + 2);
            }
        }
	</script>
</body>
