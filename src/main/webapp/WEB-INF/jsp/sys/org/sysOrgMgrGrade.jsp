				...
				<div title="用户关系" iconCls="icon-user">
					<div class="form-toolBox">
						...
						<form id="userForm" class="text-distance">
							...
							<input class="mini-textbox" id="fullname" name="fullname" emptyText="请输入姓名" style="margin-right: 6px"/>
							<input class="mini-textbox"  id="userNo" name="userNo" emptyText="请输入用户编号"/>
						</form>
					</div>
					<div class="mini-fit rx-grid-fit" style="padding:2px;">
						<div id="userGridTab" onactivechanged="loadUserGridTabData"
							 class="mini-tabs" activeIndex="0" style="width:100%;height:100%;border:none;" bodyStyle="padding:2px;">
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function onActionRenderer(e) {
		...
		s+='<span class="icon-button icon-jia" title="添加子用户组" onclick="newGroupSubRow()"></span>';
		...
		if("1"=="${userId}" || record.createBy && record.createBy=="${userId}"){
			...
			s+='<span class="icon-button icon-mgr" title="分级配置" onclick="grantGradeConfig(\'' + uid + '\')"></span>';
		}
	...
	function addSpan(record,uid){
		var s = '';
		s+='<span class="icon-button icon-shang-add" title="在前添加用户组" onclick="newBeforeRow(\''+uid+'\')"></span>';
		s+='<span class="icon-button icon-xia-add" title="在后添加用户组" onclick="newAfterRow(\''+uid+'\')"></span>';
		return s;
	}
	//配置管理员
	...
	function onUserActionRenderer(e){
		...
		s+=' <span class="icon-button icon-brush" title="编辑" onclick="userEdit(\''+uid+'\')"></span>';
		s+=' <span class="icon-button icon-trash" title="移除" onclick="userUnjoin(\''+uid+'\')"></span>';
		return s;
	}
