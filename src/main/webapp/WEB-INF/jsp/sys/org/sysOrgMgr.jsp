...
<body>
	...
	<ul id="dimNodeMenu" class="mini-contextmenu" onbeforeopen="onShowingNodeMenu">
	    <li onclick="delDim">删除维度</li>
	    <li onclick="editDim">编辑维度</li>
	    <li onclick="manageDimRank()">管理维度等级</li>
	</ul>
		    ...
		    <div region="center" showHeader="false" showCollapseButton="false" iconCls="icon-group" >
              	...
              	<div class="mini-toolbar" >
			         <ul class="toolBtnBox" >
						...
						<li>
							<a class="mini-button"  onclick="copyGroupGrant();">复制部门权限</a>
						</li>
						<li>
					    	...
					    	<div title="用户关系" iconCls="icon-user">
					    		<div class="form-toolBox">
					       		...
					       		<div class="mini-fit rx-grid-fit">
					        		<div id="userGridTab" class="mini-tabs" activeIndex="0" style="width:410px;height:100%;border:none;">
									</div>
					       		</div>
				    	</div>
	...
	<%--@mh这里加一个window--%>
	<div id="copyGrantWindow" title="复制权限" class="mini-window" style="width:660px;height:380px;"
		 showModal="true" showFooter="true" allowResize="true" showCloseButton="false">
		<div class="mini-fit">
			<div class="topToolBar" style="float: right;">
				<div style="position: relative!important;">
					<span style="float: left;font-size: 16px;color:red">复制到目标部门的权限将会清空后复制，请谨慎操作&#10071&#10071&#10071</span>
					<a id="copyGrant" class="mini-button" onclick="copyGrant()">保存</a>
					<a id="closeGrant" class="mini-button btn-red" onclick="closeGrant()">关闭</a>
				</div>
			</div>
			<table class="table-detail" cellspacing="1" cellpadding="0" style="width: 99%">
				<tr>
					<td style="width: 15%">原权限部门：<span style="color:red">*</span></td>
					<td style="width: 85%;">
						<input id="srcDept" name="srcDepartId" class="mini-dep rxc" plugins="mini-dep"
							   style="width:98%;height:34px"
							   allowinput="false" textname="departName" single="true" initlogindep="false"/>
					</td>
				</tr>
				<tr>
					<td style="width: 15%">复制到部门：<span style="color:red">*</span></td>
					<td style="width: 85%;">
						<input id="targetDept" name="targetDepartId" class="mini-dep rxc" plugins="mini-dep"
							   style="width:98%;height:34px"
							   allowinput="false" textname="targetDepartName" single="true" initlogindep="false"/>
					</td>
				</tr>
				<%--<tr>--%>
				<%--<td style="text-align: center;width: 17%">网络类型：</td>--%>
				<%--<td style="width: 33%">--%>
					<%--<input id="netType" name="netType" class="mini-checkboxlist" multiSelect="true" style="width:98%"--%>
	...
	<script type="text/javascript">
        ...
        var copyGrantWindow = mini.get("copyGrantWindow");
        ...
        function onActionRenderer(e) {
            ...
            if(record.groupId && record.groupId!='' && isGrantButton){
            	s+='<span class="icon-button icon-quanxian2" title="子系统技术网授权" onclick="grantGroupRow(\'' + uid + '\')"></span>';
            }
            if(record.groupId && record.groupId!='' && isGrantButton){
                s+='<span class="icon-button icon-quanxian2" title="子系统管理网授权" onclick="grantGroupRow2(\'' + uid + '\')"></span>';
            }
        ...
        //授权用户组给管理网
        function grantGroupRow2(uid){
            var row=groupGrid.getRowByUID(uid);
            _OpenWindow({
                title:'['+row.name + ']'+'--管理网授权管理',
                url:__rootPath+'/sys/org/osGroup/toGrant2.do?groupId='+row.groupId + '&tenantId='+tenantId + "&instType="+ instType,
                width:450,
                height:600
            });
        }
        ...
        function copyGroupGrant() {
            ...
            copyGrantWindow.show();
        }

        function copyGrant() {
            var srcDept = mini.get("srcDept").getText();
            var targetDept = mini.get("targetDept").getText();
            if (!srcDept) {
                mini.alert("请选择被复制权限的部门");
                return;
            }
            if (!targetDept) {
                mini.alert("请选择目标部门");
            ...
            var srcDeptId = mini.get("srcDept").getValue();
            var targetDeptId = mini.get("targetDept").getValue();
            if (srcDeptId == targetDeptId) {
                mini.alert("请选择不同的部门进行复制");
                return;
            }
            mini.confirm("确定将"+srcDept+"的系统及菜单权限复制到"+targetDept+"?", "提示", function (action) {
                if (action != 'ok') {
                    return;
                } else {
					_SubmitJson({
						url: __rootPath + "/sys/org/sysOrg/copyGroupGrant.do",
						method: 'POST',
						showMsg: false,
						data: {srcDeptId: srcDeptId, targetDeptId: targetDeptId},
        ...
        function closeGrant() {
            mini.get("srcDept").setValue("");
            mini.get("srcDept").setText("");
            mini.get("targetDept").setValue("");
            mini.get("targetDept").setText("");
            copyGrantWindow.hide();
        }
