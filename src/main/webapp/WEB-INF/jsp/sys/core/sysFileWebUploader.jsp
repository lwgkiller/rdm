...
function setFiles(fileArr){
	...
	for(var i=0;i<fileArr.length;i++){
        ...
        obj.size = item.totalBytes;
        obj.lastModifiedDate = new Date();
        obj.id = item.fileId;
        obj.ext = item.fileName.substr(item.fileName.indexOf(".")+1);
...
function delFile(file){
    ...
    $.ajax({
        url: "${ctxPath}/sys/core/sysFile/delImageFile.do?fileId="+file.id,
...
$(function(){
    ...
    uploader.on( 'uploadSuccess', function( file,response ) {
		...
		var str = mini.encode(tmpfiles);
		var len = str.length;
        $('#delete_'+ file.id).attr("fileId",uploadFile.fileId);
        if(len>configJson.length){
			mini.showTips({
	            content: "<b>提醒</b> <br/>上传文件数量超出字段长度限制",
	            state:'danger',
	            x: 'center',
	            y: 'center',
	            timeout: 3000
