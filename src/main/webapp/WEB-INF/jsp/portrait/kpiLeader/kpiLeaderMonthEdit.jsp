...
<script type="text/javascript">
    ...
    function OnCellBeginEdit(e){
        e.cancel = true;
        if (stageName == 'ziping'){
            if (e.field == "completeStatus" || e.field == "description"){
                e.cancel = false;
            }
        }
        if (stageName == 'zhikao' || stageName == 'queren'){
            if (e.field == "score" || e.field == "scoreDesc"){
                e.cancel = false;
            }
        }
        if (stageName == 'shenhe'){
    ...
    function initProjectForm() {
        ...
        if (id) {
            var url = jsUseCtxPath + "/kpiLeader/core/getJson.do?id="+id;
    ...
    function checkEditRequired(formData) {
        ...
        if(formData && formData.length!=0) {
            for(var index=0;index<formData.length;index++) {
                var one = formData[index];
                if (!$.trim(one.completeStatus)) {
                    mini.alert('完成情况不能为空！');
                    return false;
                }
                if (!$.trim(one.description)) {
                    mini.alert('情况说明不能为空！');
                    return false;
                }
            }
    ...
    //完成情况自评下一步
    function projectZkApprove() {
        var formData = matListGrid.getData();
        var checkResult = checkEditZKRequired(formData);
        if (!checkResult) {
            return;
        }
        //保存
    ...
    function checkEditZKRequired(formData) {
        if (!formData) {
            mini.alert('请填写必填项！');
            return false;
        }
        if(formData && formData.length!=0) {
            for(var index=0;index<formData.length;index++) {
                ...
                if (!$.trim(one.scoreDesc)) {
                    mini.alert('扣分依据不能为空！');
                    return false;
                }
                var flag = isNumber($.trim(one.score));
                if (!flag) {
                    mini.alert('指标得分应填写数字！');
                    return false;
                }
                // if ((one.score < one.targetUpperValue) || (one.score > one.targetLowerValue)){
                //     debugger
                //     mini.alert('指标得分应填写在目标上下值区间内！');
                //     return false;
                // }
            }
        }
    ...
    //流程群发起
    function saveFjScore(){
        var fjScore = mini.get("fjScore").getValue();
        if (!fjScore){
            mini.alert('请填写附加分！');
            return ;
        }
        $.ajax({
            // var url = jsUseCtxPath + "/kpiLeader/core/editMonthPage.do?action=add";
            url: jsUseCtxPath + "/kpiLeader/core/saveFjScore.do?id=" + id +"&fjScore=" + fjScore,
            type: 'get',
            async: false,
            contentType: 'application/json',
