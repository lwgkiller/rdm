...
<html>
<head>
    <title>物料扩充申请管理</title>
    <%@include file="/commons/list.jsp" %>
    <script src="${ctxPath}/scripts/materielExtend/materielExtendApplyList.js?version=${static_res_version}"
        ...
        <form id="searchForm" class="search-form" style="margin-bottom: 25px">
            <ul>
                ...
                <li style="margin-right: 15px">
                    ...
                    <input name="urgent" class="mini-combobox" style="width:80px;"
                ...
                <li style="margin-left: 10px">
                    ...
                    <a class="mini-button" plain="true" style="margin-right: 5px" id="exportApplyMat" onclick="exportApplyMateriels()">导出申请单物料</a>
                    <a class="mini-button" plain="true" style="margin-right: 5px" id="importApplyMat" onclick="batchImportOpen">导入申请单物料</a>
                </li>
                <li>
                    <p style="display: inline-block;color: red;font-size:15px;vertical-align: middle">
                        （申请单状态：1、“待更新至SAP”:SAP锁定或无法调通，请稍后点击“查看”-->点击“更新至SAP”重试；2、问题原因仅有“SAP校验失败：”:SAP调用失败，需要联系管理员）
                    </p>
                </li>
            </ul>
            <div id="moreBox">
                <ul>
                    <li style="margin-right: 15px"><span class="text" style="width:auto">状态: </span>
                        <input id="applyStatus" name="applyStatus" class="mini-combobox" style="width:150px;"
                               textField="key" valueField="value" emptyText="请选择..."
                               required="false" allowInput="false" showNullItem="true" nullItemText="请选择..."
                               data="[ {'key' : '草稿','value' : 'draft'},{'key' : '处理中','value' : 'running'},{'key' : '流程结束（无错误物料）','value' : 'successEnd'},{'key' : '流程结束（有错误物料）','value' : 'failEnd'},{'key' : '流程结束（错误已处理）','value' : 'successConfirm'},{'key' : '待更新至SAP','value' : 'needSync2Sap'}]"
                        />
                    </li>
                    <li style="margin-right: 15px">
...
<div class="mini-fit" style="height: 100%;">
    <div id="applyListGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="false"
         url="${ctxPath}/materielExtend/apply/getApplyList.do"
...
<div id="processStatusWindow" title="处理进度窗口" class="mini-window" style="width:900px;height:350px;"
    ...
    <div id="importWindow" title="物料导入窗口" class="mini-window" style="width:750px;height:200px;"
         showModal="true" showFooter="false" allowResize="true">
        <div style="display: inline-block;float: right;">
            <a id="importBtn" class="mini-button"  onclick="importApplyMateriel()">批量导入</a>
...
<script type="text/javascript">
    ...
    function onActionRenderer(e) {
        ...
        if(applyStatus=='draft'||currentUserNo=='admin') {
            s+='<span title="编辑" onclick="editOrViewApply(\'' + record.applyNo +'\',\'edit\')">编辑</span>';
            s+='<span  title="删除" onclick="removeApply('+JSON.stringify(record).replace(/"/g, '&quot;')+')">删除</span>';
        }
        if((applyStatus=='running' && opRoleName != 'SQRKC')||currentUserNo=='admin'){
            s+='<span title="办理" onclick="editOrViewApply(\'' + record.applyNo +'\',\'edit\')">办理</span>';
        }
        if((applyStatus=='failEnd' && opRoleName == 'SQRKC')||currentUserNo=='admin') {
            s+='<span title="错误处理确认" onclick="failConfirm(\'' + record.applyNo +'\')">错误确认</span>';
        }
