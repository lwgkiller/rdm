			...
			function detailRowFrom(){
				...
				if(row){
					...
					if(rows && rows.length>1){
						mini.alert("您已选择多条记录，请只选中一条记录进行查看明细！");
						return;
					}
				}else{
					mini.alert("请选中一条记录");
        		...
        		switch(json.key){
        			...
        			case "start":
        				if(json.val && ( record.status=='TEST'||record.status=='DEPLOYED')){
			            	...
			            	str= str + ' <span title="发布我的流程任务菜单" onclick="deployMenu2(\'' + record._uid + '\')">发布我的流程任务菜单</span>';
                            str= str + ' <span title="发布流程任务菜单" onclick="deployMenu3(\'' + record._uid + '\')">发布流程任务菜单</span>';
			            }
        				break;
        			case "edit":
        				break;
            ...
            function deployMenu2(uid){
                ...
                _OpenWindow({
                    ...
                    onload:function(){
                        ...
                        win.setData({
                            name:row.name+'的待办',
                            key:row.key+'_Sol_Tasks',
                            parentId:'',
                            url:'/bpm/core/bpmTask/mySol/'+row.key+'.do',
                            showType:'URL',
                            isBtnMenu:'NO',
                            isMgr:'NO'
                        });
                ...
                _OpenWindow({
                    ...
                    onload:function(){
                        ...
                        win.setData({
                            ...
                            key:row.key+'_Sol_Tasks',
                            parentId:'',
                            url:'/bpm/core/bpmTask/sol/'+row.key+'.do',
                            showType:'URL',
                            isBtnMenu:'NO',
                            isMgr:'NO'
                        });
