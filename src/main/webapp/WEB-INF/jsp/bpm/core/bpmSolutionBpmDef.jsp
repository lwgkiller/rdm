...
<body>
	<div class="form-toolBox" >
	    ...
	    <c:if test="${not empty bpmDef}">
			...
			<c:if test="${bpmDef.version!=oldBpmDef.version}">
	...
	<script type="text/javascript">
		...
		function userSetting(){
			_OpenWindow({
				...
				width : 600,
				height : 500,
				url : __rootPath + '/bpm/core/bpmSolution/nodeUser.do?nodeId=' + nodeId + '&nodeType=' + nodeType + '&solId=' + solId +'&actDefId=${bpmDef.actDefId}'+"&nodeName="+nodeName
			});
		}
				...
				var config={
				    url: url,
				   	...
				   	success:function(result){
				   		...
				   		if(result.success){
					   		designRow(data.modelId,data.defId,data.actDefId);
				   		}else{
				   			top._ShowTips({
			            		msg:result.message
			            	});
		...
		function designRow(modelId,defId,actDefId){
    		_OpenWindow({
    			width:800,
    			height:600,
    			max:true,
