				...
				<#if(destNodeUser.taskNodeUser){#>
					<#if(destNodeUser.taskNodeUser.nodeType=='userTask'){#>
							<#if(!allowNextExecutor){#>
								<input class="mini-combobox checkusers" name="users" data-options="{seqId:'<#=i#>',nodeId:'<#=destNodeUser.nodeId#>'}" id="users<#=i#>"  value="<#=destNodeUser.taskNodeUser.userIds#>"
									   textField="text" valueField="id" data='<#=destNodeUser.taskNodeUser.userIdsAndText#>' multiselect="true" valuechanged="changeDestUser('users<#=i#>')" style="width:90%"
										readonly="true"/>
							<#}#>

							<#if(allowNextExecutor){#>
								<%--<a class="mini-button" iconCls="icon-users" plain="true"  onclick="selectUsers('<#=i#>')" alt="审批者">...</a>--%>
								<input class="mini-combobox checkusers" name="users" data-options="{seqId:'<#=i#>',nodeId:'<#=destNodeUser.nodeId#>'}" id="users<#=i#>"  value="" style="width:90%"
									   textField="text" valueField="id" data='<#=destNodeUser.taskNodeUser.userIdsAndText#>' multiselect="true" valuechanged="changeDestUser('users<#=i#>')"/>
							<#}#>
					<#}#>
				<#}else{#>
								...
								<#if(destMap.nodeType=='userTask'){#>
									<tr>
										...
										<td >
											...
											<#}#>
											<#if(!allowNextExecutor){#>
											<input class="mini-combobox checkusers"  name="users"
												   data-options="{seqId:'<#=i#>_<#=key#>',nodeId:'<#=destMap.nodeId#>'}" textField="text" valueField="id"
												   id="users<#=i#>_<#=key#>"  value="<#=destMap.userIds#>" data="<#=destMap.userJsons#>" textField="text"
												   multiselect="true" valuechanged="changeDestUser('users<#=i#>_<#=key#>')" style="width:90%"
												   readonly="true"/>
											<#}#>
											<#if(allowNextExecutor){#>
											<input class="mini-combobox checkusers"  name="users"
												   data-options="{seqId:'<#=i#>_<#=key#>',nodeId:'<#=destMap.nodeId#>'}" textField="text" valueField="id"
												   id="users<#=i#>_<#=key#>"  value="" data="<#=destMap.userJsons#>" textField="text" style="width:90%"
												   multiselect="true" valuechanged="changeDestUser('users<#=i#>_<#=key#>')" />
											<#}#>
											<#if(destMap.groupIds && destMap.groupIds!=''){#>
												<input class="mini-combobox checkgroups" name="groups" id="groups<#=i#>_<#=key#>"
...
<script type="text/javascript">
                ...
                success: function (data) {
	        		...
	        		for(var i=0;i<list.length;i++){
	        			if(list[i].taskNodeUser){
	        			    //@老李：此处加了一个if判断，因为发现后续节点为2个可选节点的情况下，如果某个节点没有待办人员列表
                            //list[i].taskNodeUser会找不到userIdsAndText属性，前端加载报错
	        			    if(list[i].taskNodeUser.userIdsAndText!=null){
    ...
    function approve(){
        ...
        var flag = handUsers(formData);
        if(!flag){
        	mini.showTips({content: "下一级节点没有选择审批人！",state:"danger"});
        	return;
        }
