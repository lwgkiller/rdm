...
<body>
					 ...
					 <div class="searchBox">
		                <form id="searchForm" class="search-form">
		                    <ul>
								...
								<li>
									<span class="text">任务ID：</span>
									<input class="mini-textbox" name="Q_ID__S_EQ" onenter="onSearch"/>
								</li>
								<li class="liBtn">
	...
	<script type="text/javascript">
		...
		function onSearch(){
			...
			grid.setUrl("${ctxPath}/bpm/core/bpmTask/getAllTasks.do?"+data);
			grid.load();
		}
       	//行功能按钮
       	...
       	function getByJson(record,json){
            ...
            if (!belongSubSysKey) {
       		...
       		switch(json.key){
       			...
       			case "handle":
       				if(json.val){
       					str='<span class="" title="办理" onclick="checkAndHandTask(\'' + uid + '\',true,false,\'\',\''+belongSubSysKey+'\')">办理</span>';
       				}
       				break;
       		}
       		return str;
        ...
        grid.on("drawcell", function (e) {
            ...
            if(field=='description'){
            	if(value){
            		var str= '<a href="javascript:checkAndHandTask(\'' + record._uid + '\',true,false,\'\',\''+belongSubSysKey+'\')">';
            		if(record.taskType=='CMM'){
            			str+='<img src="'+__rootPath+'/styles/icons/commute.png"> ';
            		}
            		e.cellHtml=str+record.description+'</a>';
            	}else{
            		e.cellHtml='<a href="javascript:checkAndHandTask(\'' + record._uid + '\',true,false,\'\',\''+belongSubSysKey+'\')">'+record.name+'</a>';
            	}
            }
            if(field=='dueDate'){
            	if(value){
