...
<body>
	<div  class="topToolBar noprint">
		<div class="noPrint">
			<c:if test="${ not empty param['solId'] || not empty param['instId'] || not empty param['actInstId']  || not empty param['taskId'] || not empty params['actDefId']}">
				<div id="showImage" name="showImage" class="mini-checkbox" readOnly="false" text="显示流程图" onvaluechanged="showImage"></div>
			</c:if>
			<c:if test="${empty param['viewId'] && (not empty param['instId'] || not empty param['actInstId']  || not empty param['taskId'])}">
	...
	<div class="form-container" id="formPrint">
		...
		<c:if test="${not empty bpmInst }">
			...
			<table class="table-detail column-four table-align"  style="width:100%;" cellpadding="0" cellspacing="1">
				<tr>
					<td >
						流程发起人
					</td>
		...
		<c:if test="${ not empty param['actDefId'] || not empty param['instId'] || not empty param['actInstId']  || not empty param['taskId']}">
			<div class="form-container"  style="padding: 0;margin-top: 10px;">
				<div id="showImageDiv" style="display:none;padding: 10px">
					<div class="titles">流程图</div>
						...
						<img src="${ctxPath}/bpm/activiti/processImage.do?actDefId=${param['actDefId']}&instId=${param['instId']}&actInstId=${param['actInstId']}&taskId=${param['taskId']}"  style="max-width: 100%;"/>
					</div>
				</div>
			</div>
		</c:if>
