...
<%
		...
		%>
		<% if(alias.equals("discard")||alias.equals("reject")||conf.getName().equals("项目变更")||conf.getName().equals("驳回")) {%>
		<a id="<%=conf.getId()%>" class="mini-button btn-red"  onclick="<%=conf.getScript() %>"><%=conf.getName() %></a>
		<%} else if(alias.equals("doTask")) {%>
		<a id="<%=conf.getId()%>" class="mini-button btn-green"  onclick="<%=conf.getScript() %>"><%=conf.getName() %></a>
		<%} else {%>
		<a id="<%=conf.getId()%>" class="mini-button"  onclick="<%=conf.getScript() %>"><%=conf.getName() %></a>
		<%
		}
