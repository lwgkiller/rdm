 ...
 * */
@Service
public class GkgfApplyHandler implements ProcessStartPreHandler, TaskPreHandler, ProcessStartAfterHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(GkgfApplyHandler.class);
    @Resource
    ...
    private JSONObject addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (StringUtils.isBlank(formDataJson.getString("id"))) {
            gkgfApplyManager.add(formDataJson);
        } else {
            gkgfApplyManager.update(formDataJson);
        }
        return formDataJson;
    }
    // 流程成功结束之后的处理
