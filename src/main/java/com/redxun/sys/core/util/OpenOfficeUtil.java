...
import com.redxun.sys.core.manager.ConverterDocument;
...
public class OpenOfficeUtil {
     ...
     *
     * @param inputFilePath
     *            D:\\outFile.doc 注意文件在不同系统内的区别,请使用File.separator作为分隔符
    ...
    public static JSONObject coverFromOffice2Pdf(String inputFilePath, String outputFilePath) {
        ...
        try {
            ...
            logger.error("Exception in coverFromOffice2Pdf", e);
...
//            DocumentConverter converter = new OpenOfficeDocumentConverter(connection);
    ...
    public static JsonResult<String> getConnectStatus() {
        ...
        try {
            ...
            logger.error("Exception in getConnectStatus", e);
    ...
    public static JsonResult<String> startService() {
        String openOfficeConfig = SysPropertiesUtil.getGlobalProperty(OPEN_OFFICE_CONFIG);
        if (StringUtil.isEmpty(openOfficeConfig)) {
    ...
    public static JsonResult<String> startService(String installPath, String ip, String port) {
        ...
        try {
            ...
            if (sys.toLowerCase().startsWith("win")) {
                command = installPath + "program\\soffice.exe -headless -accept=\"socket,host=" + ip + ",port=" + port
                    + ";urp;StarOffice.ServiceManager\" -nofirststartwizard";
            } else {
                command = installPath + "program/soffice -headless -accept=\"socket,host=" + ip + ",port=" + port
                    + ";urp;\" -nofirststartwizard &";
            }
            Runtime.getRuntime().exec(command);

        } catch (Exception e) {
            logger.error("Exception in startService", e);
    ...
    public static JsonResult<String> endService() {
        ...
        try {
            ...
            logger.error("Exception in endService", e);
            result.setSuccess(false);
    ...
    public static String generateConvertPdfFileName(String fileName, String suffix) {
        if (StringUtils.isBlank(fileName)) {
            logger.error("fileName is blank");
            return "tmp.pdf";
        }
        if (StringUtils.isBlank(suffix)) {
            logger.error("suffix is blank");
            return fileName + ".pdf";
        }
        return fileName + "_" + suffix + ".pdf";
    }
    public static String generateConvertPdfFileName(String originalFileName) {
        if (StringUtils.isBlank(originalFileName)) {
            logger.error("fileName is blank");
            return "tmp.pdf";
        }
        String fileNameWithOutExt = findFileNameWithoutExt(originalFileName);
        String fileExt = findFileExt(originalFileName);
