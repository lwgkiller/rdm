...
public class SysDicDao extends BaseJpaDao<SysDic> {
    ...
    public List<SysDic> getByTreeKey(String key){
    	String ql="from SysDic sd where sd.sysTree.key=? order by sd.sn asc";
     ...
     * @return
     */
    public List<SysDic> getByParentId(String dicParentId) {
        String ql = "from SysDic sd where sd.parentId=? order by sd.sn asc";
        return this.getByJpql(ql, new Object[]{dicParentId});
    }

    /**
     * 按分类的key获取数据字典的首层（含下级节点的字典）
     *
     * @param key 分类key
     * @return
     */
    public List<SysDic> getByTreeKeyTopOne(String key) {
        String ql = "from SysDic sd where sd.sysTree.key=? and sd.parentId=0";
        return this.getByJpql(ql, new Object[]{key});
    }
}
