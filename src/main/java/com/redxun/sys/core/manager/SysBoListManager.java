...
public class SysBoListManager extends MybatisBaseManager<SysBoList>{
	...
	public Map<String,Object> getGenParams(SysBoList sysBoList,boolean needSearch) throws  Exception{
		...
		if(treeConfigs!=null){
			...
			params.put("treeConfigs", new ArrayList<TreeConfig>());
		}
	...
	private String getPermissionSql(JSONArray arr,String alias){
		...
		for(int i=0;i<arr.size();i++){
			...
			if("user".equals(type)){//按创建用户查询
				if("SELF".equals(scope)){
					if(conSb.length()>0){
						conSb.append(" or ");
					}
					conSb.append(alias)
					.append( "."+field+"='").append(curUser.getUserId()).append("'");
					continue;
				}
				...
				if("DUP_USERS".equals(scope)){
					...
					conSb.append(alias)
					.append("."+field+" in ").append("(select PARTY1_ from OS_REL_INST oi where oi.PATH_='"+upLowPath+"' and oi.REL_TYPE_ID_='3')");
				}
				//所有上级用户
					...
					conSb.append(alias)
					.append("."+field+" in ").append("(").append(StringUtil.getArrCharString(parentPath)).append(")");
				}
				//直属下级用户
					...
					conSb.append(alias)
					.append("."+field+" in ").append("(select PARTY2_ from OS_REL_INST oi where oi.PARTY1_='"+curUser.getUserId()+"' and oi.REL_TYPE_ID_='3')");
				}else if("DOWN_USERS".equals(scope)){
					if(conSb.length()>0){
						conSb.append(" or ");
					}
					conSb.append(alias)
					.append("."+ field +" in (select PARTY2_ from OS_REL_INST oi where oi.PATH_ like '"+curUser.getUserUpLowPath()+"%')");
				}
			}else if("group" .equals(type)){//按用户组查找
				...
				if("SELF_DEP".equals(scope)){
					...
					conSb.append(alias)
					.append("."+ field +"='").append(curDepId).append("'");
					continue;
				}
				//直属上级部门
					...
					conSb.append(alias).append("."+field+"='").append(parentGpId).append("'");
					...
					conSb.append(alias).append("."+field+" in ("+idArr+")");
				}
				//直接下级
					...
					conSb.append(alias).append("."+field+" in (select group_id_ from os_group p where p.parent_id_='"+mainGroup.getGroupId()+"')");
				}else if("DOWN_DEPS".equals(scope)){
					String groupPath=StringUtils.isEmpty(mainGroup.getPath())?"-1":mainGroup.getPath();//若当前的目录路径为空,则让他找不到数据
					if(conSb.length()>0){
						conSb.append(" or ");
					}
					conSb.append(alias).append("."+ field +" in (select group_id_ from os_group p where p.path_ like '"+groupPath+"%')");
				}
			}else if ("inst" .equals(type)){//按机构查询
				...
				if("SELF_TENANT".equals(scope)){
					...
					conSb.append(alias)
					.append("."+field+"='").append(curTenantId).append("'");
					continue;
					...
					conSb.append(alias).append("."+field+"='").append(pInstId).append("'");
					...
					conSb.append(alias).append("."+field+" in (select inst_id_ from sys_inst p where p.parent_id_='"+curTenantId+"')");
				}else if("DOWN_TENANTS".equals(scope)){
					String pPath=StringUtils.isEmpty(itenant.getPath())?"-1":itenant.getPath();//若当前的目录路径为空,则让他找不到数据
					if(conSb.length()>0){
						conSb.append(" or ");
					}
					conSb.append(alias).append("."+field+" in (select inst_id_ from sys_inst p where p.path_ like '"+pPath+"%')");
				}
			}
		}
