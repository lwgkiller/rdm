...
public class SubsystemManager extends MybatisBaseManager<Subsystem>{
	...
	public List<Subsystem> getGrantSubsByGroupId(String groupId,String netType){
		return subsystemDao.getGrantSubsByGroupId(groupId,netType);
	}
	 ...
	 * @return
	 */
	public List<Subsystem> getGrantSubsByGroupIds(Collection<String> groupIds,boolean netType){
		if(groupIds==null || groupIds.size()==0){
			return new ArrayList<Subsystem>();
		}
		return subsystemDao.getGrantSubsByGroupIds(groupIds,netType);
	}
    ...
    public List<Subsystem> queryAllSubSystem(){
        return subsystemDao.queryAllSubSystem();
    }
