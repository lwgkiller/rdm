...
//    	}
    ...
    //@lwgkiller:根据树形字典某个树跟的SysDic的id获取其下面的List<SysDic>
    public List<SysDic> getByParentId(String parentId) {
        return sysDicDao.getByParentId(parentId);
    }

    //@lwgkiller:获取树形字典的第一层树根的List<SysDic>
    public List<SysDic> getByTreeKeyTopOne(String key) {
        return sysDicDao.getByTreeKeyTopOne(key);
    }

    //@lwgkiller:根据字典类别key获取这个类别下的字典的key，value对List<JSONObject>
    public List<JSONObject> getSysDicKeyValueListByTreeKey(String TreeKey) {
        List<SysDic> sysDicList = this.getByTreeKey(TreeKey);
        List<JSONObject> sysDicKeyValueList = new ArrayList<>();
        for (SysDic sysDic : sysDicList) {
            JSONObject sysDicKeyValue = new JSONObject();
            sysDicKeyValue.put("key", sysDic.getKey());
            sysDicKeyValue.put("value", sysDic.getValue());
            sysDicKeyValueList.add(sysDicKeyValue);
        }
        return sysDicKeyValueList;
    }

    //@lwgkiller:根据字典类别key获取这个类别下的字典的key，value对Map
    public Map<String, String> getSysDicKeyValueMapByTreeKey(String TreeKey) {
        List<SysDic> sysDicList = this.getByTreeKey(TreeKey);
        Map<String, String> sysDicKeyValueMap = new HashedMap();
        for (SysDic sysDic : sysDicList) {
            sysDicKeyValueMap.put(sysDic.getKey(), sysDic.getValue());
        }
        return sysDicKeyValueMap;
    }
}