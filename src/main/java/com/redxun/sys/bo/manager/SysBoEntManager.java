...
public class SysBoEntManager extends MybatisBaseManager<SysBoEnt>{
	...
	/**
	 * 根据BoDefId获得主实体定义,不含属性定义
	 * @param  boDefId
	 */
	public SysBoEnt getMainByBoDefIdExludeAttrs(String boDefId){
		return sysBoEntDao.getMainByBoDefId(boDefId);
	}
   ...
   private void removeRefFields(SysBoEnt boent){
        ...
        while (it.hasNext()) {
            ...
            if ("mini-ref".equals(attr.getControl()) || "mini-commonfield".equals(attr.getControl()) || attr.getName().equals(boent.getPkField()) || attr.getName().equals(boent.getFkField()) || attr.getName().equals(boent.getParentField())) {
                it.remove();
            }
        }
   }
	...
	/**
	 * 根据子表的id获取boDefId来获取BOENT对象。
	 * @param boDefId
	 * @return
	 */
	public SysBoEnt getBySubBoId(String id){
		if(StringUtils.isEmpty(id)){
			return null;
		}

		List<SysBoEnt> list= sysBoEntDao.getBySqlKey("getBySubBoId",id);
    	...
    	for(int i=0;i<rowArr.size();i++){
    		...
    		while(fieldIt.hasNext()){
    			...
    			if(StringUtils.isNotEmpty(field) && !field.startsWith("_")){
        			...
        			if(isInsert){
        				if(field.equals(fkField))continue;
        				if(cn>0){
            				sqlBuffer.append(",");
            				valBuffer.append(",");
            			}
	...
	private void updBoEnt(SysBoEnt oldBoEnt ,SysBoEnt boEnt){
		...
		oldBoEnt.setFkField(boEnt.getFkField());
		if(!boEnt.isDbMode()){
			oldBoEnt.setTableName(boEnt.getTableName());
		}
