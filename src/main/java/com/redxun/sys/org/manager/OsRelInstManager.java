...
public class OsRelInstManager extends MybatisBaseManager<OsRelInst>{
	 ...
	 * @param tenantId
	 */
	public void addGroupUserLeader(String groupId,String userId,String tenantId){
		OsRelInst inst=new OsRelInst();
		inst.setInstId(IdUtil.getId());
		inst.setDim1(OsDimension.DIM_ADMIN_ID);
		inst.setDim2(null);
		inst.setParty1(groupId);
		inst.setParty2(userId);
		inst.setRelTypeId(OsRelType.REL_CAT_GROUP_USER_LEADER_ID);
		inst.setRelTypeKey(OsRelType.REL_CAT_GROUP_USER_LEADER);
		inst.setRelType(OsRelType.REL_TYPE_GROUP_USER);
		inst.setStatus(MBoolean.ENABLED.name());
