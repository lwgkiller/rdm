...
public class OsGroupManager extends BaseManager<OsGroup>{
	...
	public void delAndUpdateChilds(String groupId){
		...
		//删除中间的关系
		logger.error("!!!delete inst, groupId {}",groupId);
	 ...
	 * @param sysIdSet 系统Id
	 */
	public void saveGrantMenus(String groupId,Set<String> menuIds,Set<String> sysIdSet ,String netType){
		
		//删除该Group当前网段原授权的子系统deleteByGroupIdAndNet
		osGroupSysManager.deleteByGroupIdAndNet(groupId,netType);

		//删除该Group当前网段原授权的子菜单deleteByGroupIdAndNet
		osGroupMenuManager.deleteByGroupIdAndNet(groupId,netType);
		
		//生成可以访问的子系统
		for(String sysId:sysIdSet){
			OsGroupSys gs=new OsGroupSys(idGenerator.getSID(), groupId, sysId, netType);
		...
		for(String menuId:menuIds){
			...
			OsGroupMenu m = new OsGroupMenu(idGenerator.getSID(), groupId, menuId, sysMenu.getSysId(),netType);
	...
	public List<OsGroup> getByDimAndParent(Map<String,Object> params){
		return osGroupDao.getByDimAndParent(params);
