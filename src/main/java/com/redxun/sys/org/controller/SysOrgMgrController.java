...
public class SysOrgMgrController extends BaseController {
    ...
    public ModelAndView editOrgAttr(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        List<SysTree> treeList =
            sysTreeManager.getByCatKeyTenantId("CAT_CUSTOMATTRIBUTE_GROUP", ContextUtil.getCurrentTenantId());
        List<OsCustomAttribute> osCustomAttributes =
            osCustomAttributeManager.getUserTypeAttributeByTenantId(tenantId, OsGroup.GROUP);
    ...
    @ResponseBody
    public List<OsCustomAttribute> listAttributes(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String groupId = request.getParameter(OsGroup.GROUPID);
        ...
        if ("true".equals(isEdit)) {
            ...
            osCustomAttributes =
                osCustomAttributeManager.getUserTypeAttributeByTarGetId(tenantId, groupId, OsGroup.GROUP);
        }
    ...
    @ResponseBody
    public List<OsGroup> listByMyConfig(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    public ModelAndView mgrConfig(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    @ResponseBody
    public List<OsGroup> listByMyDimId(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isEmpty(dimId)) {
            OsDimension osDimension =
                osDimensionManager.getByDimKeyTenantId(OsDimension.DIM_ADMIN, ITenant.PUBLIC_TENANT_ID);
        ...
        if ("0".equals(parentId)) {
            osGroups = osGroupManager.getByDimIdGroupIdTenantId(dimId, parentId, tenantId);
        } else {
        ...
        for (OsGroup osGroup : osGroups) {
            if (osGroup.getRankLevel() != null) {
                OsRankType type =
                    osRankTypeManager.getByDimIdRankLevelTenantId(dimId, osGroup.getRankLevel(), tenantId);
                if (type != null) {
    ...
    public ModelAndView mgrGrade(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String instId = request.getParameter(OsGroup.TENANTID);
        SysInst sysInst = null;
        if (StringUtils.isNotEmpty(instId) && !"null".equals(instId)) {
            // 当前用户为指定的管理机构下的用户才可以查询到指定的租用机构下的数据
        ...
        return getPathView(request).addObject(OsGroup.SYSINST, sysInst)
            .addObject("userId", ContextUtil.getCurrentUserId()).addObject("isSuperAdmin", curUser.isSaaSAdmin());
    }
    ...
    public JsonResult delGroup(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String groupId = request.getParameter(OsGroup.GROUPID);
    ...
    public JsonResult saveGroup(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String dimId = request.getParameter(OsGroup.DIMID);
    ...
    @ResponseBody
    public List<OsGroup> listByGroupId(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String groupId = request.getParameter(OsGroup.GROUPID);
        // 懒加载点击树控件
    ...
    @ResponseBody
    public List<OsGroup> listByDimId(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("topContacts".equals(dimId)) {// 如果是展示常用联系人则采用自定义查询返回常用联系人
            List<?> contactTypes = commonDao.query("select F_LXRFL from w_topcontacts where CREATE_USER_ID_="
                + ContextUtil.getCurrentUserId() + " group by F_LXRFL");
    ...
    @ResponseBody
    public List<OsGroup> search(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String dimId = request.getParameter(OsGroup.DIMID);
        ...
        if ("true".equals(excludeAdmin)) {// 显示角色维度
            OsDimension osDimension = osDimensionManager.get(OsDimension.DIM_ROLE_ID);
            ...
            OsDimension osDimension = osDimensionManager.get(OsDimension.DIM_ADMIN_ID);
            tempDimId = osDimension.getDimId();
        }
        ...
        if ((StringUtils.isNotEmpty(name) || StringUtils.isNotEmpty(key)) && StringUtils.isEmpty(parentId)) {
            list = osGroupManager.getByDimIdNameKey(tenantId, tempDimId, name, key);
        } else if (StringUtils.isEmpty(parentId)) {
            list = osGroupManager.getByDimIdGroupIdTenantId(tempDimId, "0", tenantId);
        } else {
    ...
    @ResponseBody
    public List<OsGroup> searchQY(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String dimId = request.getParameter(OsGroup.DIMID);
        String showDimId = request.getParameter("showDimId");
        String tempDimId = null;
        String tenantId = getCurTenantId(request);
    ...
    @ResponseBody
    public JsonPageResult listByGroupIdRelTypeId(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String groupId = request.getParameter(OsGroup.GROUPID);
        String relTypeId = request.getParameter("relTypeId");
        SqlQueryFilter filter = QueryFilterBuilder.createSqlQueryFilter(request);
        List<OsRelInst> list = osRelInstManager.getByGroupIdRelTypeId(groupId, relTypeId, filter);
    ...
    public JsonResult saveGroupRelInst(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String insts = request.getParameter("insts");
        JSONArray arrs = JSONArray.fromObject(insts);
        for (int i = 0; i < arrs.size(); i++) {
            JSONObject obj = arrs.getJSONObject(i);
            OsRelInst inst = (OsRelInst)JSONObject.toBean(obj, OsRelInst.class);
    ...
    public JsonResult removeOsRelInst(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    public JsonResult joinGroups(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String groupId = request.getParameter(OsGroup.GROUPID);
    ...
    public JsonResult batchSaveGroup(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String dimId = request.getParameter(OsGroup.DIMID);
        OsDimension osDimension = osDimensionManager.get(dimId);
     ...
     * @param osDimension
     */
    private void genGroups(String groupJson, OsGroup parentGroup, String tenantId, OsDimension osDimension) {
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            if (StringUtils.isNotEmpty(children)) {
                ...
                List<OsGroup> subGroup = osGroupManager.getByParentId(osGroup.getGroupId());
                String json = JSON.toJSONString(subGroup);
                genGroups(json, osGroup, tenantId, osDimension);
            }
            if (isCreated) {
                osGroupManager.create(osGroup);
            } else {
    ...
    /**
     * config: 结构: { type:"specific", groupId:"指定的组", grouplevel:"当type 为 level 时指定" } type: all:"打开对话框时没有指定父组织,默认为all"
     * specific:指定组 current:当前组织作为根节点 level:"指定级别,需要指定 grouplevel 级别"
     * 
     * @param request
     * @param response
     * @return
    ...
    public List<OsGroup> getInitData(HttpServletRequest request, HttpServletResponse response) {
        ...
        String deptKey = RequestUtil.getString(request, "deptKey", "");
        ...
        String tenantId = getCurTenantId(request);
        if (StringUtils.isNotBlank(deptKey) || StringUtils.isNotBlank(deptName)) {
            Map<String, Object> paramDept = new HashMap<String, Object>();
            paramDept.put("dimId", OsDimension.DIM_ADMIN_ID);
            if (StringUtils.isNotBlank(deptKey)) {
                paramDept.put("deptKey", deptKey);
            }
            if (StringUtils.isNotBlank(deptName)) {
                paramDept.put("deptName", deptName);
            }
            paramDept.put("tenantId", tenantId);
            result = osGroupManager.getByDimAndParent(paramDept);
            // 如果是搜索某个部门，则将child置成0
            if (result != null && !result.isEmpty()) {
                for(OsGroup group:result) {
                    group.setChilds(0);
                }
            }
        } else {
            ...
            result = osGroupManager.getByDimAndParent(params);
     ...
     * @throws Exception
     */
    @RequestMapping("copyGroupGrant")
    @ResponseBody
    public JsonResult copyGroupGrant(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            String srcDeptId = RequestUtil.getString(request, "srcDeptId");
            String targetDeptId = RequestUtil.getString(request, "targetDeptId");
            if (StringUtils.isBlank(srcDeptId)||StringUtils.isBlank(targetDeptId)) {
                return new JsonResult(false, "部门id不能为空！");
            }
            com.alibaba.fastjson.JSONObject params = new com.alibaba.fastjson.JSONObject();
            params.put("srcDeptId", srcDeptId);
            params.put("targetDeptId", targetDeptId);

            //以原授权方式更新权限
            params.put("netType", "1");
            List<String> includeSysIdMenuIds = rdmDao.getDistinctGrantMenu(params);
            List<String> sysIds = rdmDao.getDistinctGrantSys(params);
            if (includeSysIdMenuIds.size() > 0) {
                Set<String> menuIdsSet = new HashSet<>(includeSysIdMenuIds);
                Set<String> sysIdsSet = new HashSet<>(sysIds);
                osGroupManager.saveGrantMenus(targetDeptId, menuIdsSet, sysIdsSet, "1");
            }


            params.put("netType", "2");
            includeSysIdMenuIds = rdmDao.getDistinctGrantMenu(params);
            sysIds = rdmDao.getDistinctGrantSys(params);
            if (includeSysIdMenuIds.size() > 0) {
                Set<String> menuIdsSet = new HashSet<>(includeSysIdMenuIds);
                Set<String> sysIdsSet = new HashSet<>(sysIds);
                osGroupManager.saveGrantMenus(targetDeptId, menuIdsSet, sysIdsSet, "2");
            }
            ...
            logger.error("Exception in copyGroupGrant", e);
