...
public class OsGroupController extends BaseListController{
	...
	public JsonResult getGrantedResourceIds(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		String netType=StringUtil.isNotEmpty(request.getParameter("netType"))?request.getParameter("netType"):"1";
		List<Subsystem> grantSyses=subsystemManager.getGrantSubsByGroupId(groupId,netType);
		List<SysMenu> grantMenus=sysMenuManager.getGrantMenusByGroupId(groupId,netType);
	...
	public JsonResult saveGrant(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		osGroupManager.saveGrantMenus(groupId, includeSysIdMenuIds, sysIdSet, netType);
