...
public class OsUserController extends TenantListController {
    ...
    protected QueryFilter getQueryFilter(HttpServletRequest request) {
        ...
        queryFilter.addWhereClause(new QueryParam("iu.STATUS_", QueryParam.FIELD_TYPE_STRING, QueryParam.OP_IN,
            "'" + OsUser.STATUS_IN_JOB + "','" + OsUser.STATUS_OUT_JOB + "'"));
        return queryFilter;
    }
    ...
    public JsonResult del(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotEmpty(uId)) {
            ...
            for (String id : ids) {
                ...
                CacheUtil.delCache(OsUser.ACCOUNT_PRE + user.getUserNo());
                if (user.isSaaSAdmin()) {
                    hasAdmin = true;
                    continue;
                }
                user.setStatus(OsUser.STATUS_DEL_JOB);
                logger.error("!!!delete user, userId {}", id);
    ...
    public JsonResult modifyPassword(HttpServletRequest request, HttpServletResponse response) throws Exception {
        JsonResult result = new JsonResult(true, "成功修改密码！");
    ...
    public JsonResult editForgetPwd(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 检查验证码
        String code =
            (String)request.getSession().getAttribute(com.google.code.kaptcha.Constants.KAPTCHA_SESSION_CONFIG_KEY);
    ...
    public JsonPageResult<OsUser> search(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotEmpty(groupId)) {
            ...
            for (Object obj : list) {
                OsUser oneUser = (OsUser)obj;
                userIds.add(oneUser.getUserId());
            }
            // 根据用户查所属部门
            if (!userIds.isEmpty()) {
                ...
                Map<String, String> userId2DeptName = new HashMap<>();
                for (JSONObject oneDept : deptInfos) {
                    userId2DeptName.put(oneDept.getString("userId"), oneDept.getString("deptName"));
                }
                for (Object obj : list) {
                    OsUser oneUser = (OsUser)obj;
                    String userId = oneUser.getUserId();
                    oneUser.setDeptName(userId2DeptName.get(userId));
                }
            }
    ...
    public ModelAndView editAttr(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        List<SysTree> treeList =
            sysTreeManager.getByCatKeyTenantId("CAT_CUSTOMATTRIBUTE", ContextUtil.getCurrentTenantId());
        List<OsCustomAttribute> osCustomAttributes =
            osCustomAttributeManager.getUserTypeAttributeByTenantId(tenantId, OsUser.ATTR_USER);
    ...
    public ModelAndView edit(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotEmpty(pkId)) {
            ...
            setUserStatus(osUser,
                (StringUtil.isNotEmpty(editTenantId) ? editTenantId : ContextUtil.getCurrentTenantId()));
        ...
        return mv.addObject("osUser", osUser).addObject("domain", domain)
            .addObject("isSaasMode", WebAppUtil.getIsSaasMode()).addObject("tenantId", tenantId);
    ...
    @ResponseBody
    public JsonPageResult<OsGroup> getBelongGroups(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String userId = request.getParameter("userId");
    ...
    public JsonResult unjoinUser(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotEmpty(groupId) && StringUtils.isNotEmpty(relTypeId)) {
            ...
            for (String userId : uIds) {
                OsRelInst inst1 = osRelInstManager.getByParty1Party2RelTypeId(groupId, userId, relTypeId);
                logger.error("!!!delete inst, instId {}", inst1.getInstId());
    ...
    public JsonResult removeBelongRelType(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String userId = request.getParameter("userId");
        OsRelInst inst1 =
            osRelInstManager.getByParty1Party2RelTypeId(groupId, userId, OsRelType.REL_CAT_GROUP_USER_BELONG_ID);
        if (inst1 != null) {
            osRelInstManager.delete(inst1.getInstId());
        }
    ...
    @ResponseBody
    public JsonPageResult<OsUser> getSaasAdmin(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
    ...
    @ResponseBody
    public JsonPageResult<OsUser> getInstAdmin(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
