...
@Repository
public interface AwardGrouplScienceProgressDao {
    ...
    @Select("SELECT fileId FROM award_groupl_science_progress WHERE id = #{id} ")
    String selectFjId(@Param("id") String id);

    @Select("SELECT id from award_groupl_science_progress  WHERE fileId = #{fileId} ")
