...
@Repository
public interface YdgztbDao {

    // 表单的新增、更新和删除
    void insertYdgztb(JSONObject param);

    void updateYdgztb(JSONObject param);

    void updateYdgztbStatus();

    void chooseYxal(JSONObject param);

    void cancelYxal(JSONObject param);

    void deleteYdgztb(JSONObject param);
    ...
    List<JSONObject> queryJoinDept();

    List<JSONObject> queryDeptSub(JSONObject params);

    List<JSONObject> queryWtbInstIds();
}
