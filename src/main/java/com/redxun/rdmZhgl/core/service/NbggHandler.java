 ...
 * 集团内部外出交流
 * 
 * @mh 2022年5月18日08:49:56
 */

@Service
public class NbggHandler implements ProcessStartPreHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(NbggHandler.class);
    @Autowired
    ...
    private String createOrUpdateByFormData(AbstractExecutionCmd cmd) {
        ...
        if (StringUtils.isBlank(formDataJson.getString("id"))) {
            nbggManager.createNbgg(formDataJson);
        } else {
            nbggManager.updateNbgg(formDataJson);
        }
