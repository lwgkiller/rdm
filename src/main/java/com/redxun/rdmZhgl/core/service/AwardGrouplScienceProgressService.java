...
import com.redxun.rdmZhgl.core.dao.AwardGrouplScienceProgressDao;
import com.redxun.saweb.context.ContextUtil;
import com.redxun.saweb.util.IdUtil;
import com.redxun.saweb.util.RequestUtil;
import com.redxun.saweb.util.WebAppUtil;
import com.redxun.serviceEngineering.core.service.PartsAtlasService;
...
@Service
public class AwardGrouplScienceProgressService {
    ...
    @Autowired
    private AwardGrouplScienceProgressDao awardGrouplScienceProgressDao;
     ...
     * @return
     */
    public JsonPageResult<?> getCpaList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> cpaList = awardGrouplScienceProgressDao.queryAgpList(params);


        result.setData(cpaList);
        int countCpsfyList = awardGrouplScienceProgressDao.countAgpfyList(params);
        result.setTotal(countCpsfyList);
        return result;
    }

    // 根据id查询标准详情
    public Map<String, Object> queryAgpById(String awardId) {

        Map<String, Object> param = new HashMap<>();
        param.put("awardId", awardId);
        Map<String, Object> oneInfo = awardGrouplScienceProgressDao.queryAgpById(param);
        return oneInfo;
    }

    // 保存（包括新增保存、编辑保存）
    public void saveAgp(JSONObject result, HttpServletRequest request) {
        try {
...
//            result.put("id", objBody.get("id"));
    ...
    private void addOrUpdateCpa(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj, String awardId) throws IOException {

        String fjId = awardGrouplScienceProgressDao.selectFjId(parameters.get("id")[0]);
...
//            fileInfo.put("fileSize",parameters.get("fileSize")[0]);
            ...
            awardGrouplScienceProgressDao.addFileInfos(fileInfo);

            //获奖信息添加
            JSONObject userInfo = new JSONObject();
            userInfo.put("id", newId);
            userInfo.put("awardType", parameters.get("awardType")[0]);
//            userInfo.put("projetName", parameters.get("projetName")[0]);
            userInfo.put("honor", parameters.get("honor")[0]);
            userInfo.put("commendUnit", parameters.get("commendUnit")[0]);
            userInfo.put("certificateNumber", parameters.get("certificateNumber")[0]);
            userInfo.put("validity", parameters.get("validity")[0]);

            userInfo.put("nextReviewState", parameters.get("nextReviewState")[0]);
            ...
            userInfo.put("winningUnit", parameters.get("winningUnit")[0]);


            String prizeTime = parameters.get("prizeTime")[0];
            String expiryTime = parameters.get("expiryTime")[0];
            String predictRecheckTime = parameters.get("predictRecheckTime")[0];
            ...
            try {
                ...
                String expiryTimeStr = expiryTime.split(Pattern.quote("(中国标准时间)"))[0].replace("GMT+0800", "GMT+08:00");
                Date expiryTimedate = sdf.parse(expiryTimeStr);
                userInfo.put("expiryTime", sdf1.format(expiryTimedate));

                String predictRecheckTimeStr = predictRecheckTime.split(Pattern.quote("(中国标准时间)"))[0].replace("GMT+0800", "GMT+08:00");
                Date predictRecheckTimeStrdate = sdf.parse(predictRecheckTimeStr);
                userInfo.put("predictRecheckTime", sdf1.format(predictRecheckTimeStrdate));
            } catch (Exception e) {
            ...
            awardGrouplScienceProgressDao.saveApgList(userInfo);
...
//            fileInfo.put("projetName", parameters.get("projetName")[0]);
            fileInfo.put("honor", parameters.get("honor")[0]);
            fileInfo.put("commendUnit", parameters.get("commendUnit")[0]);
            fileInfo.put("certificateNumber", parameters.get("certificateNumber")[0]);
            fileInfo.put("validity", parameters.get("validity")[0]);
            fileInfo.put("nextReviewState", parameters.get("nextReviewState")[0]);
            ...
            fileInfo.put("winningUnit", parameters.get("winningUnit")[0]);
            ...
            try {
                ...
                String expiryTimeStr = expiryTime.split(Pattern.quote("(中国标准时间)"))[0].replace("GMT+0800", "GMT+08:00");
                Date expiryTimedate = sdf.parse(expiryTimeStr);
                fileInfo.put("expiryTime", sdf1.format(expiryTimedate));

                String predictRecheckTimeStr = predictRecheckTime.split(Pattern.quote("(中国标准时间)"))[0].replace("GMT+0800", "GMT+08:00");
                Date predictRecheckTimeStrdate = sdf.parse(predictRecheckTimeStr);
                fileInfo.put("predictRecheckTime", sdf1.format(predictRecheckTimeStrdate));
            } catch (Exception e) {
                logger.error("时间格式转化异常");
            }
            ...
            awardGrouplScienceProgressDao.updateAgpList(fileInfo);
        }
    }

    private void updatePublicStandardFile2Disk(String standardId, MultipartFile fileObj, Map<String, String[]> parameters, String awardId) throws IOException {
        if (StringUtils.isBlank(standardId) || fileObj == null) {
            logger.warn("no standardId or fileObj");
            return;
        }
        String standardFilePathBase = WebAppUtil.getProperty("agspeFilePathBase");
        if (StringUtils.isBlank(standardFilePathBase)) {
            logger.error("can't find agspeFilePathBase");
            return;
        }
       ...
       /* if (file.exists()) {
        ...
        String convertPdfPath = filePath + File.separator
                + File.separator + convertPdfDir + File.separator + standardId + ".pdf";
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                //添加
                message.put("id", id);
                awardGrouplScienceProgressDao.addAgpList(message);
            }
    ...
    private JSONObject generateDataFromRow(Row row, List<Map<String, Object>> itemList, List<String> titleList, String errorMessage) {
...
//                    oneRowMap.put("awardType", cellValue);
                ...
                case "序号":
                    ...
                    break;
                case "获得荣誉":
                    oneRowMap.put("honor", cellValue);
                    break;
                case "有效期":
                    oneRowMap.put("validity", cellValue);
                    break;
                case "失效时间":
                    if (cellValue != null && !"".equals(cellValue)) {
//                        boolean legalDate = isLegalDate(cellValue.length(), cellValue, "yyyy-MM-dd");
//                        if (legalDate) {
//                            oneRowMap.put("expiryTime", cellValue);
//                        } else {
//                            sb.append("列“" + title + "”时间格式异常;");
//                        }
                        oneRowMap.put("expiryTime", cellValue);
                    } else {
                        oneRowMap.put("expiryTime", null);
                    }
...
//                        boolean legalDate = isLegalDate(cellValue.length(), cellValue, "yyyy-MM-dd");
//                        if (legalDate) {
//                            oneRowMap.put("predictRecheckTime", cellValue);
//                        } else {
//                            sb.append("列“" + title + "”时间格式异常;");
//                        }
                        oneRowMap.put("predictRecheckTime", cellValue);
                    } else {
                        oneRowMap.put("predictRecheckTime", null);
                    }

                    break;
                case "下次评审时状态":
                    if ("有效".equals(cellValue)) {
                        oneRowMap.put("nextReviewState", "1");
                    } else {
                        oneRowMap.put("nextReviewState", "0");
                    }
                    break;
                case "证书编号":
                    ...
                    break;
                case "获奖单位(集体)":
                    oneRowMap.put("winningUnit", cellValue);
                    break;
                 case "表彰单位或部门":
...
//                    return oneRowCheck;
    ...
    public void exportAgspeList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getCpaList(request, response, false);
        ...
        for (Map<String, Object> oneMap : listData) {
            ...
            oneMap.put("statusName", convertpsStatusName(oneMap.get("nextReviewState")));
        }
        String nowDate = DateFormatUtil.getNowByString("yyyy-MM-dd");
        String title = "高品奖列表";
        String excelName = nowDate + title;
        String[] fieldNames = {"获奖类别", "荣誉", "表彰单位", "证书编号", "有效期", "失效时间", "预计复审时间", "下次评审状态", "获奖单位","获奖人","获奖时间", "创建时间"};
        String[] fieldCodes = {"typeName", "honor", "commendUnit", "certificateNumber", "validity","expiryTime","predictRecheckTime",
                "statusName", "winningUnit", "prizewinner", "prizeTime", "CREATE_TIME_"};
    ...
    private String convertpsStatusName(Object statusKey) {
        if (statusKey == null) {
            return "";
        }
        String statusKeyStr = statusKey.toString();
