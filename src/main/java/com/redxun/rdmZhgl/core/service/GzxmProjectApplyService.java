...
@Service
@Slf4j
public class GzxmProjectApplyService {
    private static Logger logger = LoggerFactory.getLogger(GzxmProjectApplyService.class);
    @Resource
    ...
    public void add(Map<String, Object> params) {
        ...
        gzxmProjectApplyDao.add(params);
    ...
    public void update(Map<String, Object> params) {
        params.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
        params.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
        gzxmProjectApplyDao.update(params);
    }
    public List<JSONObject> getSubjectList(HttpServletRequest request){
        List<JSONObject> list = new ArrayList<>();
        try {
            Map<String, Object> params = new HashMap<>(16);
            params = CommonFuns.getSearchParam(params, request, true);
            String ids = CommonFuns.nullToString(params.get("projectIds"));
            String[] idArr = ids.split(",", -1);
            List<String> idList = Arrays.asList(idArr);
            params.put("ids", idList);
            list = gzxmProjectApplyDao.getSubjectList(params);
            List<String> timeNames = new ArrayList<>();
            timeNames.add("startDate");
            timeNames.add("endDate");
            CommonFuns.convertJsonListDate(list,timeNames,"yyyy-MM-dd");
        }catch (Exception e){
            logger.error("Exception in getSubjectList", e);
        }
        return list;
    }
    ...
    public JsonResult delete(String[] applyIdArr, String[] instIdArr) {
        ...
        for (int i = 0; i < applyIdArr.length; i++) {
            ...
            gzxmProjectApplyDao.delete(applyId);
            // 删除实例
    ...
    public JsonPageResult<?> queryList(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<Map<String, Object>> applyList = gzxmProjectApplyDao.queryList(params);
    ...
    public JSONObject getBpmSolution(Map<String, Object> param) {
        ...
        List<Map<String, Object>> list = gzxmProjectApplyDao.getBmpSolution(param);
