 ...
 * */
@Service
public class XpszApplyHandler implements ProcessStartPreHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(XpszApplyHandler.class);
    @Resource
    private XpszApplyService xpszApplyService;
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            xpszApplyService.add(applyObj);
        } else {
            xpszApplyService.update(applyObj);
        }
        return applyObj;
    }
}
