 ...
 * */
@Service
public class GzxmProjectApplyHandler implements ProcessStartPreHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(GzxmProjectApplyHandler.class);
    @Resource
    private GzxmProjectApplyService gzxmProjectApplyService;
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            gzxmProjectApplyService.add(applyObj);
        } else {
            gzxmProjectApplyService.update(applyObj);
        }
        return applyObj;
    }
}
