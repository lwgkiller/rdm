...
import com.redxun.rdmZhgl.core.dao.AwardPatentDao;
...
@Service
public class AwardPatentService {

    private static Logger logger = LoggerFactory.getLogger(AwardPatentService.class);

    @Autowired
    private RdmZhglUtil rdmZhglUtil;

    @Autowired
    private AwardPatentDao awardPatentDao;
     ...
     * @return
     */
    public JsonPageResult<?> getApeList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> cpaList = awardPatentDao.queryApeList(params);


        result.setData(cpaList);
        int countCpsfyList = awardPatentDao.countAgpfyList(params);
    ...
    public Map<String, Object> queryAgpById(String awardId) {
        ...
        Map<String, Object> oneInfo = awardPatentDao.queryAgpById(param);
        return oneInfo;
    }
...
//            result.put("id", objBody.get("id"));
    ...
    private void addOrUpdateCpa(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj, String awardId) throws IOException {
        ...
        String fjId = awardPatentDao.selectFjId(parameters.get("id")[0]);
...
//            fileInfo.put("fileSize",parameters.get("fileSize")[0]);
            ...
            awardPatentDao.addFileInfos(fileInfo);
            ...
            awardPatentDao.saveApgList(userInfo);
...
//            info.put("fileSize",parameters.get("fileSize")[0]);
            ...
            awardPatentDao.updateAgpList(fileInfo);
        }
    }
    ...
    private void updatePublicStandardFile2Disk(String standardId, MultipartFile fileObj, Map<String, String[]> parameters, String awardId) throws IOException {
        ...
        String standardFilePathBase = WebAppUtil.getProperty("apeFilePathBase");
        if (StringUtils.isBlank(standardFilePathBase)) {
            logger.error("can't find apeFilePathBase");
            return;
        }
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                //添加
                message.put("id", id);
                awardPatentDao.addAgpList(message);
            }
...
//                    return oneRowCheck;
    ...
    public void exportApeList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getApeList(request, response, false);
        ...
        String[] fieldNames = {"获奖类别", "项目名称", "表彰单位或部门", "荣誉", "获奖单位", "获奖人", "获奖时间", "备注", "创建时间"};
        String[] fieldCodes = {"typeName", "projetName", "commendUnit", "honor",
