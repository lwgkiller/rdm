...
@Service
@Slf4j
public class XpszFinishApplyService {
    private static Logger logger = LoggerFactory.getLogger(XpszFinishApplyService.class);
    @Resource
    ...
    public void update(Map<String, Object> params) {
        params.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
        params.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
        xpszFinishApplyDao.update(params);
    }
    ...
    public JsonResult delete(String[] applyIdArr, String[] instIdArr) {
        ...
        for (int i = 0; i < applyIdArr.length; i++) {
            ...
            xpszFinishApplyDao.delete(applyId);
            // 删除实例
    ...
    public JsonPageResult<?> queryList(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<Map<String, Object>> applyList = xpszFinishApplyDao.queryList(params);
    ...
    public JSONObject getBpmSolution(Map<String, Object> param) {
        ...
        List<Map<String, Object>> list = xpszFinishApplyDao.getBmpSolution(param);
    ...
    public JSONObject changePlan(JSONObject postDataJson) {
        ...
        try {
            ...
            JSONObject changeObj = xpszFinishApplyDao.getJsonObject(id);
    ...
    public JSONObject verify(JSONObject postDataJson) {
        JSONObject resultJson = new JSONObject();
        try {
            JSONObject changeObj = xpszFinishApplyDao.getObjectByInfo(postDataJson);
            if(changeObj!=null){
                return ResultUtil.result(false, "审批已存在！", "");
            }
        } catch (Exception e) {
            logger.error("Exception in getObjectByInfo ", e);
            return ResultUtil.result(false, "获取信息异常！", "");
        }
