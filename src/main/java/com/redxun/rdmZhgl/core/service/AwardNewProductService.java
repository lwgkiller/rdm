...
@Service
public class AwardNewProductService {
    private static Logger logger = LoggerFactory.getLogger(AwardNewProductService.class);
    ...
    * 查询列表
    * */
    public JsonPageResult<?> getAnpList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> anpList = awardNewProductDao.queryAnpList(params);

        result.setData(anpList);
        int countAnpfyList = awardNewProductDao.countAnpfyList(params);
        result.setTotal(countAnpfyList);
        return result;
    }
    public Map<String, Object> queryAnpById(String awardId) {

        Map<String, Object> param = new HashMap<>();
        param.put("awardId", awardId);
        Map<String, Object> oneInfo = awardNewProductDao.queryAnpById(param);
        return oneInfo;
    }
    // 保存（包括新增保存、编辑保存）
    public void saveAnp(JSONObject result, HttpServletRequest request) {
        try {
            ...
            addOrUpdateAnp(parameters, objBody, fileObj, awardId);
    ...
    private void addOrUpdateAnp(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj,
                                String awardId) throws IOException {

        String fjId = awardNewProductDao.selectFjId(parameters.get("id")[0]);
        if (StringUtils.isBlank(awardId)) {
            ...
            awardNewProductDao.addFileInfos(fileInfo);
            ...
            awardNewProductDao.saveAnpList(userInfo);
            ...
            if (StringUtils.isBlank(fjId)) {
                ...
                if (StringUtils.isNotBlank(parameters.get("fileName")[0])) {
                    ...
                    awardNewProductDao.updateFileInfos(info);
                }
            ...
            awardNewProductDao.updateAnpList(fileInfo);
        }
    }
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                // 添加
                message.put("id", id);
                awardNewProductDao.addAnpList(message);
            }
...
/*            cell.setCellType(Cell.CELL_TYPE_STRING);
            String cellValue = null;
            if (cell != null) {
                if("获奖时间".equals(title)){
                    List<String> stList = Arrays.asList(cell.getStringCellValue().split("/",-1));
                    StringBuilder s1= new StringBuilder();
                    s1.append(stList.get(2)).append("-").append(stList.get(1)).append("-").append(stList.get(0));
                    cellValue=StringUtils.trim(s1.toString());
                }else {
...
//                        }
    ...
    public void exportAnpList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getAnpList(request, response, false);
        ...
        String[] fieldNames = {"获奖类别", "证书编号", "荣誉", "表彰单位或部门", "获奖人", "获奖时间", "有效期", "创建时间"};
        String[] fieldCodes = {"typeName", "certificateNumber", "honor", "commendUnit", "prizewinner",
                "prizeTime", "validity", "CREATE_TIME_"};
