 ...
 * */
@Service
public class NdkfjhDelApplyHandler implements ProcessStartPreHandler, TaskPreHandler, ProcessStartAfterHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(NdkfjhDelApplyHandler.class);
    @Resource
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            ndkfjhDelApplyService.add(applyObj);
        } else {
            ndkfjhDelApplyService.update(applyObj);
        }
        return applyObj;
    }
    // 流程成功结束之后的处理
    ...
    public void endHandle(BpmInst bpmInst) {
        if ("SUCCESS_END".equalsIgnoreCase(bpmInst.getStatus())) {
            ...
            JSONObject detailObj = ndkfjhDelApplyDao.getJsonObject(id);
            String mainId = detailObj.getString("mainId");
            ndkfjhPlanDetailDao.delById(mainId);
            ndkfjhPlanDetailDao.delHistoryById(mainId);
        }
    }
