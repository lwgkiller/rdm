...
@Service
public class AwardGroupProgressService {

    private static Logger logger = LoggerFactory.getLogger(AwardGroupProgressService.class);
    ...
    public JsonPageResult<?> getCpaList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> cpaList = awardGroupProgressDao.queryAgpList(params);

        result.setData(cpaList);
        int countCpsfyList = awardGroupProgressDao.countAgpfyList(params);
    ...
    public Map<String, Object> queryAgpById(String awardId) {
        ...
        Map<String, Object> oneInfo = awardGroupProgressDao.queryAgpById(param);
        return oneInfo;
    }
    ...
    private void addOrUpdateCpa(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj,
        ...
        String fjId = awardGroupProgressDao.selectFjId(parameters.get("id")[0]);
        if (StringUtils.isBlank(awardId)) {
            ...
            awardGroupProgressDao.addFileInfos(fileInfo);
            ...
            awardGroupProgressDao.saveApgList(userInfo);
            ...
            awardGroupProgressDao.updateAgpList(fileInfo);
        }
    }
    ...
    private void updatePublicStandardFile2Disk(String standardId, MultipartFile fileObj,
        ...
        String standardFilePathBase = WebAppUtil.getProperty("agpFilePathBase");
        if (StringUtils.isBlank(standardFilePathBase)) {
            logger.error("can't find agpFilePathBase");
            return;
        }
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            Sheet sheet = wb.getSheet("集团科技进步奖");
            if (sheet == null) {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                // 添加
                message.put("id", id);
                awardGroupProgressDao.addAgpList(message);
            }
    ...
    public ResponseEntity<byte[]> importTemplateDownload() {
        try {
            String fileName = "集团科技进步奖导入模板.xlsx";
            // 创建文件实例
            File file =
...
//                        }
                        ...
                        oneRowCheck.put("message", "获奖时间为空");
                        return oneRowCheck;
                    }
    ...
    public void exportAgpList(HttpServletRequest request, HttpServletResponse response) {
        ...
        String title = "集团科技进步奖列表";
        String excelName = nowDate + title;
        String[] fieldNames = {"获奖类别", "项目名称", "荣誉", "获奖单位", "获奖人", "获奖时间", "备注", "创建时间"};
        String[] fieldCodes = {"typeName", "projetName", "honor", "winningUnit", "prizewinner",
                "prizeTime", "remark", "CREATE_TIME_"};
