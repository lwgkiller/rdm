 ...
 * */
@Service
public class MonthWorkDelayApplyHandler implements ProcessStartPreHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(MonthWorkDelayApplyHandler.class);
    @Autowired
    private MonthWorkDelayApplyService monthWorkDelayApplyService;
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            monthWorkDelayApplyService.add(applyObj);
        } else {
            monthWorkDelayApplyService.update(applyObj);
        }
        return applyObj;
    }
}
