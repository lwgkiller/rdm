...
@Service
public class JszbHandler implements ProcessStartPreHandler, ProcessStartAfterHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(JszbHandler.class);
    @Autowired
    ...
    public void processStartPreHandle(ProcessStartCmd processStartCmd) {
        String jszbId = createOrUpdateByFormData(processStartCmd);
        if (StringUtils.isNotBlank(jszbId)) {
            processStartCmd.setBusinessKey(jszbId);
        }
    }
    ...
    private String createOrUpdateByFormData(AbstractExecutionCmd cmd) {
        ...
        return formDataJson.getString("jszbId");
    }
}
