...
import com.redxun.rdmZhgl.core.dao.CityProgressAwardDao;
...
@Service
public class CityProgressAwardService {

    private static Logger logger = LoggerFactory.getLogger(CityProgressAwardService.class);

    @Autowired
    private RdmZhglUtil rdmZhglUtil;

    @Autowired
    private CityProgressAwardDao cityProgressAwardDao;
    ...
    public JsonPageResult<?> getCpaList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> cpaList = cityProgressAwardDao.queryCpaList(params);


        result.setData(cpaList);
        int countCpsfyList = cityProgressAwardDao.countCpsfyList(params);
        result.setTotal(countCpsfyList);
        return result;
    }

    // 根据id查询标准详情
    public Map<String, Object> queryCpaById(String awardId) {

        Map<String, Object> param = new HashMap<>();
        param.put("awardId", awardId);
        Map<String, Object> oneInfo = cityProgressAwardDao.queryCpaById(param);
        return oneInfo;
    }

    // 保存（包括新增保存、编辑保存）
    public void saveCpa(JSONObject result, HttpServletRequest request) {
        try {
...
//            result.put("id", objBody.get("id"));
    ...
    private void addOrUpdateCpa(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj, String awardId) throws IOException {
        ...
        String fjId = cityProgressAwardDao.selectFjId(parameters.get("id")[0]);
...
//            fileInfo.put("fileSize",parameters.get("fileSize")[0]);
            ...
            cityProgressAwardDao.addFileInfos(fileInfo);
            ...
            cityProgressAwardDao.saveCpaList(userInfo);
...
//            info.put("fileSize",parameters.get("fileSize")[0]);
            ...
            cityProgressAwardDao.updateCpaList(fileInfo);
        }
    }
    ...
    private void updatePublicStandardFile2Disk(String standardId, MultipartFile fileObj, Map<String, String[]> parameters, String awardId) throws IOException {
        ...
        String standardFilePathBase = WebAppUtil.getProperty("cpaFilePathBase");
        if (StringUtils.isBlank(standardFilePathBase)) {
            logger.error("can't find cpaFilePathBase");
            return;
        }
    ...
    public void exportMaterialList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getCpaList(request, response, true);
        ...
        if (!listData.isEmpty()) {
            ...
            if (languageList.contains("0")) {
                fieldNames.add("英文");
                fieldCodes.add("englishName");
            }
            if (languageList.contains("2")) {
                fieldNames.add("葡文");
                fieldCodes.add("portugueseName");
            }
            if (languageList.contains("3")) {
                fieldNames.add("德文");
                fieldCodes.add("germanyName");
            }
            if (languageList.contains("4")) {
                fieldNames.add("西文");
                fieldCodes.add("spanishName");
            }
            if (languageList.contains("5")) {
                fieldNames.add("法文");
                fieldCodes.add("frenchName");
            }
            if (languageList.contains("6")) {
                fieldNames.add("意文");
                fieldCodes.add("italianName");
            }
            if (languageList.contains("1")) {
                fieldNames.add("俄文");
                fieldCodes.add("russianName");
            }

            HSSFWorkbook wbObj = ExcelUtils.exportExcelWB(listData, fieldNames.toArray(new String[fieldNames.size()]), fieldCodes.toArray(new String[fieldCodes.size()]), title);
            // 输出
            ExcelUtils.writeWorkBook2Stream(excelName, wbObj, response);

        } else {
            String[] fieldNames = {"中文", "英文", "葡文", "德文", "西文", "法文", "意文", "俄文"};
            String[] fieldCodes = {"chineseName", "englishName", "portugueseName", "germanyName", "spanishName",
                    "frenchName", "italianName", "russianName"};
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            Sheet sheet = wb.getSheet("市级及以上科技进步奖");
            if (sheet == null) {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                //添加
                message.put("id", id);
                cityProgressAwardDao.addCpaList(message);
            }
    ...
    public ResponseEntity<byte[]> importTemplateDownload() {
        try {
            String fileName = "市级及以上科技进步奖导入模板.xlsx";
            // 创建文件实例
            File file = new File(
...
//                    oneRowMap.put("awardType", cellValue);
                ...
                case "序号":
                    ...
                    break;
                case "项目名称":
                    oneRowMap.put("projetName", cellValue);
                    break;
                case "证书编号":
...
//                    return oneRowCheck;
    ...
    public void exportCpaList(HttpServletRequest request, HttpServletResponse response) {
        ...
        String nowDate = DateFormatUtil.getNowByString("yyyy-MM-dd");
        String title = "市级及以上科技进步奖列表";
        String excelName = nowDate + title;
        String[] fieldNames = {"获奖类别", "表彰单位", "项目名称", "证书编号", "荣誉", "获奖单位", "获奖人", "获奖时间", "备注", "创建时间"};
        String[] fieldCodes = {"typeName", "commendUnit", "projetName", "certificateNumber", "honor",
                "winningUnit", "prizewinner", "prizeTime", "remark", "CREATE_TIME_"};
        HSSFWorkbook wbObj = ExcelUtils.exportExcelWB(listData, fieldNames, fieldCodes, title);
        // 输出
        ExcelUtils.writeWorkBook2Stream(excelName, wbObj, response);
    }
