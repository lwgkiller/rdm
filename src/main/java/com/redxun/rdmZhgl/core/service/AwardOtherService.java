...
import com.redxun.rdmZhgl.core.dao.AwardOtherDao;
...
@Service
public class AwardOtherService {
    private static Logger logger = LoggerFactory.getLogger(AwardOtherService.class);
    @Autowired
    private AwardOtherDao awardOtherDao;
    @Autowired
    private RdmZhglUtil rdmZhglUtil;

    public JsonPageResult<?> getAotList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> aotList = awardOtherDao.queryAotList(params);


        result.setData(aotList);
        int countAotfyList = awardOtherDao.countAotfyList(params);
        result.setTotal(countAotfyList);
        return result;
    }
    // 根据id查询标准详情
    public Map<String, Object> queryAotById(String awardId) {

        Map<String, Object> param = new HashMap<>();
        param.put("awardId", awardId);
        Map<String, Object> oneInfo = awardOtherDao.queryAotById(param);
        return oneInfo;
    }
    // 保存（包括新增保存、编辑保存）
    public void saveAot(JSONObject result, HttpServletRequest request) {
        try {
            ...
            addOrUpdateAot(parameters, objBody, fileObj, awardId);
...
//            result.put("id", objBody.get("id"));
    ...
    private void addOrUpdateAot(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj, String awardId) throws IOException {

        String fjId = awardOtherDao.selectFjId(parameters.get("id")[0]);
...
//            fileInfo.put("fileSize",parameters.get("fileSize")[0]);
            ...
            awardOtherDao.addFileInfos(fileInfo);
            ...
            awardOtherDao.saveAotList(userInfo);
            ...
            awardOtherDao.updateAotList(fileInfo);
        }
    }
    private void updatePublicStandardFile2Disk(String standardId, MultipartFile fileObj, Map<String, String[]> parameters, String awardId) throws IOException {
        ...
        String standardFilePathBase = WebAppUtil.getProperty("aotFilePathBase");
        if (StringUtils.isBlank(standardFilePathBase)) {
            logger.error("can't find aotFilePathBase");
            return;
        }
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                //添加
                message.put("id", id);
                awardOtherDao.addAotList(message);
            }
...
//                    return oneRowCheck;
    ...
    public void exportAotList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getAotList(request, response, false);
        ...
        String[] fieldNames = {"获奖类别", "项目名称", "荣誉", "表彰单位", "获奖单位", "获奖人", "获奖时间", "备注", "创建时间"};
        String[] fieldCodes = {"typeName", "projetName", "honor", "commendUnit",
