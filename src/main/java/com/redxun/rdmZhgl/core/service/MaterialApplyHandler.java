 ...
 * */
@Service
public class MaterialApplyHandler implements ProcessStartPreHandler, TaskPreHandler, ProcessStartAfterHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(MaterialApplyHandler.class);
    @Resource
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            materialApplyService.add(applyObj);
        } else {
            materialApplyService.update(applyObj);
        }
        return applyObj;
    }
    // 流程成功结束之后的处理
    ...
    public void endHandle(BpmInst bpmInst) {
        if ("SUCCESS_END".equalsIgnoreCase(bpmInst.getStatus())) {
            //流程结束以后，推送信息到SAP系统中；
            materialApplyService.sendToSap(bpmInst.getBusKey());
        }
    }
