...
public class AwardManagementService {
    private static Logger logger = LoggerFactory.getLogger(AwardManagementService.class);
    @Autowired
     ...
     * 查询列表
     * */
    public JsonPageResult<?> getAmgList(HttpServletRequest request, HttpServletResponse response, boolean doPage) {
        ...
        List<Map<String, Object>> amgList = awardManagementDao.queryAmgList(params);

        result.setData(amgList);
        int countAmgfyList = awardManagementDao.countAmgfyList(params);
        result.setTotal(countAmgfyList);
        return result;
    }
    public Map<String, Object> queryAmgById(String awardId) {

        Map<String, Object> param = new HashMap<>();
        param.put("awardId", awardId);
        Map<String, Object> oneInfo = awardManagementDao.queryAmgById(param);
        return oneInfo;
    }
    // 保存（包括新增保存、编辑保存）
    public void saveAmg(JSONObject result, HttpServletRequest request) {
        try {
    ...
    private void addOrUpdateAnp(Map<String, String[]> parameters, Map<String, Object> objBody, MultipartFile fileObj,
        ...
        String fjId = awardManagementDao.selectFjId(parameters.get("id")[0]);
        if (StringUtils.isBlank(awardId)) {
            ...
            awardManagementDao.addFileInfos(fileInfo);
            ...
            awardManagementDao.saveAmgList(userInfo);
            ...
            if (StringUtils.isBlank(fjId)) {
                ...
                if (StringUtils.isNotBlank(parameters.get("fileName")[0])) {
                    ...
                    awardManagementDao.updateFileInfos(info);
                }
            ...
            awardManagementDao.updateAmgList(fileInfo);
        }
    }
    ...
    public void importMaterial(JSONObject result, HttpServletRequest request) {
        try {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                // 添加
                message.put("id", id);
                awardManagementDao.addAmgList(message);
            }
...
//                        }
    ...
    public void exportAmgList(HttpServletRequest request, HttpServletResponse response) {
        JsonPageResult result = getAmgList(request, response, false);
        ...
        String[] fieldNames = {"获奖类别", "项目名称", "荣誉", "表彰单位或部门", "获奖人", "获奖时间", "备注", "创建时间"};
        String[] fieldCodes = {"typeName",  "projectName", "honor",
                "commendUnit", "prizewinner", "prizeTime", "remark", "CREATE_TIME_"};
