 ...
 */
@Service
public class YfjbProductionNoticeService {
    private static final Logger logger = LoggerFactory.getLogger(YfjbProductionNoticeService.class);
    @Resource
    ...
    public JsonPageResult<?> query(HttpServletRequest request) {
        ...
        try {
            ...
            list = yfjbProductionNoticeDao.query(params);
            ...
            totalList = yfjbProductionNoticeDao.query(params);
            convertDate(list);
            // 返回结果
            result.setData(list);
    ...
    public JSONObject add(HttpServletRequest request) {
        ...
        try {
            ...
            yfjbProductionNoticeDao.addObject(objBody);
        } catch (Exception e) {
    ...
    public JSONObject update(HttpServletRequest request) {
        ...
        try {
            ...
            objBody.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
            objBody.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
            yfjbProductionNoticeDao.updateObject(objBody);
    ...
    public JSONObject remove(HttpServletRequest request) {
        ...
        try {
            ...
            yfjbProductionNoticeDao.batchDelete(params);
        } catch (Exception e) {
            logger.error("Exception in update 删除试制通知单", e);
            return ResultUtil.result(false, "删除试制通知单异常！", "");
        }
        return ResultUtil.result(true, "删除成功", resultJson);
    }
    public JSONObject getObjectById(String id){
        JSONObject jsonObject =  yfjbProductionNoticeDao.getObjectById(id);
        return jsonObject;
    }
    public void exportExcel(HttpServletRequest request, HttpServletResponse response) {
        String nowDate = DateFormatUtil.getNowByString("yyyy-MM-dd");

        String excelName = nowDate + "试制通知单";
        Map<String, Object> params = new HashMap<>(16);
        params = CommonFuns.getSearchParam(params, request, false);
        String deptId = CommonFuns.nullToString(params.get("deptId"));
        if(StringUtil.isEmpty(deptId)){
        ...
        List<Map<String, Object>> list = yfjbProductionNoticeDao.query(params);
        convertDate(list);
        //转换字典项
        ...
        String[] fieldNames = {"原物料编码", "原物料名称", "原供应商", "原物料价格", "降本方式", "降本措施", "替代物料编码", "替代物料名称", "替代供应商",
                ...
                "实际切换时间","所属专业","责任人","涉及机型","试制通知号","试制通知单下发时间","备注"};
        String[] fieldCodes = {"orgItemCode", "orgItemName", "orgSupplier","orgItemPrice", "costTypeText", "costMeasure", "newItemCode", "newItemName", "newSupplier",
                ...
                "sjqh_date","majorText","responseMan","saleModel","productionNo","noticeDate","remark"};
