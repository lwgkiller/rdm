 ...
 * */
@Service
public class NdkfjhMonthApplyHandler implements ProcessStartPreHandler, TaskPreHandler, ProcessStartAfterHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(NdkfjhMonthApplyHandler.class);
    @Resource
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            ndkfjhMonthApplyService.add(applyObj);
        } else {
            ndkfjhMonthApplyService.update(applyObj);
        }
        return applyObj;
    }
    // 流程成功结束之后的处理
