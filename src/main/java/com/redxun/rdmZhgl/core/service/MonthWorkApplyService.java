...
@Service
@Slf4j
public class MonthWorkApplyService {
    private static Logger logger = LoggerFactory.getLogger(MonthWorkApplyService.class);
    @Resource
    ...
    public void add(Map<String, Object> params) {
        ...
        monthWorkApplyDao.add(params);

    }
    public void update(Map<String, Object> params) {
        params.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
        params.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
        monthWorkApplyDao.update(params);
    }
    /**
     * 删除表单及关联的所有信息
     */
    public JsonResult delete(String[] applyIdArr, String[] instIdArr) {
        ...
        for (int i = 0; i < applyIdArr.length; i++) {
            ...
            monthWorkApplyDao.delete(applyId);
            // 删除实例
    ...
    public JsonPageResult<?> queryList(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<Map<String, Object>> applyList = monthWorkApplyDao.queryList(params);
    ...
    public List<JSONObject> getApplyListByParam(JSONObject jsonObject){
        return monthWorkApplyDao.getApplyListByParam(jsonObject);
    }
    ...
    public JSONObject getBpmSolution(Map<String, Object> param) {
        ...
        List<Map<String, Object>> list = monthWorkApplyDao.getBmpSolution(param);
    ...
    public JSONObject getAbolishInfo(JSONObject postDataJson, HttpServletResponse response, HttpServletRequest request)
        ...
        List<Map<String, String>> list = monthWorkApplyDao.queryAbolishInfo(params);
        JSONObject resultJson = new JSONObject();
        if (list != null && list.size() > 0) {
    ...
    public JSONObject getActTaskById(JSONObject paramJson){
        ...
        String yearMonth = paramJson.getString("yearMonth");
        String deptId = paramJson.getString("deptId");
        List<JSONObject> list = new ArrayList<>();
        JSONObject resultJson;
        JSONObject processInfo = new JSONObject();
        boolean flag = true;
        if("addUnplan".equalsIgnoreCase(action)){
            JSONObject param = new JSONObject();
            param.put("yearMonth",yearMonth);
            param.put("deptId",deptId);
            List<JSONObject> ydzgApplyInfo = monthWorkApplyDao.getYdzgApplyInfo(param);
            if(ydzgApplyInfo!=null&&!ydzgApplyInfo.isEmpty()){
                flag = false;
            }
        }else {
            if("0".equals(type)){
                list = monthWorkApplyDao.getProjectApplyInfo(projectId);
            }else if("1".equals(type)){
                list = monthWorkApplyDao.getUnProjectApplyInfo(projectId);
            }else if("2".equals(type)){
                list = monthWorkApplyDao.getUnTaskApplyInfo(projectId);
            }
            if(list!=null&&list.size()==0){
                flag = true;
            }else{
                for(JSONObject temp : list){
                    ...
                    processInfo = monthWorkApplyDao.getActTaskById(applyId);
                    if(!"调度员发起审批".equals(processInfo.getString("actName"))){
                        flag = false;
                        break;
                    }
                }
        ...
        if(flag){
            resultJson =  ResultUtil.result(true,"允许编辑！",processInfo);
        }else{
            resultJson =  ResultUtil.result(false,"流程已审批，不允许编辑！",processInfo);
        }
      return resultJson;
    }
}
