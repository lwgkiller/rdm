...
import com.redxun.rdmZhgl.core.dao.GzxmSubjectInfoDao;
 ...
 */
@Service
public class GzxmSubjectInfoService {
    private static final Logger logger = LoggerFactory.getLogger(GzxmSubjectInfoService.class);
    @Resource
    GzxmSubjectInfoDao gzxmSubjectInfoDao;
    @Resource
    ...
    /**
     * 管理员可以看到所有数据，课题填报人，可以看到可填报项目，其他人看不到
     * */
    public JsonPageResult<?> query(HttpServletRequest request) {
        ...
        try {
            ...
            list = gzxmSubjectInfoDao.query(params);
            ...
            totalList = gzxmSubjectInfoDao.query(params);
            convertDate(list);
            // 返回结果
            result.setData(list);
    ...
    public JSONObject add(HttpServletRequest request) {
        ...
        try {
            ...
            gzxmSubjectInfoDao.addObject(objBody);
        } catch (Exception e) {
    ...
    public JSONObject update(HttpServletRequest request) {
        ...
        try {
            ...
            objBody.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
            objBody.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
            gzxmSubjectInfoDao.updateObject(objBody);
        ...
        try {
            ...
            for(int i=0;i<changeGridDataJson.size();i++) {
                ...
                if("added".equals(state)|| StringUtils.isBlank(id)) {
                    ...
                    //根据负责人id 获取对应部门
                    gzxmSubjectInfoDao.addItem(oneObject);
                } else if ("modified".equals(state)) {
                    oneObject.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
                    oneObject.put("UPDATE_TIME_", new Date());
                    gzxmSubjectInfoDao.updateItem(oneObject);
                }else if ("removed".equals(state)) {
                    // 删除
                    gzxmSubjectInfoDao.delItem(oneObject.getString("id"));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public JSONObject remove(HttpServletRequest request) {
        ...
        try {
            ...
            gzxmSubjectInfoDao.batchDelete(params);
            gzxmSubjectInfoDao.batchDeleteItems(params);
        } catch (Exception e) {
    ...
    public JSONObject getObjectById(String id){
        JSONObject jsonObject =  gzxmSubjectInfoDao.getObjectById(id);
        return jsonObject;
    }
    public List<JSONObject> getItemList(HttpServletRequest request) {
        ...
        try {
            ...
            resultArray = gzxmSubjectInfoDao.getItemList(paramJson);
    ...
    public List<JSONObject> getGzxmList(HttpServletRequest request) {
        ...
        try {
            ...
            if(StringUtil.isNotEmpty(yearMonth)){
                JSONObject paramJson = new JSONObject();
                resultArray = gzxmSubjectInfoDao.getGzxmList(paramJson);
            }
    ...
    public ResponseEntity<byte[]> importTemplateDownload() {
        try {
            String fileName = "国重项目课题任务导入模板.xlsx";
            // 创建文件实例
            File file = new File(
    ...
    public void importGtzz(JSONObject result, HttpServletRequest request) {
        try {
            ...
            //保存主表数据
            String subjectName = sheet.getRow(0).getCell(0).getStringCellValue();
            List<JSONObject> subjectList = gzxmSubjectInfoDao.getSubjectInfoByName(subjectName);
            String mainId = "";
            if(subjectList!=null){
                if(subjectList.size()==0){
                    result.put("message", "未找到课题信息！");
                    return;
                }else if(subjectList.size()>1){
                    result.put("message", "匹配到多条课题！");
                    return;
                }else{
                    mainId = subjectList.get(0).getString("id");
                }
            }
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                gzxmSubjectInfoDao.addItem(itemList.get(i));
            }
    ...
    public void exportSubjectTask(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 传入条件(不包括分页)
        params = CommonFuns.getSearchParam(params, request, false);
        String subjectId = CommonFuns.nullToString(params.get("mainId"));
        JSONObject projectObj = gzxmSubjectInfoDao.getObjectById(subjectId);
        String subjectName = projectObj.getString("subjectName");
        String nowDate = DateFormatUtil.getNowByString("yyyy-MM-dd");
        String excelName = nowDate + subjectName;
        ...
        HSSFWorkbook wbObj = ExcelUtils.exportMonthWorkExcel(list, fieldNames, fieldCodes, subjectName);
        // 输出
