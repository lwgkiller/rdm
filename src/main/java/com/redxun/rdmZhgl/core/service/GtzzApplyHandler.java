 ...
 * */
@Service
public class GtzzApplyHandler implements ProcessStartPreHandler, TaskPreHandler {
    private Logger logger = LoggerFactory.getLogger(GtzzApplyHandler.class);
    @Resource
    private GtzzApplyService gtzzApplyService;
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            gtzzApplyService.add(applyObj);
        } else {
            gtzzApplyService.update(applyObj);
        }
        return applyObj;
    }
}
