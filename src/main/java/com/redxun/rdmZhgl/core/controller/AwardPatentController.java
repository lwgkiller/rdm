...
import com.redxun.rdmZhgl.core.service.AwardPatentService;
 ...
 */
@RestController
@RequestMapping("/zhgl/core/awardPatent/")
public class AwardPatentController {

    private Logger logger = LogManager.getLogger(AwardPatentController.class);

    @Resource
    private AwardPatentService awardPatentService;
    ...
    public ModelAndView listPage(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String jspPath = "rdmZhgl/core/awardPatentEdit.jsp";
    ...
    @RequestMapping("getApeList")
    @ResponseBody
    public JsonPageResult<?> getApeList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        return awardPatentService.getApeList(request, response, true);
    }
    ...
    public ModelAndView edit(HttpServletRequest request, HttpServletResponse response) {
        ...
        String jspPath = "rdmZhgl/core/apeManagement.jsp";
        ModelAndView mv = new ModelAndView(jspPath);
        ...
        if (StringUtils.isNotBlank(awardId)) {
            Map<String, Object> cpaInfo = awardPatentService.queryAgpById(awardId);
    ...
    public JSONObject saveAgp(HttpServletRequest request, HttpServletResponse response) {
        ...
        awardPatentService.saveAgp(result, request);
        return result;
    }
    ...
    public JsonResult removeLbj(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            ...
            if (!ids.isEmpty()){
                awardPatentDao.deleteAgp(param);
    ...
    public JSONObject importExcel(HttpServletRequest request, HttpServletResponse response) {
        JSONObject result = new JSONObject();
        awardPatentService.importMaterial(result, request);
        return result;
    }
    ...
    public ResponseEntity<byte[]> importTemplateDownload(HttpServletRequest request, HttpServletResponse response) {
        return awardPatentService.importTemplateDownload();
    }
    ...
    public  Map<String, Object> getIsLbj(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean isLbj = awardPatentService.getIsLbj(chineseName,chineseId);
        if (isLbj){
...
//        String fileId = njjdDao.selectFjIdByNjBaseId(formId);
        String fileBasePath = WebAppUtil.getProperty("apeFilePathBase");
...
//        String fileId = njjdDao.selectFjIdByNjBaseId(formId);
    ...
    public ResponseEntity<byte[]> fileDownload(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (jsjlId == null || "".equals(jsjlId)){
                jsjlId = awardPatentDao.selectAgpByFjId(fileId);
            }
    ...
    @RequestMapping("exportApeList")
    public void exportExcelApe(HttpServletRequest request, HttpServletResponse response) {
        awardPatentService.exportApeList(request, response);
    }
