...
@Controller
@RequestMapping("/zhgl/core/cxypojfile")
public class CxyProjectFileController {
    private Logger logger = LoggerFactory.getLogger(CxyProjectFileController.class);
    ...
    public ModelAndView openUploadWindow(HttpServletRequest request, HttpServletResponse response) throws Exception {
        String jspPath = "rdmZhgl/core/cxyProjectFileUpload.jsp";
    ...
    public Map<String, Object> upload(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            cxyProjectFileService.saveCxyProjectUploadFiles(request);
    ...
    public List<JSONObject> dataList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        List<String> cxyProjectIds = new ArrayList<>();
        cxyProjectIds.add(cxyProjectId);
        params.put("cxyProjectIds", cxyProjectIds);
        List<JSONObject> fileInfos = cxyProjectFileService.queryCxyProjectFileList(params);
        // 格式化时间
    ...
    public void delete(HttpServletRequest request, HttpServletResponse response, @RequestBody String postBody) {
        ...
        String cxyProjectId = postBodyObj.getString("cxyProjectId");
        cxyProjectFileService.deleteOneFile(id, fileName, cxyProjectId);
    }
    ...
    public ResponseEntity<byte[]> fileDownload(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String fileBasePath = ConstantUtil.PREVIEW.equalsIgnoreCase(action)
                    ? WebAppUtil.getProperty("cxyProjectFilePathBase_preview") : WebAppUtil.getProperty("cxyProjectFilePathBase");
            if (StringUtils.isBlank(fileBasePath)) {
                logger.error("操作失败，找不到存储路径");
                return null;
            }
            String suffix = CommonFuns.toGetFileSuffix(fileName);
            String fullFilePath = fileBasePath + File.separator + cxyProjectId + File.separator + fileId + "." + suffix;
            // 创建文件实例
    ...
    public void officePreview(HttpServletRequest request, HttpServletResponse response) {
        ...
        String fileBasePath = WebAppUtil.getProperty("cxyProjectFilePathBase_preview");
        if (StringUtils.isBlank(fileBasePath)) {
            logger.error("操作失败，找不到存储路径");
            return;
        }
        String suffix = CommonFuns.toGetFileSuffix(fileName);
        String originalFilePath = fileBasePath + File.separator + cxyProjectId + File.separator + fileId + "." + suffix;
        String convertPdfDir = WebAppUtil.getProperty("convertPdfDir");
        String convertPdfPath = fileBasePath + File.separator + cxyProjectId + File.separator + convertPdfDir + File.separator+ fileId + ".pdf";
