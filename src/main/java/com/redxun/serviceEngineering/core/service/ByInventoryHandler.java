...
@Service
public class ByInventoryHandler
    implements ProcessStartPreHandler, ProcessStartAfterHandler, TaskPreHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(ByInventoryHandler.class);
    @Autowired
    ...
    public void processStartPreHandle(ProcessStartCmd processStartCmd) {
        String id = createOrUpdateByInventoryByFormData(processStartCmd);
    ...
    public String processStartAfterHandle(ProcessConfig processConfig, ProcessStartCmd processStartCmd,
        ...
        param.put("id", id);
        String processNum = toGetByInventoryNumber();
        param.put("processNum", processNum);
        byInventoryDao.updateByInventoryNumber(param);
        return id;
    }
    ...
    public void taskPreHandle(IExecutionCmd iExecutionCmd, Task task, String busKey) {
        ...
        createOrUpdateByInventoryByFormData(processNextCmd);
    }
    ...
    public void endHandle(BpmInst bpmInst) {
        if ("SUCCESS_END".equalsIgnoreCase(bpmInst.getStatus())) {
            ...
            //暂时保留自动生成excel
            /*JSONObject oneForm = byInventoryService.getByInventoryDetail(id);
            List<JSONObject> detailList = byInventoryService.queryDetail(id);
            int index = 1;
...
//            }
    ...
    // 驳回场景cmd中没有表单数据
    private String createOrUpdateByInventoryByFormData(AbstractExecutionCmd cmd) {
    ...
    public String toGetByInventoryNumber() {
        ...
        if (thisNumber < 10) {
            num = "BYJQD" + newDate + "-00" + thisNumber;
        } else if (thisNumber < 100 && thisNumber >= 10) {
            num = "BYJQD" + newDate  +"-0"+ thisNumber;
        }else if (thisNumber < 1000 && thisNumber >= 100) {
            num = "BYJQD" + newDate  +"-"+ thisNumber;
        }
        return num;
    }
