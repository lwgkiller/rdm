...
@Service
public class ManualFileDownloadApplyManager {
    private static final Logger logger = LoggerFactory.getLogger(ManualFileDownloadApplyManager.class);
    @Autowired
    ...
    public JsonPageResult<?> queryApplyList(HttpServletRequest request, boolean doPage) {
        ...
        rdmZhglUtil.addOrder(request, params, "service_engineering_maintenance_downloadApply.CREATE_BY_", "desc");
        // 增加分页条件
        if (doPage) {
        ...
        List<JSONObject> applyList = manualFileDownloadApplyDao.queryApplyList(params);
        ...
        int countZlgjDataList = manualFileDownloadApplyDao.countApplyList(params);
    ...
    public JSONObject queryApplyDetail(String id) {
        ...
        params.put("id", id);
        JSONObject obj = manualFileDownloadApplyDao.queryApplyDetail(params);
        if (obj == null) {
            return result;
        }
        return obj;
    ...
    public void createApply(JSONObject formData) {
        ...
        // 基本信息
        manualFileDownloadApplyDao.insertApply(formData);
        // 成员信息
    ...
    public void updateApply(JSONObject formData) {
        ...
        // 基本信息
        manualFileDownloadApplyDao.updateApply(formData);
        // 成员信息
        demandProcess(formData.getString("id"), formData.getJSONArray("changeDemandGrid"));
    }
    ...
    private void demandProcess(String applyId, JSONArray demandArr) {
        ...
        for (int index = 0; index < demandArr.size(); index++) {
            ...
            if ("added".equals(state) || StringUtils.isBlank(id)) {
                ...
                manualFileDownloadApplyDao.insertDemand(oneObject);
        ...
        if (!param.getJSONArray("ids").isEmpty()) {
            manualFileDownloadApplyDao.deleteDemand(param);
        }
    }
    ...
    public JsonResult delApplys(String[] applyIdArr) {
        ...
        manualFileDownloadApplyDao.deleteDemand(param);
        // 查询附件信息
        List<JSONObject> files = manualFileDownloadApplyDao.getApplyFiles(param);
        if (files.size() > 0) {
            // 删除数据库中的附件信息
        ...
        manualFileDownloadApplyDao.deleteApply(param);
        return result;
    }
    ...
    public void saveUploadFiles(HttpServletRequest request) {
        ...
        if (StringUtils.isBlank(filePathBase)) {
            logger.error("can't find manualFileDownloadApply");
            return;
        }
        try {
            ...
            manualFileDownloadApplyDao.insertFile(fileInfo);
        } catch (Exception e) {
    ...
    /**
     * 从磁盘中删除某个表单的文件夹（包括预览文件夹），前提是已经将文件删除完
     *
     * @param formId
     */
    ...
    public List<JSONObject> queryFileList(JSONObject params) {
        List<JSONObject> files = manualFileDownloadApplyDao.getApplyFiles(params);
        for (JSONObject oneFile : files) {
