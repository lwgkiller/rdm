...
@Service
public class PartsAtlasFileDownloadApplyManager {
    private static final Logger logger = LoggerFactory.getLogger(PartsAtlasFileDownloadApplyManager.class);
    @Autowired
    ...
    public JsonPageResult<?> queryApplyList(HttpServletRequest request, boolean doPage) {
        ...
        rdmZhglUtil.addOrder(request, params, "service_engineering_parts_apply_list.CREATE_TIME_", "desc");
        // 增加分页条件
        if (doPage) {
        ...
        List<JSONObject> applyList = partsAtlasFileDownloadApplyDao.queryApplyList(params);
        ...
        int countZlgjDataList = partsAtlasFileDownloadApplyDao.countApplyList(params);
    ...
    public JSONObject queryApplyDetail(String id) {
        ...
        params.put("id", id);
        JSONObject obj = partsAtlasFileDownloadApplyDao.queryApplyDetail(params);
        if (obj == null) {
            return result;
        }
        return obj;
    ...
    public void createApply(JSONObject formData) {
        ...
        // 基本信息
        partsAtlasFileDownloadApplyDao.insertApply(formData);
        // 成员信息
    ...
    public void updateApply(JSONObject formData) {
        ...
        // 基本信息
        partsAtlasFileDownloadApplyDao.updateApply(formData);
        // 成员信息
        demandProcess(formData.getString("id"), formData.getJSONArray("changeDemandGrid"));
    }
    ...
    private void demandProcess(String applyId, JSONArray demandArr) {
        ...
        for (int index = 0; index < demandArr.size(); index++) {
            ...
            if ("added".equals(state) || StringUtils.isBlank(id)) {
                ...
                partsAtlasFileDownloadApplyDao.insertDemand(oneObject);
                ...
                partsAtlasFileDownloadApplyDao.updateDemand(oneObject);

            }
        }
        if (!param.getJSONArray("ids").isEmpty()) {
            partsAtlasFileDownloadApplyDao.deleteDemand(param);
        }
    }
    ...
    public JsonResult delApplys(String[] applyIdArr) {
        ...
        partsAtlasFileDownloadApplyDao.deleteApply(param);
        return result;
    }


    public List<JSONObject> queryDemandList(JSONObject params) {
        List<JSONObject> demandList = partsAtlasFileDownloadApplyDao.queryDemandList(params);
        return demandList;
    }
