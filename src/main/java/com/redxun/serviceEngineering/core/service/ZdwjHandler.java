...
public class ZdwjHandler
    ...
    public String processStartAfterHandle(ProcessConfig processConfig, ProcessStartCmd processStartCmd,
        ...
        String applyNumber =
            "zdwj-" + XcmgProjectUtil.getNowLocalDateStr("yyyyMMddHHmmssSSS") + (int)(Math.random() * 100);
        JSONObject param = new JSONObject();
        param.put("applyNumber", applyNumber);
        param.put("id", applyId);

        zdwjDao.updateZdwjNumber(param);
        return applyId;
    }
    ...
    private String createOrUpdateByFormData(AbstractExecutionCmd cmd) {
        ...
        if (StringUtils.isBlank(formDataJson.getString("id"))) {
            zdwjManager.createZdwj(formDataJson);
        } else {
            zdwjManager.updateZdwj(formDataJson);
        }
