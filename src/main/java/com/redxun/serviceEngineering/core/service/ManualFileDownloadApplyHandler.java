...
@Service
public class ManualFileDownloadApplyHandler
    implements ProcessStartPreHandler, ProcessStartAfterHandler, TaskPreHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(ManualFileDownloadApplyHandler.class);
    @Autowired
    ...
    public String processStartAfterHandle(ProcessConfig processConfig, ProcessStartCmd processStartCmd,
        ...
        String applyNumber =
            "cbscxz-" + XcmgProjectUtil.getNowLocalDateStr("yyyyMMddHHmmssSSS") + (int)(Math.random() * 100);
        JSONObject param = new JSONObject();
        param.put("applyNumber", applyNumber);
        param.put("id", applyId);

        manualFileDownloadApplyDao.updateApplyNumber(param);
        return applyId;
    }
    ...
    public void endHandle(BpmInst bpmInst) {
        if ("SUCCESS_END".equalsIgnoreCase(bpmInst.getStatus())) {
            ...
            noticeObj.put("content", "您申请的操保手册下载流程已审批完成，请前往申请表单下载，申请编号：" + formDataJson.getString("applyNumber"));
    ...
    private String createOrUpdateByFormData(AbstractExecutionCmd cmd) {
        ...
        if (StringUtils.isBlank(formDataJson.getString("id"))) {
            manualFileDownloadApplyManager.createApply(formDataJson);
        } else {
            manualFileDownloadApplyManager.updateApply(formDataJson);
        }
