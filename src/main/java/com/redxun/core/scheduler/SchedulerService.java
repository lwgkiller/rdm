...
public class SchedulerService {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public JsonResult addJob(String jobName, String className,
			Map<String, Object> parameterMap, String description)
			throws SchedulerException {
		if (scheduler == null) return new JsonResult(false, "scheduler 没有配置!");
	 ...
	 */
	@SuppressWarnings({ "unchecked" })
	public JsonResult addJob(String jobName, Class<?> cls,
			Map<String, Object> parameterMap, String description) {
		if (scheduler == null) 	return new JsonResult(false, "scheduler 没有配置!");

		JsonResult resultMsg = null;
		try {
			...
			JobDetail jobDetail = jb.build();
			scheduler.addJob(jobDetail, true);
			resultMsg = new JsonResult(true, "添加任务成功!");
			return resultMsg;
		} catch (Exception e) {
			resultMsg = new JsonResult(false, e.getMessage());
			return resultMsg;
		}
	}
	 ...
	 * @throws SchedulerException
	 */
	public JobInfo getJobByName(String name) throws SchedulerException{
		JobKey jobKey=new JobKey(name,schedGroup);
		JobDetail detail= scheduler.getJobDetail(jobKey);
		JobDataMap jobMap= detail.getJobDataMap();
		
		JobInfo jobInfo=new JobInfo();
		jobInfo.setName(name);
		jobInfo.setClassName(detail.getJobClass().getName());
		jobInfo.setDescription(detail.getDescription());
		
		List<ParameterObj> paramList=new ArrayList<ParameterObj>();
		
		for(Map.Entry<String, Object> entry:jobMap.entrySet()){
			String key=entry.getKey();
			Object val=entry.getValue();
			String type=val==null? ParameterObj.TYPE_STRING :val.getClass().getSimpleName();
			String sVal=val==null ?"":val.toString();
			paramList.add(new ParameterObj(type, key, sVal));
		}
		jobInfo.setParamList(paramList);
	...
	public List<JobDetail> getJobList() throws SchedulerException {
		if (scheduler == null) return new ArrayList<JobDetail>();
	...
	public List<Trigger> getTriggersByJob(String jobName)
			throws SchedulerException {
		if (scheduler == null) return new ArrayList<Trigger>();
		JobKey key = new JobKey(jobName, schedGroup);
	...
	public HashMap<String, Trigger.TriggerState> getTriggerStatus(
		...
		if (scheduler == null) 	return new HashMap<String, Trigger.TriggerState>();
...
//		if(trig instanceof CronTriggerImpl){
//			CronTriggerImpl cronTriggerImpl=(CronTriggerImpl)trig;
//			cronTriggerImpl.setMisfireInstruction(CronTrigger.MISFIRE_INSTRUCTION_DO_NOTHING);
//		}
//		else if(trig instanceof CalendarIntervalTriggerImpl){
//			CalendarIntervalTriggerImpl intervalTriggerImpl=(CalendarIntervalTriggerImpl)trig;
//			intervalTriggerImpl.setMisfireInstruction(CronTrigger.MISFIRE_INSTRUCTION_DO_NOTHING);
//		}
	...
	private void setJobMap(String json, JobBuilder job) {
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (type.equals(ParameterObj.TYPE_INT)) {
				Integer val = StringUtils.isEmpty(value) ? 0 : Integer.parseInt(value);
				job.usingJobData(name, val);
			} else if (type.equals(ParameterObj.TYPE_LONG)) {
				Long val = StringUtils.isEmpty(value) ? 0L : Long.parseLong(value);
				job.usingJobData(name, val);
			} else if (type.equals(ParameterObj.TYPE_FLOAT)) {
				Float val = StringUtils.isEmpty(value) ? 0.0f : Float.parseFloat(value);
				job.usingJobData(name, val);
			} else if (type.equals(ParameterObj.TYPE_BOOLEAN)) {
				Boolean val = StringUtils.isEmpty(value) ? false : Boolean.parseBoolean(value);
				job.usingJobData(name, val);
			} else {
