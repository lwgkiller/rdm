...
public class ActivityCompletedListener implements EventHandler {
    ...
    private void updBpmRuPath(String actInstId, String nodeId, IExecutionCmd cmd) {
        ...
        if (cmd.getTransientVars().containsKey("signUsers")) {
            ...
            //一般情况下会签通过后设置执行人为会签人
            ruPath.setAssignee(ruPath.getUserIds());
            //如果是从普通节点驳回会签，当前ruPath并不是真正的会签节点，因此还要找到历史上的真正会签节点，做人员更新
            //目前实测结果是会签节点无论流程驳回几回，其路径信息只记录1份
            BpmRuPath ruPathRel = bpmRuPathManager.getFarestPath(actInstId, nodeId);
            ruPathRel.setUserIds((String) cmd.getTransientVars().get("signUsers"));
            ruPathRel.setAssignee(ruPath.getUserIds());
            bpmRuPathManager.update(ruPathRel);
        }
        //更新其数据
