...
public class TaskCreateListener  implements EventHandler {
	...
	private boolean handFromDb(TaskEntity taskEntity,BpmNodeJump nodeJump,boolean allowEmpty){
		...
		vars.put("nodeName", taskEntity.getName());
		Collection<TaskExecutor> idInfoList = bpmIdentityCalService.calNodeUsersOrGroups(taskEntity.getProcessDefinitionId(), taskEntity.getTaskDefinitionKey(),vars);
