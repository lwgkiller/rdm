...
public class ActRepService {
	...
	public Collection<ActivityNode> getUserTasks(String actDefId){
		...
		Collections.sort(nodes, new Comparator<ActivityNode>() {
			@Override
...
//		CacheUtil.addCache(ACT_PROCESS_DEF + actDefId,processDef);
	 ...
	 * @return
	 */
	public ActNodeDef getFirstUserTask(String actDefId){
		ActNodeDef startEvent=getStartNode(actDefId);
		if(startEvent==null) return null;
		List<ActNodeDef> defs= startEvent.getOutcomeNodes();
