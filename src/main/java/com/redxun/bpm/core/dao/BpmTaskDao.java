...
public class BpmTaskDao extends BaseMybatisDao<BpmTask> {
    ...
    public List<BpmTask> getSolTasks(QueryFilter filter) {
        return getPageBySqlKey("getSolTasks", filter);
    }

    public List<BpmTask> getMySolTasks(QueryFilter filter) {
        return getPageBySqlKey("getUserSolTasks", filter);
    }
    ...
    public JsonResult updateAssignee(String taskId, String userId) {
        ...
        try {
            if (StringUtils.isNotBlank(taskId) && StringUtils.isNotBlank(userId)) {
                ...
                this.getBySqlKey("updateAssignee", params);
            }
        } catch (Exception e) {
