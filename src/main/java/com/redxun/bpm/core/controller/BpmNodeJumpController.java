...
public class BpmNodeJumpController extends BaseListController {
    ...
    @ResponseBody
    public JsonPageResult<BpmNodeJump> listForInst(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        ...
        String doPage = RequestUtil.getString(request, "doPage", "");
        String more = request.getParameter("more");
        Page page = QueryFilterBuilder.createPage(request);
        if (StringUtils.isNotBlank(doPage) && "false".equalsIgnoreCase(doPage)) {
        ...
        if (StringUtils.isNotEmpty(taskId)) {
            ...
            getByLiveInstId(list, bpmInst.getLiveInstId());
    ...
    private void getByLiveInstId(List<BpmNodeJump> list, String liveInstId) {
        if (StringUtil.isNotEmpty(liveInstId)) {
            BpmInst inst = bpmInstManager.get(liveInstId);
            BpmNodeJump jump = new BpmNodeJump();
            jump.setCheckStatus("复活");
            jump.setJumpType("LIVE");
            jump.setCreateTime(inst.getUpdateTime());
            jump.setNodeName("流程复活");
            jump.setHandlerId(inst.getUpdateBy());
            list.add(jump);
            list.addAll(bpmNodeJumpManager.getByActInstId(inst.getActInstId()));
    ...
    @ResponseBody
    public JsonPageResult<BpmNodeJump> listForInstMobile(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String actInstId = request.getParameter("actInstId");
        String taskId = request.getParameter("taskId");
        String instId = request.getParameter("instId");
    ...
    @ResponseBody
    public JsonPageResult<BpmNodeJump> listForInstShowName(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String actInstId = request.getParameter("actInstId");
        Page page = QueryFilterBuilder.createPage(request);
    ...
    @ResponseBody
    public JsonPageResult<BpmNodeJump> actLlistData(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        String actInstId = request.getParameter("actInstId");
        Page page = QueryFilterBuilder.createPage(request);
        List<BpmNodeJump> list = bpmNodeJumpManager.getByActInstId(actInstId, page);
