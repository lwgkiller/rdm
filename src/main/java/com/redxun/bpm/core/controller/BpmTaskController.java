...
import com.redxun.bpm.core.entity.config.*;
import com.redxun.bpm.core.identity.service.BpmIdentityCalService;
import com.redxun.bpm.core.manager.*;
...
public class BpmTaskController extends TenantListController {
    ...
    public JsonResult changePath(HttpServletRequest request, HttpServletResponse response) throws Exception {
            ...
            try {
                ...
                if (formModels != null && formModels.size() > 0) {
                    ...
                    for (FormModel fm : formModels) {
                        //@lwgkiller:BPM干预，此处必须屏蔽掉自开发表单，因为默认会去取自开发表单的东西，会
                        //在bpmTaskManager.doNext(processNextCmd)的saveFormData中报错
                        if(!fm.getType().equalsIgnoreCase("SEL-DEV")){
        ...
        //@lwgkiller:BPM干预，这个死循环不明所以，去掉
//        if (flag) {
//            changePath(request, response);
//        }
        return result;
    }

    @RequestMapping("mySol/{solKey}")
    public ModelAndView mySolTasks(@PathVariable("solKey") String solKey, HttpServletRequest request) {
        ModelAndView mv = new ModelAndView("/bpm/core/bpmTaskMySol.jsp");
        mv.addObject("solKey", solKey);
        return mv;
    }

    @RequestMapping("sol/{solKey}")
    public ModelAndView solTasks(@PathVariable("solKey") String solKey, HttpServletRequest request) {
        ModelAndView mv = new ModelAndView("/bpm/core/bpmTaskSol.jsp");
        mv.addObject("solKey", solKey);
        return mv;
    }
     ...
     * @throws Exception
     */
    @RequestMapping("getMySolTasks/{solKey}")
    @ResponseBody
    public JsonPageResult<BpmTask> getMySolTasks(@PathVariable("solKey") String solKey, HttpServletRequest request,
        HttpServletResponse response) throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
        List<BpmTask> bpmTasks = bpmTaskManager.getMySolTasks(solKey, queryFilter);
        return new JsonPageResult<>(true, bpmTasks, queryFilter.getPage().getTotalItems(), "success");
    }
     ...
     * @throws Exception
     */
    @RequestMapping("getSolTasks/{solKey}")
    @ResponseBody
    public JsonPageResult<BpmTask> getSolTasks(@PathVariable("solKey") String solKey, HttpServletRequest request,
        HttpServletResponse response) throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
        List<BpmTask> bpmTasks = bpmTaskManager.getSolTasks(solKey, queryFilter);
        return new JsonPageResult<>(true, bpmTasks, queryFilter.getPage().getTotalItems(), "success");
    }
    ...
    @ResponseBody
    public JsonPageResult<BpmTask> getAllTasks(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
    ...
    public ModelAndView calUsers(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.isNotEmpty(taskId)) {
            ...
            // 确定具体的执行人
            if (bpmTask.getTaskDefKey().equals(nodeId) && StringUtils.isNotEmpty(bpmTask.getAssignee())) {
                actNodeDef = actRepService.getActNodeDef(actDefId, nodeId);
                // 若为会签，则显示会签的人员
                if (StringUtils.isNotEmpty(actNodeDef.getMultiInstance())) {
                    List<TaskExecutor> uIds = (List<TaskExecutor>)taskService.getVariable(bpmTask.getId(),
                        TaskVarType.SIGN_USER_IDS_.getKey() + bpmTask.getTaskDefKey());
                    for (TaskExecutor taskExecutor : uIds) {
                        taskExecutor.setName(userService.getByUserId(taskExecutor.getId()).getFullname());
                    }
                    identityInfos.addAll(uIds);
                } else {// 非会签，则显示当前任务的人员
                ...
                Collection<TaskExecutor> bpmIdenties =
                    bpmIdentityCalService.calNodeUsersOrGroups(bpmTask.getProcDefId(), nodeId, variables);
            ...
            if (BpmInst.STATUS_RUNNING.equals(bpmInst.getStatus())) {
                ...
                Collection<TaskExecutor> bpmIdenties =
                    bpmIdentityCalService.calNodeUsersOrGroups(bpmInst.getActDefId(), nodeId, variables);
                identityInfos.addAll(bpmIdenties);
            } else {// 流程已经结束
                Set<String> nodeUsers = new HashSet<String>();
                // 查找审批环节中的处理人
                List<BpmNodeJump> bpmNodeJumps =
                    bpmNodeJumpManager.getByActInstIdNodeId(bpmInst.getActInstId(), nodeId);
            ...
            // 计算人员
            Collection<TaskExecutor> bpmIdenties =
                bpmIdentityCalService.calNodeUsersOrGroups(bpmDef.getActDefId(), nodeId, variables);
            identityInfos.addAll(bpmIdenties);
        }
        if (actNodeDef == null) {
            actNodeDef = actRepService.getActNodeDef(actDefId, nodeId);
    ...
    public ModelAndView approve(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 获得任务的下一步的人员映射列表
        // List<DestNodeUsers> destNodeUserList = bpmTaskManager.getDestnationUsers(taskId);
        UserTaskConfig taskConfig =
            bpmNodeSetManager.getTaskConfig(bpmInst.getSolId(), bpmTask.getProcDefId(), bpmTask.getTaskDefKey());
    ...
    public JsonResult doNext(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            // 增加驳回场景的会签数据删除
            if (processNextCmd.getJumpType().equalsIgnoreCase(TaskOptionType.BACK.name())
                || processNextCmd.getJumpType().equalsIgnoreCase(TaskOptionType.BACK_TO_STARTOR.name())
                || processNextCmd.getJumpType().equalsIgnoreCase(TaskOptionType.BACK_SPEC.name())) {
                //@lwgkiller:如果任务脚本抛出异常，此处bpmInst为null，继续执行则报空指针，影响脚本实际错误信息的显示，加上判空
                if(bpmInst!=null){
                    bpmSignDataManager.delByActInstIdNodeId(bpmInst.getActInstId(), processNextCmd.getHandleNodeId());
                }
            }
            // 在处理过程中，是否有错误的消息抛出
            if (handleMessage.getErrorMsges().size() > 0) {
    ...
    public ModelAndView toStart(HttpServletRequest request) throws Exception {
        ...
        return getPathView(request).addObject("bpmTask", bpmTask).addObject("bpmInst", bpmInst)
            .addObject("taskConfig", taskConfig).addObject("isShowDiscardBtn", isShowDiscardBtn)
            .addObject("formModels", formModels).addObject("canReject", backNodes.size() > 0)
            .addObject("canAddSign", canAddSign).addObject("allowTask", result)
    ...
    private ModelAndView handExternalUrl(UserTaskConfig taskConfig, BpmTask bpmTask, BpmInst bpmInst) throws Exception {
        if (StringUtils.isEmpty(taskConfig.getExtFormUrl()))
            return null;
    ...
    private Boolean canDiscard(BpmInst bpmInst) {
        ...
        ActProcessDef processDef = actRepService.getProcessDef(bpmInst.getActDefId());
        Collection<ActNodeDef> outNodes = processDef.getActNodeDefs();
        List<BpmTask> bpmTasks = bpmTaskManager.getByActInstId(bpmInst.getActInstId());
        String curUserId = ContextUtil.getCurrentUserId();
        for (BpmTask t : bpmTasks) {
            for (ActNodeDef node : outNodes) {
                // 当前节点处于经办人环节，并且处理人为经办人，即允许作废单据
                // todo:@lwgkiller 加入管理员
                if (node.getNodeId().equals(t.getTaskDefKey())
                    && (curUserId.equals(t.getAssignee()) || curUserId.equalsIgnoreCase("1"))) {
    ...
    public ModelAndView transfer(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        return getPathView(request).addObject("taskIds", taskIdSb.toString()).addObject("taskNames",
            taskNameSb.toString());
    }
    ...
    @ResponseBody
    public JsonPageResult<BpmTask> getAllTasksForSaasAdmin(HttpServletRequest request, HttpServletResponse response)
        throws Exception {
        QueryFilter queryFilter = QueryFilterBuilder.createQueryFilter(request);
        String treeId = RequestUtil.getString(request, "treeId");
        if (StringUtil.isNotEmpty(treeId)) {
    ...
    public ModelAndView myListNode(HttpServletRequest request, @PathVariable(value = "nodeId") String nodeId)
        throws Exception {
    ...
    public ModelAndView approval(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 获得任务节点的配置
        UserTaskConfig taskConfig =
            bpmNodeSetManager.getTaskConfig(bpmSolution.getSolId(), bpmTask.getProcDefId(), bpmTask.getTaskDefKey());
        ...
        ProcessConfig processConfig =
            bpmNodeSetManager.getProcessConfig(bpmSolution.getSolId(), bpmTask.getProcDefId());
        ...
        for (DestNodeUsers destNodeUsers : destNodeUserList) {
            if (destNodeUsers.getTaskNodeUser() == null
                || StringUtils.isEmpty(destNodeUsers.getTaskNodeUser().getUserIds())) {
                continue;
            }
        ...
        BpmNodeSet bpmNodeSet =
            bpmNodeSetManager.getBySolIdActDefIdNodeId(bpmTask.getSolId(), bpmTask.getProcDefId(), nodeId);
        ...
        return getPathView(request).addObject("bpmTask", bpmTask).addObject("fileIds", fileIds.toString())
            .addObject("fileNames", fileNames.toString()).addObject("fileBytes", fileBytes)
            .addObject("bpmInst", bpmInst).addObject("bpmDef", bpmDef).addObject("checkButtonsJson", checkButtonsJson)
            .addObject("bpmSolution", bpmSolution).addObject("taskConfig", taskConfig)
            .addObject("isShowDiscardBtn", isShowDiscardBtn).addObject("processConfig", processConfig)
    ...
    public ModelAndView reject(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String taskId = RequestUtil.getString(request, "taskId");
        BpmTask bpmTask = bpmTaskManager.get(taskId);
        BpmNodeSet bpmNodeSet = bpmNodeSetManager.getBySolIdActDefIdNodeId(bpmTask.getSolId(), bpmTask.getProcDefId(),
            bpmTask.getTaskDefKey());
        JSONObject json =
            JSONObject.parseObject("{'BACK':'驳回(上一节点)','BACK_TO_STARTOR':'驳回(发起人)','BACK_SPEC':'驳回指定节点'}");
        String jumpTypes = "";
        ...
        if (bpmNodeSet != null && StringUtil.isEmpty(jumpTypes)) {
            bpmNodeSet = bpmNodeSetManager.getBySolIdActDefIdNodeId(bpmTask.getSolId(), bpmTask.getProcDefId(),
        ...
        for (int i = 0; i < jumpType.length; i++) {
            JSONObject obj = new JSONObject();
            // 去掉驳回发起人，防止误操作
            // if (!"BACK_TO_STARTOR".equalsIgnoreCase(jumpType[i])) {
    ...
    // 转办任务
    @RequestMapping("transferTaskSelf")
    @ResponseBody
    public JsonResult transferTaskSelf(HttpServletRequest request, HttpServletResponse response) {
        String taskId = RequestUtil.getString(request, "taskId");
        String userId = RequestUtil.getString(request, "userId");

        JsonResult result = bpmTaskDao.updateAssignee(taskId, userId);
        return result;
