...
public class BpmRuPathController extends BaseListController{
	...
	@RequestMapping("getBackNodes/{instId}")
	@ResponseBody
	public List<BpmRuPath> getBackNodes(@PathVariable(value="instId")String instId){
		BpmInst inst = bpmInstManager.get(instId);
		Collection<ActivityNode> nodes= actRepService.getProcessNodes(inst.getActDefId());
		List<BpmRuPath> temp=new ArrayList<BpmRuPath>();
		for(ActivityNode node:nodes){
			if("userTask".equals(node.getType())) {
    			BpmRuPath path = new BpmRuPath();
    			path.setNodeId(node.getActivityId());
