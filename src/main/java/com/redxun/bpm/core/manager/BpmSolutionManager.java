...
public class BpmSolutionManager extends MybatisBaseManager<BpmSolution> {
    ...
    public void delete(String id) {
        ...
        bpmSolVarManager.delBySolId(id);
        bpmSolUsergroupManager.delBySolId(id);
        bpmSolFvManager.delBySolId(id);
        bpmSolCtlManager.delBySolId(id);
        bpmNodeSetManager.delBySolId(id);
        bpmRemindDefManager.delBySolId(id);
        super.delete(id);
    }
    ...
    /**
     * 1.获取全部的分类。 2.获取使用了的分类。 3.对数据进行处理，删除没有使用的树形数据。
     * 
     * @return
     */
