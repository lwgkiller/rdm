...
import com.redxun.bpm.listener.TaskInformListener;
...
public class BpmTaskManager extends MybatisBaseManager<BpmTask> {
    ...
    @Resource
    TaskInformListener taskInformListener;
...
//		sendMessage(toUserId,bpmTask,noticeTypes,"任务转办","transfer");
        ...
        TaskEntity msgObj=new TaskEntity();
        msgObj.setAssignee(bpmTask.getAssignee());
        msgObj.setName(bpmTask.getName());
        msgObj.setDescription(bpmTask.getDescription());
        taskInformListener.sendMsg(msgObj);
	...
	public TaskNodeUser calUserNode(String actDefId, ActNodeDef nodeDef, Map<String, Object> vars) {
		...
		// 计算taskNodeUser里的值
		// 取算该节点的人员配置，并且计算其规则
		vars.put("nodeName",nodeDef.getNodeName());
	...
	private void handJumpRule(ProcessNextCmd cmd,String curNodeId,BpmInst bpmInst){
		if(TaskOptionType.AGREE.name().equals(cmd.getJumpType()) || TaskOptionType.OVERTIME_AUTO_AGREE.name().equals(cmd.getJumpType()) && StringUtil.isEmpty(cmd.getDestNodeId())){
	...
	public Set<TaskExecutor> getTaskHandlerUsersIds(String taskId){
		...
		if(nodeDef!=null && StringUtils.isNotEmpty(nodeDef.getMultiInstance())){
			String signUserIds=(String)runtimeService.getVariable(bpmTask.getExecutionId(), TaskVarType.SIGN_USER_IDS_+bpmTask.getTaskDefKey());
			if(BeanUtil.isNotEmpty(signUserIds)) {
				String[] signIds=signUserIds.split(",");
				for (String signId : signIds) {
					IUser user = userService.getByUserId(signId);
					if(user!=null) userIds.add(new TaskExecutor(user.getUserId(), user.getFullname()));
				}
			}
		}else{
			//从执行用户获取
	 ...
	 * @return
	 */
	public List<BpmTask> getSolTasks(String solKey, QueryFilter filter){
		filter.addFieldParam("solKey",solKey);
		List<BpmTask> list= bpmTaskDao.getSolTasks(filter);
		calcExecutors(list);
		return list;
	}
	 ...
	 * @return
	 */
	public List<BpmTask> getMySolTasks(String solKey, QueryFilter filter){
		IUser user=ContextUtil.getCurrentUser();
		filter.addFieldParam("userId", user.getUserId());
		filter.addFieldParam("solKey",solKey);
		List<BpmTask> list=bpmTaskDao.getMySolTasks(filter);
		calcExecutors(list);
		return list;
	}
	...
	public void doChangeParallelUser(String actInstId,String nodeId,String[] userIds){
		String varName=TaskVarType.SIGN_USER_IDS_ + nodeId;
	...
	private List<String> getSeqUsers(String[] userIds, Integer idx, String orignUser) {
		String[] aryOriginUser=new String[] {};
		if(StringUtil.isNotEmpty(orignUser)) {
			aryOriginUser=orignUser.split(",");
		}
	...
	private Map<String,List<String>> getDiffUsers(String actInstId,String nodeId,String[] userIds){
		...
		String[] aryOrgin=new String[] {};
		if(StringUtil.isNotEmpty(orginUsers)) {
			aryOrgin=orginUsers.split(",");
		}
