...
public class BpmInstManager extends BaseManager<BpmInst> {
	...
	public void doDiscardProcessInstance(String instId) {
		...
		//针对科技项目,作废后删除已发放的积分
		String solkey = bpmInst.getSolKey();
		if(StringUtils.isNotBlank(solkey)&&solkey.contains("KJXMGL")){
			// 删除积分表
			xcmgProjectScoreDao.deleteScoreByPid(bpmInst.getBusKey());
		}
	}
...
//		bpmFormInstManager.delByBpmInstId(inst.getInstId());
	...
	public BpmInst doStartProcess(ProcessStartCmd cmd, boolean startFlow) throws Exception{
		...
		bpmInst.setIsTest(BpmSolution.FORMAL_YES.equals(bpmSolution.getFormal()) ? MBoolean.NO.name() : MBoolean.YES.name());
		...
		if (startFlow) {
			...
			if(StringUtil.isNotEmpty(cmd.getDestNodeId())) {
				actTaskService.changeStartDest(actDefId, cmd.getDestNodeId());
			}
	...
	public void doLiveProcessInstance(String actInstId, String destNodeConfig, String opinion, String opFiles) throws Exception {
		...
		JSONArray destNode = JSONArray.parseArray(destNodeConfig);
		// 目标节点
		String destNodeId = destNode.getJSONObject(0).getString("nodeId");
		...
		// 节点人员配置
		cmd.setNodeUserIds(destNodeConfig);
		cmd.setDestNodeUsers(destNodeConfig);

		cmd.setVars(vars);
		BpmInst newInst = doStartProcess(cmd);
	...
	/**
	 * 此函数专为rdm系统开发，其他场景不要使用。by liangchuanjiang 2020-1-16
	 *
	 * @param request
	 * @param reponse
	 * @return
	 * @throws Exception
	 */
	public JsonResult startProcessForRdm(HttpServletRequest request, HttpServletResponse reponse) throws Exception {
		// 加上处理的消息提示
		...
		try {
			...
			ProcessStartCmd cmd = getProcessStartCmdForRdm(request);
			// 启动流程
			bpmInst = doStartProcess(cmd);
	 ...
	 * @throws Exception
	 */
	private ProcessStartCmd getProcessStartCmdForRdm(HttpServletRequest request) throws Exception {
		ProcessStartCmd cmd = new ProcessStartCmd();
		String solId = request.getParameter("solId");
		if (StringUtils.isBlank(solId)) {
			solId = request.getAttribute("solId") == null ? "" : request.getAttribute("solId").toString();
		}
		String taskId = request.getParameter("taskId");
		String mainSolId = request.getParameter("mainSolId");
		String jsonData = request.getParameter("jsonData");
		if (StringUtils.isBlank(jsonData)) {
			jsonData = request.getAttribute("jsonData") == null ? "" : request.getAttribute("jsonData").toString();
		}
