...
public class TaskInformListener implements ApplicationListener<TaskCreateApplicationEvent>, Ordered {
    ...
    public void onApplicationEvent(TaskCreateApplicationEvent event) {
        ...
        if (BeanUtil.isEmpty(notifyType)) {
            ProcessConfig procConfig =
                bpmNodeSetManager.getProcessConfig(taskEnt.getSolId(), taskEnt.getProcessDefinitionId());
    ...
    public void sendMsg(TaskEntity taskEnt) {
        try {
            ...
            for (IUser iUser : userSet) {
                OsUser osUser = (OsUser)iUser;
                JSONObject oneUser = new JSONObject();
                String certNo = osUser.getCertNo() == null ? "" : osUser.getCertNo();
                oneUser.put("userCertNo", certNo);
                userList.add(oneUser);
            }
            JSONObject taskObj = new JSONObject();
            taskObj.put("content", taskEnt.getDescription() + "，当前任务：" + taskEnt.getName());
    ...
    private void handCopyTo(BpmInst bpmInst, TaskEntity task, String noteType) {
        ...
        if (StringUtil.isEmpty(ccUserIds)) {
            return;
        }
