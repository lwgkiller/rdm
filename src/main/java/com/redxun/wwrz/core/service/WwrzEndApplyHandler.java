 ...
 * */
@Service
public class WwrzEndApplyHandler implements ProcessStartPreHandler, TaskPreHandler, ProcessStartAfterHandler, ProcessEndHandler {
    private Logger logger = LoggerFactory.getLogger(WwrzEndApplyHandler.class);
    @Resource
    ...
    private Map<String, Object> addOrUpdateApplyInfo(AbstractExecutionCmd cmd) {
        ...
        if (applyObj.get("id") == null || StringUtils.isBlank(applyObj.get("id").toString())) {
            wwrzEndApplyService.add(applyObj);
        } else {
            wwrzEndApplyService.update(applyObj);
        }
        return applyObj;
    }
    // 流程成功结束之后的处理
    ...
    public void endHandle(BpmInst bpmInst) {
        if ("SUCCESS_END".equalsIgnoreCase(bpmInst.getStatus())) {
            ...
            JSONObject applyObj = wwrzEndApplyDao.getJsonObject(applyId);
            String mainId = applyObj.getString("mainId");
            JSONObject testApplyObj = wwrzTestApplyDao.getJsonObject(mainId);
            String instId = testApplyObj.getString("instId");
