...
@Service
@Slf4j
public class QbgzService {
    private Logger logger = LogManager.getLogger(QbgzService.class);
    @Autowired
    ...
    public JsonPageResult<?> queryQbgz(HttpServletRequest request, boolean doPage) {
...
//        addQbgzRoleParam(params, ContextUtil.getCurrentUserId(), ContextUtil.getCurrentUser().getUserNo());
        List<Map<String, Object>> qbgzList = qbgzDao.queryQbgz(params);
        ...
        int countQbgzDataList = qbgzDao.countQbgzList(params);
    ...
    public void createQbgz(JSONObject formData) {
        JSONObject currentUserDepInfo = xcmgProjectManager.isCurrentUserDepRespman();
        ...
        formData.put("qbgzId", IdUtil.getId());
        formData.put("CREATE_BY_", ContextUtil.getCurrentUserId());
        formData.put("CREATE_TIME_", new Date());
        formData.put("deptName", deptName);
        qbgzDao.createQbgz(formData);
    }

    public String toGetQbgzNumber() {
        String qbNum = null;
        ...
        JSONObject maxQbgz = qbgzDao.queryMaxQbgzNum(param);
        int existNumber = 0;
        if (maxQbgz != null) {
            existNumber = Integer.parseInt(maxQbgz.getString("qbNum").substring(6));
        }
        int thisNumber = existNumber + 1;
        if (thisNumber < 10) {
            qbNum = "QB" + DateFormatUtil.getNowByString("yyyy") + "000" + thisNumber;
        } else if (thisNumber < 100 && thisNumber >= 10) {
            qbNum = "QB" + DateFormatUtil.getNowByString("yyyy") + "00" + thisNumber;
        } else if (thisNumber < 1000 && thisNumber >= 100) {
            qbNum = "QB" + DateFormatUtil.getNowByString("yyyy") + "0" + thisNumber;
        } else if (thisNumber < 10000 && thisNumber >= 1000) {
            qbNum = "QB" + DateFormatUtil.getNowByString("yyyy") + thisNumber;
        }
        return qbNum;
    }

    public void updateQbgz(JSONObject formData) {
        formData.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
        formData.put("UPDATE_TIME_", new Date());
        qbgzDao.updateQbgz(formData);
    }

    public void saveQbgzUploadFiles(HttpServletRequest request) {
        ...
        String filePathBase = WebAppUtil.getProperty("qbgzFilePathBase");
        if (StringUtils.isBlank(filePathBase)) {
            logger.error("can't find qbgzFilePathBase");
            return;
        }
        try {
            ...
            String note = toGetParamVal(parameters.get("note"));
            ...
            fileInfo.put("note", note);
            fileInfo.put("CREATE_BY_", ContextUtil.getCurrentUserId());
            fileInfo.put("CREATE_TIME_", new Date());
            qbgzDao.addFileInfos(fileInfo);
        } catch (Exception e) {
            logger.error("Exception in saveUploadFiles", e);
        }
    }

    public JSONObject getQbgzDetail(String qbgzId) {
        JSONObject detailObj = qbgzDao.queryQbgzById(qbgzId);
        if (detailObj == null) {
    ...
    public List<JSONObject> getQbgzFileList(List<String> qbgzIdList) {
        List<JSONObject> qbgzFileList = new ArrayList<>();
        Map<String, Object> param = new HashMap<>();
        param.put("qbgzIds", qbgzIdList);
        qbgzFileList = qbgzDao.queryQbgzFileList(param);
        return qbgzFileList;
    }

    public void deleteOneQbgzFile(String fileId, String fileName, String qbgzId) {
        rdmZhglFileManager.deleteOneFileFromDisk(fileId, fileName, qbgzId, WebAppUtil.getProperty("qbgzFilePathBase"));
        Map<String, Object> param = new HashMap<>();
        param.put("fileId", fileId);
        param.put("belongId", qbgzId);
        qbgzDao.deleteQbgzFile(param);
    }

    public JsonResult deleteQbgz(String[] ids) {
        JsonResult result = new JsonResult(true, "操作成功！");
        List<String> qbgzIds = Arrays.asList(ids);
        List<JSONObject> files = getQbgzFileList(qbgzIds);
        String qbgzFilePathBase = WebAppUtil.getProperty("qbgzFilePathBase");
        for (JSONObject oneFile : files) {
            rdmZhglFileManager.deleteOneFileFromDisk(oneFile.getString("fileId"), oneFile.getString("fileName"),
                oneFile.getString("belongId"), qbgzFilePathBase);
        }
        for (String oneJstbId : ids) {
            rdmZhglFileManager.deleteDirFromDisk(oneJstbId, qbgzFilePathBase);
        }
        Map<String, Object> param = new HashMap<>();
        param.put("qbgzIds", qbgzIds);
        qbgzDao.deleteQbgzFile(param);
        qbgzDao.deleteQbgz(param);
        return result;
    }
    ...
    public ResponseEntity<byte[]> importTemplateDownload() {
        try {
            ...
            // 创建文件实例
            File file = new File(QbgzService.class.getClassLoader().getResource("templates/qbgc/" + fileName).toURI());
