 ...
 */
public class SpiderProductContent implements PageProcessor {
    private Logger logger = Logger.getLogger(SpiderProductContent.class);
    private Site site = Site.me().setUserAgent(
        "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_2) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.65 Safari/537.31")
        .setRetryTimes(3).setSleepTime(1000).setTimeOut(10000);
    private JSONObject configObj;

    public SpiderProductContent() {}

    public SpiderProductContent(JSONObject configObj) {
    ...
    public void process(Page page) {
        String content=page.getHtml().xpath("//div[@class='techData pro_pad']").toString();
