 ...
 * 铁甲网的新品模块分析
 */
public class SpiderNewsList implements PageProcessor {
    private Logger logger = Logger.getLogger(SpiderNewsList.class);
    ...
    public SpiderNewsList() {}

    public SpiderNewsList(JSONObject configObj) {
    ...
    public void process(Page page) throws ParseException {
        ...
        for (JSONObject parentPage : result) {
            Spider.create(new SpiderNewsContent(parentPage)).addUrl(parentPage.getString("subUrl")).thread(1).runAsync();
        }
