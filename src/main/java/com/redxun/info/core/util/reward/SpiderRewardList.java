 ...
 * 铁甲网的新品模块分析
 */
public class SpiderRewardList implements PageProcessor {
    private Logger logger = Logger.getLogger(SpiderRewardList.class);
    ...
    public SpiderRewardList() {}

    public SpiderRewardList(JSONObject configObj) {
    ...
    public void process(Page page)  {
        ...
        int onePageNewSize = page.getHtml().xpath("//ul[@class='tpl2_list mb10']/li").nodes().size();
        ...
        try {
            for (int index = 1; index <= onePageNewSize; index++) {
                ...
                String title = page.getHtml().xpath("//ul[@class='tpl2_list mb10']/li[" + index + "]/span/a/text()").toString();
                oneNew.put("title", title);
                if(title.indexOf("奖")==-1){
                    continue;
                }
                //判断是否存在，存在则不导入
            ...
            for (JSONObject parentPage : result) {
                Spider.create(new SpiderRewardContent(parentPage)).addUrl(parentPage.getString("subUrl")).thread(1).runAsync();
            }
