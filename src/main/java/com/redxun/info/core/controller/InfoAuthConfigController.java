...
import com.redxun.info.core.manager.InfoAuthConfigManager;
...
public class InfoAuthConfigController {
    @Resource
    InfoAuthConfigManager authConfigManager;
    /**
     * 获取编辑页面
     * */
    @RequestMapping("editPage")
    public ModelAndView getEditPage(HttpServletRequest request, HttpServletResponse response) {
        String jspPath = "info/config/infoAuthConfigEditPage.jsp";
        ...
        if(ConstantUtil.FORM_EDIT.equals(action)||ConstantUtil.FORM_VIEW.equals(action)){
            applyObj = authConfigManager.getObjectById(id);
        }
    ...
    public ModelAndView getListPage(HttpServletRequest request, HttpServletResponse response) {
        String jspPath = "info/config/infoAuthConfigListPage.jsp";
        ModelAndView mv = new ModelAndView(jspPath);
        return mv;
    }
    /**
    ...
    public JsonPageResult<?> getListData(HttpServletRequest request, HttpServletResponse response) {
        return authConfigManager.query(request);
    }
    @RequestMapping("save")
    @ResponseBody
    public JSONObject save(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(StringUtil.isEmpty(id)){
            resultJSON = authConfigManager.add(request);
        }else{
            resultJSON = authConfigManager.update(request);
        }
        return resultJSON;
    }
    ...
    public JSONObject remove(HttpServletRequest request, HttpServletResponse response) {
        JSONObject resultJSON = null;
        resultJSON = authConfigManager.remove(request);
        return resultJSON;
    }
}
