...
import com.redxun.info.core.manager.InfoPaperManager;
...
public class InfoPaperController {
    @Resource
    InfoPaperManager infoPaperManager;
    @Resource
    SpiderManager spiderManager;
    /**
     * 获取列表页面
     * */
    @RequestMapping("paperListPage")
    public ModelAndView getIndexListPage(HttpServletRequest request, HttpServletResponse response) {
        String jspPath = "info/paper/infoPaperList.jsp";
    ...
    @ResponseBody
    public JsonPageResult<?> paperList(HttpServletRequest request, HttpServletResponse response) {
        return infoPaperManager.query(request);
    }
    /**
     * 获取新闻详情
     * */
    ...
    public ModelAndView getDetailViewPage(HttpServletRequest request, HttpServletResponse response) {
        String jspPath = "info/paper/infoDetailView.jsp";
        ModelAndView mv = new ModelAndView(jspPath);
        ...
        paramJSON.put("id", id);
        applyObj = infoPaperManager.getObjectById(id);
    ...
    public ModelAndView getEditPage(HttpServletRequest request, HttpServletResponse response) {
        String jspPath = "info/paper/infoPaperEdit.jsp";
        ModelAndView mv = new ModelAndView(jspPath);
    ...
    public JSONObject save(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(StringUtil.isEmpty(id)){
            resultJSON = infoPaperManager.add(request);
        }else{
            resultJSON = infoPaperManager.update(request);
        }
        return resultJSON;
    }
    ...
    public JSONObject remove(HttpServletRequest request, HttpServletResponse response) {
        JSONObject resultJSON = null;
        resultJSON = infoPaperManager.remove(request);
        return resultJSON;
    }
}
