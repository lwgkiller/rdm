...
import com.redxun.portrait.core.dao.PortraitContractDao;
 ...
 */
@Service
public class PortraitContractManager {
    private static final Logger logger = LoggerFactory.getLogger(PortraitContractManager.class);
    @Resource
    PortraitContractDao portraitContractDao;
    @Autowired
    OsUserDao osUserDao;
    @Resource
        ...
        try {
            ...
            if ("admin".equals(ContextUtil.getCurrentUser().getUserNo())) {
                ...
                if (resultJson.getBoolean(RdmConst.ALLDATA_QUERY_KEY) || resultJson.getBoolean("HX-GLY")) {
                    ...
                    Map<String,String> deptMap=commonInfoManager.queryDeptUnderJSZX();
                    idList.addAll(deptMap.keySet());
                } else if (resultJson.getBoolean("contractAdmin")) {
                    Map<String,String> deptMap=commonInfoManager.queryDeptUnderJSZX();
                    idList.addAll(deptMap.keySet());
            ...
            list = portraitContractDao.query(params);
            ...
            if (!resultJson.getBoolean(RdmConst.ALLDATA_QUERY_KEY) && !resultJson.getBoolean("isLeader") && !resultJson.getBoolean("HX-GLY")
                && !resultJson.getBoolean("contractAdmin")
                && !"admin".equals(ContextUtil.getCurrentUser().getUserNo())) {
                params.put("userId", ContextUtil.getCurrentUserId());
            }
            totalList = portraitContractDao.query(params);
    ...
    public JSONObject asyncContract() {
        try {
            List<JSONObject> list = portraitContractDao.getContractList();
            ...
            for (JSONObject temp : list) {
                ...
                objBody.put("userId", temp.getString("signerUserId"));
                objBody.put("signYear", temp.getString("signYear"));
                objBody.put("mainId", temp.getString("id"));
                objBody.put("score", contractScore);
                tempInsert.add(objBody);
                portraitContractDao.updateContractStatus(temp.getString("id"));
                if (tempInsert.size() % 20 == 0) {
                    portraitContractDao.batchInsert(tempInsert);
                    tempInsert.clear();
                }
            }
            ...
            logger.error("Exception in add asyncContract！", e);
            return ResultUtil.result(false,"同步失败",null);
        }
        return ResultUtil.result(true,"同步成功",null);
    }
    public JSONObject getObjectById(String id) {
        return portraitContractDao.getObjectById(id);
    }

    public List<JSONObject> getPersonContractList(JSONObject jsonObject){
        List<JSONObject> list = portraitContractDao.getPersonContractList(jsonObject);
