...
import com.redxun.portrait.core.dao.PortraitSGYKDao;
 ...
 */
@Service
public class PortraitSGYKManager {
    private static final Logger logger = LoggerFactory.getLogger(PortraitSGYKManager.class);
    @Resource
    PortraitSGYKDao portraitSGYKDao;
    @Autowired
    OsUserDao osUserDao;
    @Resource
    ...
    public JsonPageResult<?> query(HttpServletRequest request) {
        ...
        try {
            ...
            list = portraitSGYKDao.query(params);
            ...
            totalList = portraitSGYKDao.query(params);
    ...
    public JSONObject add(HttpServletRequest request) {
        ...
        try {
            ...
            for(String userId:userArray){
                ...
                portraitSGYKDao.addObject(objBody);
            }
        } catch (Exception e) {
            logger.error("Exception in add 添加三高一可异常！", e);
            return ResultUtil.result(false, "添加三高一可异常！", "");
        }
    ...
    public JSONObject update(HttpServletRequest request) {
        ...
        try {
            ...
            objBody.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
            objBody.put("UPDATE_TIME_", XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
            portraitSGYKDao.updateObject(objBody);
        } catch (Exception e) {
            logger.error("Exception in update 更新三高一可异常", e);
            return ResultUtil.result(false, "更新三高一可异常！", "");
        }
    ...
    public JSONObject remove(HttpServletRequest request) {
        ...
        try {
            ...
            params.put("ids", idList);
            portraitSGYKDao.batchDelete(params);
        } catch (Exception e) {
            logger.error("Exception in update 删除三高一可", e);
            return ResultUtil.result(false, "删除三高一可异常！", "");
        }
        return ResultUtil.result(true, "删除成功", resultJson);
    }
    public JSONObject getObjectById(String id){
        return portraitSGYKDao.getObjectById(id);
    }

    public JSONArray getSgykByUserId(JSONObject jsonObject){
        return portraitSGYKDao.getSgykByUserId(jsonObject);
    }
    ...
    public ResponseEntity<byte[]> importTemplateDownload() {
        try {
            String fileName = "三高一可贡献导入模板.xls";
            // 创建文件实例
            File file = new File(
    ...
    public void importSgyk(JSONObject result, HttpServletRequest request) {
        try {
            MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest)request;
            MultipartFile fileObj = multipartRequest.getFile("sgykImportFile");
            if (fileObj == null) {
                result.put("message", "数据导入失败，内容为空！");
                return;
            }
            HSSFWorkbook wb = new HSSFWorkbook(fileObj.getInputStream());
            HSSFSheet sheet = wb.getSheet("三高一可贡献");
            if (sheet == null) {
                logger.error("找不到三高一可贡献导入页");
                result.put("message", "数据导入失败，找不到三高一可贡献导入页！");
                return;
            }
            ...
            for (int i = 0; i < dataInsert.size(); i++) {
                ...
                if (tempInsert.size() % 20 == 0) {
                    portraitSGYKDao.batchInsertSgyk(tempInsert);
                    tempInsert.clear();
                }
            }
            ...
            logger.error("Exception in importSgyk", e);
    ...
    private JSONObject generateDataFromRow(HSSFRow row, List<Map<String, Object>> dataInsert, List<String> titleList,
        ...
        for (int i = 0; i < titleList.size(); i++) {
            ...
            switch (title) {
                ...
                case "年月":
                    if (StringUtils.isBlank(cellValue)) {
                        oneRowCheck.put("message", "年月为空");
                        return oneRowCheck;
                    }
                    oneRowMap.put("yearMonth", cellValue);
                    break;
                case "指标名称":
                    if (StringUtils.isBlank(cellValue)) {
                ...
                case "完成情况":
                    if (!StringUtils.isBlank(cellValue)) {
                        oneRowMap.put("finishRate", cellValue);
                    }else{
                        oneRowMap.put("finishRate", "");
                    }
                    break;
                case "权重":
                    if (!StringUtils.isBlank(cellValue)) {
                        oneRowMap.put("weight", cellValue);
                    }else{
                        oneRowMap.put("weight", "");
                    }
                    break;
                case "得分":
                    if (StringUtils.isBlank(cellValue)) {
                        oneRowCheck.put("message", "得分为空");
                        return oneRowCheck;
                    }
        ...
        if(StringUtil.isEmpty(certNo)){
            ...
            if(list!=null && list.isEmpty()){
                ...
                userId = list.get(0).getString("USER_ID_");
             ...
             userId = CommonFuns.nullToString(userNo2UserId.get(certNo));
        ...
        List<JSONObject> list = portraitSGYKDao.getIndexIdByName(paramJson);
        if(list!=null&&list.size()==1){
            oneRowMap.put("indexId", list.get(0).getString("id"));
        }else if(list!=null&&list.size()==0){
            oneRowCheck.put("message", "根据指标名称未匹配到相应的指标！");
            return oneRowCheck;
        }else if(list!=null&&list.size()>1){
            oneRowCheck.put("message", "根据指标名称匹配到多个指标！");
            return oneRowCheck;
        }else{
            oneRowCheck.put("message", "根据指标名称匹配到指标出错！");
            return oneRowCheck;
        }
