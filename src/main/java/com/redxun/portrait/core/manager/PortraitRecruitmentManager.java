...
import com.redxun.portrait.core.dao.PortraitRecruitmentDao;
 ...
 */
@Service
public class PortraitRecruitmentManager {
    private static final Logger logger = LoggerFactory.getLogger(PortraitRecruitmentManager.class);
    @Resource
    PortraitRecruitmentDao portraitRecruitmentDao;
    @Autowired
    OsUserDao osUserDao;
    @Resource
    ...
    public JsonPageResult<?> query(HttpServletRequest request) {
        ...
        try {
            ...
            if("admin".equals(ContextUtil.getCurrentUser().getUserNo())||resultJson.getBoolean(RdmConst.ALLDATA_QUERY_KEY)||resultJson.getBoolean("HX-GLY")) {
            ...
            list = portraitRecruitmentDao.query(params);
            ...
            params = new HashMap<>(16);
            params = CommonFuns.getSearchParam(params, request, false);
            totalList = portraitRecruitmentDao.query(params);
            convertDate(list);
            // 返回结果
            result.setData(list);
    ...
    public JSONObject add(HttpServletRequest request) {
        ...
        try {
            ...
            portraitRecruitmentDao.addObject(objBody);
        } catch (Exception e) {
            logger.error("Exception in add 添加荣誉信息！", e);
            return ResultUtil.result(false, "保存更新荣誉信息信息异常！", "");
        }
    ...
    public JSONObject update(HttpServletRequest request) {
        ...
        try {
            ...
            objBody.put("UPDATE_BY_", ContextUtil.getCurrentUserId());
            objBody.put("UPDATE_TIME_",XcmgProjectUtil.getNowUTCDateStr("yyyy-MM-dd HH:mm:ss"));
            portraitRecruitmentDao.updateObject(objBody);
        } catch (Exception e) {
            logger.error("Exception in update 更新荣誉信息异常", e);
            return ResultUtil.result(false, "更新荣誉信息异常！", "");
        }
    ...
    public JSONObject remove(HttpServletRequest request) {
        ...
        try {
            ...
            portraitRecruitmentDao.batchDelete(params);
        } catch (Exception e) {
            logger.error("Exception in update 删除荣誉信息", e);
            return ResultUtil.result(false, "删除荣誉信息异常！", "");
        }
        return ResultUtil.result(true, "删除成功", resultJson);
    }
    public JSONObject getObjectById(String id){
        return portraitRecruitmentDao.getObjectById(id);
    }
